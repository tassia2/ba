...
static const char *PARAM_FILENAME = "ex10_MagnusEffect.xml";
...
void ExamCFD::time_loop() 
{
  ...
  while (time < T)
  {
    ...
    if (bdf_type == 2 && step == 1)
    {
      ...
      // update LHS and RHS
      this->eval_LHS_and_RHS(&matrix_, &rhs_, 0.5 * dt, 1);
    }
    else 
    {
      // update time
      ...
      // update LHS and RHS
      this->eval_LHS_and_RHS(&matrix_, &rhs_, dt, bdf_type);
...
typename LAD::DataType ExamCFD::compute_kinetic_energy(VectorType &u) 
{
  ...
  PostProcessingIntegral< DIM, LAD > int_div(u, 0, params_["Equation"]["nu"].get< DataType >());
  this->global_asm_.integrate_scalar(space_, int_div, local);

  // collect results from all parallel processes
  this->parcom_->sum(local, recv);
...
typename LAD::DataType ExamCFD::compute_dissipation_energy(VectorType &u) 
{
  ...
  PostProcessingIntegral< DIM, LAD > int_div(u, 1, params_["Equation"]["nu"].get< DataType >());
  this->global_asm_.integrate_scalar(space_, int_div, local);
