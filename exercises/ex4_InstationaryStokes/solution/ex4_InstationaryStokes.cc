...
#include "ex4_InstationaryStokes.h"

#ifdef WITH_GPERF
#include "profiler.h"
#endif

static const char *PARAM_FILENAME = "ex4_InstationaryStokes.xml";
...
void CavityStokes::compute_bc(DataType time)
{
  ...
  VelocityDirichletBC bc_dirichlet(time,
                                   params_["Equation"]["TopMaterialNumber"].get< int >(),
...
void CavityStokes::assemble_system(DataType time) 
{  
...
void CavityStokes::solve_linear_system() 
{
...
void CavityStokes::time_loop() 
{
  ...
  while (time < T)
  {
    ...
    std::vector<DataType> cur_pp_vals(2*(DIM+1)+2, 0.);
    cur_pp_vals[0] = time;
    ...
    this->evaluate_solution(sol_, cur_pp_vals);
...
void CavityStokes::visualize(int time_step) 
{     
...
void CavityStokes::evaluate_solution(VectorType &u, std::vector<DataType>& cur_pp_vals) 
{
...
typename LAD::DataType CavityStokes::compute_L2_divergence(VectorType &u) 
{
  DataType recv = 0.;
  DataType l2_div = 0.;
  u.Update();
