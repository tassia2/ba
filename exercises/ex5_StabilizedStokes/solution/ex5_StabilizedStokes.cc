...
#include "ex5_StabilizedStokes.h"

#ifdef WITH_GPERF
#include "profiler.h"
#endif

static const char *PARAM_FILENAME = "ex5_StabilizedStokes.xml";
...
class StabilizedStokes {
public:
  StabilizedStokes(const std::string &param_filename,
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    StabilizedStokes app(param_filename, path_mesh);
    app.run();
...
//////////////// StabilizedStokes implementation //////////////

void StabilizedStokes::build_initial_mesh() {
...
void StabilizedStokes::prepare_system() 
{
...
{ 
  ...
  local_asm.set_parameters(params_["Equation"]["nu"].get< DataType >(), 
                           params_["Equation"]["PressureStabType"].get< int >(),
...
void StabilizedStokes::compute_error() 
{ 
  ExactSol exact_sol;
...
void StabilizedStokes::solve_system() 
{
...
void StabilizedStokes::visualize() {
...
typename LAD::DataType StabilizedStokes::compute_pressure_int(VectorType &u) 
{
  DataType recv;
...
typename LAD::DataType StabilizedStokes::compute_volume_int() 
{
...
typename LAD::DataType StabilizedStokes::compute_average_pressure(VectorType &u) 
{
...
void StabilizedStokes::ApplyFilter(VectorType &u) 
{
  if (params_["Equation"]["PressureStabType"].get< int >() == 1)
  {
    return;
  }
