                                   ...
                                   DataType &lift_force) 
{
  ...
  ForceIntegral< DIM, LAD > int_drag(mu, &u, 0, obstacle_mat_num);
  this->global_asm_.integrate_scalar_boundary(space_, int_drag, local_drag);

  // collect results from all parallel processes
  drag_force = 0.;
  this->parcom_->sum(local_drag, drag_force);
    
  ForceIntegral< DIM, LAD > int_lift(mu, &u, 1, obstacle_mat_num);
  this->global_asm_.integrate_scalar_boundary(space_, int_lift, local_lift);

  // collect results from all parallel processes
  lift_force = 0.;
  this->parcom_->sum(local_lift, lift_force);
