        ...
        for (int j = 0; j < num_dof; ++j) 
        {
              ...
              && this->first_dof_for_var(0) <= j && j < this->last_dof_for_var(DIM-1))
          {
            ...
            if (this->equation_type_ == 1)
            {
              ...
              // nu * grad(u) : grad(v)
              l0 = this->nu_ * dot(DphiV_j, DphiV_i);
              for (int v=0; v!= DIM; ++v)
              {
                for (int d=0; d!= DIM; ++d)
                {
                  // u_k * grad(u)^T * v
                  l1 += vk[d] * DphiV_j(v,d) * phiV_i[v];
                  
                  // u * grad(u_k)^T * v
                  l2 += phiV_j[d] * Dvk(v,d) * phiV_i[v];
                }
              }
            }
            else
              ...
              && this->first_dof_for_var(DIM) <= j && j < this->last_dof_for_var(DIM))
          {
            // TODO EXERCISE B
            // - p * div(v)
            DataType l3 = - phiP_j * trace(DphiV_i);
            lm(i, j) += wq * l3 * dJ;
            // END EXERCISE B
          }
              ...
              && this->first_dof_for_var(0) <= j && j < this->last_dof_for_var(DIM-1))
          {
            // TODO EXERCISE B
            // div(u) * q
            DataType l4 = phiP_i * trace(DphiV_j);
            lm(i,j) += wq * l4 * dJ;
            // END EXERCISE B
          }
        }
