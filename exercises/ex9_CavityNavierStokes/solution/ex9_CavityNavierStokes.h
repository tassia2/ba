        ...
        for (int j = 0; j < num_dof; ++j) 
        {
              ...
              && this->first_dof_for_var(0) <= j && j < this->last_dof_for_var(DIM-1))
          {
            ...
            if (solver_type_ == 0)
            {
              ...
              DataType l1 = this->nu_ * dot(DphiV_j, DphiV_i);
              DataType l2 = 0.;
              ...
              for (int v=0; v!= DIM; ++v)
              {
                ...
                {
                  // u_k * grad(u)^T * v
                  l2 += vn[d] * DphiV_j(v,d) * phiV_i[v];
                }
              }
          
              lm(i,j) += wq * (l0 + dt_ * theta_ * (l1 + l2)) * dJ;
              // END EXERCISE A
            }
      ...
      { 
        ...
        if ( this->first_dof_for_var(0) <= i && i < this->last_dof_for_var(DIM-1) )
        {
          ...
          if (solver_type_ == 0)
          {
            // TODO exercise A
            // Semi-implicit formulation:
            DataType l0_n = dot(vn, phiV_i);
            DataType l1_n = this->nu_ * dot(Dvn, DphiV_i);
            DataType lf_k = dot(this->force(this->x(q), this->time_), phiV_i);
            DataType lf_n = dot(this->force(this->x(q), this->time_-dt_), phiV_i);
          ...
          else if (solver_type_ == 2)
          {
            ...
            lf_k = -dot(this->force(this->x(q), this->time_), phiV_i);
            lf_n = -dot(this->force(this->x(q), this->time_-dt_), phiV_i);
            lp = - pk * trace(DphiV_i); 

            // Linearized convection term for Newton method: 
            for (int v=0; v!= DIM; ++v)
            {
              for (int d=0; d!= DIM; ++d)
              {
