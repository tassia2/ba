...
template < class T > inline void read_binary(std::istream &is, T &val) {
  char bytes[sizeof(T)];
...
template < class T >
inline void read_binary(std::istream &is, std::vector< T > &val) {
  array_size_t sz;
  read_binary(is, sz);
...
inline void read_binary(std::istream &is, std::string &str) {
  array_size_t sz;
  read_binary(is, sz);
  std::vector< char > buf(sz * sizeof(char));
  if (sz > 0) {
    is.read(&buf[0], static_cast< std::streamsize >(sz * sizeof(char)));
    str = std::string(&buf[0], sz);
  }
}

template < class T > inline void write_binary(std::ostream &os, T val) {
  char bytes[sizeof(T)];
...
template < class T >
inline void write_binary(std::ostream &os, const std::vector< T > &vec) {
  write_binary(os, static_cast< array_size_t >(vec.size()));

  if (vec.size() > 0) {
    os.write(reinterpret_cast< char * >(const_cast< T * >(&vec[0])),
             vec.size() * sizeof(T));
  }
}

inline void write_binary(std::ostream &os, const std::string &str) {
  write_binary(os, static_cast< array_size_t >(str.size()));

  if (!str.empty()) {
    os.write(str.data(),
             static_cast< std::streamsize >(str.size() * sizeof(char)));
  }
}
} // namespace hiflow
#endif
