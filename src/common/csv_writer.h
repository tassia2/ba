...
/// @author Simon Gawlok
/// @brief Class for handling CSV files. Provides functionality to create, write
/// and read CSV files. Templatization is used such that I/O with data of
/// different types can be done.

template < class T > class CSVWriter {
public:
  /// Constructor
  /// \param[in] filename Filename of CSV file

  CSVWriter(std::string filename) { filename_ = filename; }
  ...
  void InitFilename(std::string filename) { filename_ = filename; }

  /// Initialize CSV file with headings of columns.
  /// WARNING: If file already exists, it will be overwritten!!!
  /// \param[in] col_names Vector of strings containing the headings of the
  /// columns in the created CSV file
  ...
  void Init(std::vector< std::string > col_names) {
    ...
    if (out_file.is_open()) {
      ...
      std::cerr << "Could not create CSV file " << filename_ << "!"
                << std::endl;
      quit_program();
    }
  }
  ...
  /// CSV file.
  /// \param[in] row_of_values Vector containing the entries for the next
  /// line/dataset
  // in the CSV file.
  ...
  /// as a vector (each element is a dataset) of vectors (the datasets
  /// themselves). \param[out] values Read-in content of CSV file
