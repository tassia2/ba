...
template < class Data > class HierarchicalReport {
public:
  inline HierarchicalReport();
  virtual inline ~HierarchicalReport();

  // Start a new section.
  inline Data *begin_section(const std::string &name,
...
template < class Data > HierarchicalReport< Data >::HierarchicalReport() {
  // Create root node.
  parent_.push_back(-1);
...
template < class Data > HierarchicalReport< Data >::~HierarchicalReport() {
  for (typename std::vector< Data * >::iterator it = data_.begin(),
                                                end_it = data_.end();
       it != end_it; ++it) {
    delete *it;
  }
...
template < class Data > Data *HierarchicalReport< Data >::end_section() {
  if (curr_ == -1) {
