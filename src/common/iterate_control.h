...
public:
  ...
  void Init(int maxits, double atol, double rtol, double dtol);
  State Check(int current_step, double current_value);

  void set_first(int first_step, double first_value);

  double first_value() const { return first_value_; }

  void set_first_value(double first_value) { this->first_value_ = first_value; }

  int first_step() const { return first_step_; }

  int maxits() const { return maxits_; }

  double absolute_tol() const { return absolute_tol_; }

  double relative_tol() const { return relative_tol_; }

  double divergence_tol() const { return divergence_tol_; }

  State status() const { return status_; }

  void set_status(State status) { this->status_ = status; }
