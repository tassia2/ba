...
Log::Log() : target_(&std::cerr) {}

Log::Log(std::ostream *target) : target_(target) {}
...
void Log::flush() {
  if (target_ != 0) {
    for (std::vector< LogEntry * >::const_iterator it = entries_.begin();
      ...
      if (*it != 0) {
        *target_ << "[" 
                 << std::setw(25) << std::left << (*it)->file_name 
                 << ":" 
                 << std::setw(4) << std::left << (*it)->line_number 
                 << "] "
                 << (*it)->message.str() 
                 << "\n";
      }
...
std::ostream *Log::get_target() const { return target_; }

void Log::set_target(std::ostream *target) { target_ = target; }
...
std::ostream &Log::create_entry(const char *file, const char *function,
  ...
  last_slash = last_slash != 0 ? last_slash + 1 : file;
...
Log &LogKeeper::create_log(const char *log_name, std::ostream *target) {
  ...
  if (instance()->logs_.count(name) == 0) {
    ...
    if (name == "info") {
      time_t tp;
      time(&tp);
      instance()->logs_[name]->create_entry("INFO", "", 0, LOG_INFO_TAG)
          << std::setw(20) << std::left << "Time " << " : " << std::asctime(localtime(&tp));
    }
#endif
  }
  return *(instance()->logs_[name]);
...
LogKeeper::~LogKeeper() {
  for (std::map< std::string, Log * >::iterator it = logs_.begin();
       it != logs_.end(); ++it) {
    Log *log = it->second;
    log->flush();
    delete log;
...
LogKeeper *LogKeeper::instance() { return &instance_; }
