...
void TableColumn::insert(const std::string &str) { values_.push_back(str); }

int TableColumn::num_rows() const { return static_cast< int >(values_.size()); }
...
const std::ios_base::fmtflags Table::DEFAULT_FORMAT =
    std::ios_base::left | std::ios_base::boolalpha | std::ios_base::showpoint |
    std::ios_base::dec | std::ios_base::fixed;

Table::Table() : format_(Table::DEFAULT_FORMAT), precision_(6) {}
...
void Table::print(std::ostream &os,
  ...
  const size_t num_cols = col_iters.size();
  const int col_width = compute_column_width(col_iters, num_rows, 10);
  const int table_width = static_cast< int >(num_cols) * (col_width + 1);
...
std::ios_base::fmtflags Table::format() const { return format_; }

void Table::set_format(std::ios_base::fmtflags format) { format_ = format; }

void Table::set_precision(int precision) { precision_ = precision; }
...
int Table::compute_column_width(const std::vector< ColumnIterator > &col_iters,
                                int num_rows, int min_width) const {
  ...
  for (int r = 0; r != num_rows; ++r) {
    for (size_t c = 0; c != num_cols; ++c) {
      col_width =
          std::max(col_width, int(col_iters[c]->second.value(r).length()));
    }
  }
  return col_width;
}
