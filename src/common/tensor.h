...
template < int R > class TensorIndex {
public:
  TensorIndex() {}
  ...
  int dim(int r) const { return strides_[r] / strides_[r + 1]; }

  int size() const { return strides_[0]; }

  int operator()(int i) const { return checked_index(i * stride(0)); }
...
inline TensorIndex< 0 > make_tensor_index() {
  int s[1] = {1};
  return TensorIndex< 0 >(s);
}

inline TensorIndex< 1 > make_tensor_index(int dim1) {
  int s[2] = {dim1, 1};
  return TensorIndex< 1 >(s);
}

inline TensorIndex< 2 > make_tensor_index(int dim1, int dim2) {
  int s[3] = {dim1 * dim2, dim2, 1};
  return TensorIndex< 2 >(s);
}

inline TensorIndex< 3 > make_tensor_index(int dim1, int dim2, int dim3) {
  int s[4] = {dim1 * dim2 * dim3, dim2 * dim3, dim3, 1};
  return TensorIndex< 3 >(s);
}

inline TensorIndex< 4 > make_tensor_index(int dim1, int dim2, int dim3,
                                          int dim4) {
  int s[5] = {dim1 * dim2 * dim3 * dim4, dim2 * dim3 * dim4, dim3 * dim4, dim4,
              1};
  return TensorIndex< 4 >(s);
}

inline TensorIndex< 5 > make_tensor_index(int dim1, int dim2, int dim3,
                                          int dim4, int dim5) {
  int s[6] = {dim1 * dim2 * dim3 * dim4 * dim5,
              dim2 * dim3 * dim4 * dim5,
              dim3 * dim4 * dim5,
              dim4 * dim5,
              dim5,
...
inline TensorIndex< 6 > make_tensor_index(int dim1, int dim2, int dim3,
...
public:
  Tensor() : is_owner_(false), values_(0) {}

  Tensor(const TensorIndex< R > &index, double *values = 0,
  ...
  double &operator()(int i) { return values_[index_(i)]; }

  const double &operator()(int i) const { return values_[index_(i)]; }

  double &operator()(int i, int j) { return values_[index_(i, j)]; }

  const double &operator()(int i, int j) const { return values_[index_(i, j)]; }

  double &operator()(int i, int j, int k) { return values_[index_(i, j, k)]; }
  ...
  double *as_array() { return values_; }
