...
public:
  ...
  /// Stop the timer and add the time since the last call to start() (or the
  /// constructor) to its duration.
  inline void stop();
...
double Timer::get_duration() const {
  ...
  return get_seconds() + static_cast< double >(get_micro_seconds()) /
                             static_cast< double >(us_per_s);
}

long Timer::get_seconds() const { return _duration.tv_sec; }

long Timer::get_micro_seconds() const { return _duration.tv_usec; }
