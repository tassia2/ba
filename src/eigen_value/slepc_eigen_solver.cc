...
void Complex2Real(const std::vector< PetscScalar > &vec,
...
void Complex2Imag(const std::vector< PetscScalar > &vec,
...
void Real2Complex(const std::vector< PetscReal > &real,
                  std::vector< PetscScalar > &vec, double s) {
  if (s != 0) {
...
void Imag2Complex(const std::vector< PetscReal > &imag,
                  std::vector< PetscScalar > &vec, double s) {
  if (s != 0) {
...
template < class LAD > PetscErrorCode MatVecMult(Mat A, Vec x, Vec y) {
  void *ctx;
...
template < class LAD > PetscErrorCode MatTransVecMult(Mat A, Vec x, Vec y) {
  void *ctx;
  PetscErrorCode ierr = 0;
  Operator_wrapper< LAD > op_ptr;
  std::vector< int > ind;
...
SLEPcEigenSolver< LAD >::SLEPcEigenSolver(slepc::EPS_wrapper *ptr_eps_wrapper)
    : ptr_eps_wrapper_(ptr_eps_wrapper), comm_(MPI_COMM_NULL), rank_(-1),
      nb_procs_(-1), mat_A_set_(false), mat_B_set_(false),
      ptr_A_wrapper_(new petsc::Mat_wrapper),
      ptr_B_wrapper_(new petsc::Mat_wrapper), EigenSolver< LAD >() {
...
SLEPcEigenSolver< LAD >::SLEPcEigenSolver(slepc::SVD_wrapper *ptr_svd_wrapper)
    : ptr_svd_wrapper_(ptr_svd_wrapper), rank_(-1), nb_procs_(-1),
      comm_(MPI_COMM_NULL), mat_A_set_(false), mat_B_set_(false),
      ptr_A_wrapper_(new petsc::Mat_wrapper),
      ptr_B_wrapper_(new petsc::Mat_wrapper), EigenSolver< LAD >() {
  struct_A_.is_set = false;
  struct_Ai_.is_set = false;
...
template < class LAD >
void SLEPcEigenSolver< LAD >::SetupMatrixStructure(
    const LaCouplings &cp, const std::vector< int > &d_rows,
    const std::vector< int > &d_cols, const std::vector< int > &o_rows,
    const std::vector< int > &o_cols, slepc::MatrixStructure &m_struct) {
  ...
  for (int l = 0; l < m_struct.diag_nnz; ++l)
    m_struct.nz_col_ind_in_diag_row[m_struct.diag_rows[l] - m_struct.dof_offset]
        .push_back(m_struct.diag_cols[l]);

  for (int l = 0; l < m_struct.off_diag_nnz; ++l)
    m_struct
        .nz_col_ind_in_offdiag_row[m_struct.off_diag_rows[l] -
                                   m_struct.dof_offset]
        .push_back(m_struct.off_diag_cols[l]);
...
template < class LAD >
void SLEPcEigenSolver< LAD >::SetupMatrixStructure(
    const LaCouplings &cp, slepc::MatrixStructure &m_struct) {
...
template < class LAD >
void SLEPcEigenSolver< LAD >::ClearMatrixStructure(
...
template <>
void SLEPcEigenSolver< LADescriptorCoupledD >::InitVec(const LaCouplings &cp) {
...
#ifdef WITH_HYPRE
template <>
void SLEPcEigenSolver< LADescriptorHypreD >::InitVec(const LaCouplings &cp) {
...
template <>
void SLEPcEigenSolver< LADescriptorPETSc >::InitVec(const LaCouplings &cp) {
  aux_vec_.Init(this->comm_, cp);
}
#endif

template < class LAD >
void SLEPcEigenSolver< LAD >::SetupOperatorA(
    const OperatorType *opA, const OperatorType *opAi, const LaCouplings &cp,
    const std::vector< int > &d_rows, const std::vector< int > &d_cols,
    const std::vector< int > &o_rows, const std::vector< int > &o_cols) {
...
template < class LAD >
void SLEPcEigenSolver< LAD >::SetupOperatorA(const OperatorType *opA,
                                             const OperatorType *opAt,
                                             const OperatorType *opAi,
                                             const OperatorType *opAit,
...
template < class LAD >
void SLEPcEigenSolver< LAD >::SetupOperatorB(
    const OperatorType *opB, const OperatorType *opBi, const LaCouplings &cp,
    const std::vector< int > &d_rows, const std::vector< int > &d_cols,
    const std::vector< int > &o_rows, const std::vector< int > &o_cols) {
  ...
  //	std::cout << opB << " " << opBi << " " << struct_B_.is_set << " " <<
  // struct_Bi_.is_set << std::endl;
...
template < class LAD >
void SLEPcEigenSolver< LAD >::SetupOperatorB(const OperatorType *opB,
                                             const OperatorType *opBt,
                                             const OperatorType *opBi,
                                             const OperatorType *opBit,
                                             const LaCouplings &cp) {
...
template < class LAD >
void SLEPcEigenSolver< LAD >::SetupPetscMat(
    const OperatorType *op, const OperatorType *opi,
    slepc::MatrixStructure struct_real, slepc::MatrixStructure struct_imag,
    petsc::Mat_wrapper *ptr_mat_wrapper) {
  ...
  // std::cout << this->rank_ << ": " << local_dofs_ << 	" - " <<
  // op.num_rows_local() << " / " << op.num_cols_local() << std::endl;
  MatCreate(this->comm_, &ptr_mat_wrapper->mat_);
  MatSetType(ptr_mat_wrapper->mat_, MATMPIAIJ);
  MatSetSizes(ptr_mat_wrapper->mat_, op->num_rows_local(), op->num_cols_local(),
              op->num_rows_global(), op->num_cols_global());
  MatMPIAIJSetPreallocation(ptr_mat_wrapper->mat_, 0, vec2ptr(d_nnz), 0,
                            vec2ptr(o_nnz));
  ...
  for (int l=0; l<diag_nnz_; ++l)
  {
          ...
          // std::cout << diag_rows_[l] << " , " << diag_cols_[l] << " : " <<
  val << std::endl; MatSetValue(ptr_mat_wrapper->mat_, diag_rows_[l],
  diag_cols_[l], val, INSERT_VALUES);
  }
  for (int l=0; l<off_diag_nnz_; ++l)
  {
          DataType val;
          op->GetValues(&off_diag_rows_[l], 1, &off_diag_cols_[l], 1, &val);
          //std::cout << off_diag_rows_[l] << " , " << off_diag_cols_[l] << " :
  " << val << std::endl; MatSetValue(ptr_mat_wrapper->mat_, off_diag_rows_[l],
  off_diag_cols_[l], val, INSERT_VALUES);
  }
   */
  ...
  if (struct_real.is_set) {
    for (int j = 0; j < struct_real.local_dofs; ++j) {
      ...
      op->GetValues(&cur_row, 1, &struct_real.nz_col_ind_in_diag_row[j][0],
                    nnz_diag_row, &diag_vals[0]);
      op->GetValues(&cur_row, 1, &struct_real.nz_col_ind_in_offdiag_row[j][0],
                    nnz_offdiag_row, &offdiag_vals[0]);
      ...
      MatSetValues(ptr_mat_wrapper->mat_, 1, &cur_row, nnz_diag_row,
                   &struct_real.nz_col_ind_in_diag_row[j][0],
                   &petsc_diag_vals[0], INSERT_VALUES);
      MatSetValues(ptr_mat_wrapper->mat_, 1, &cur_row, nnz_offdiag_row,
                   &struct_real.nz_col_ind_in_offdiag_row[j][0],
                   &petsc_offdiag_vals[0], INSERT_VALUES);
    }
    if (struct_imag.is_set) {
  ...
  if (struct_imag.is_set) {
    ...
    for (int j = 0; j < struct_imag.local_dofs; ++j) {
      ...
      opi->GetValues(&cur_row, 1, &struct_imag.nz_col_ind_in_diag_row[j][0],
                     nnz_diag_row, &diag_vals[0]);
      opi->GetValues(&cur_row, 1, &struct_imag.nz_col_ind_in_offdiag_row[j][0],
                     nnz_offdiag_row, &offdiag_vals[0]);
      ...
      MatSetValues(ptr_mat_wrapper->mat_, 1, &cur_row, nnz_diag_row,
                   &struct_imag.nz_col_ind_in_diag_row[j][0],
                   &petsc_diag_vals[0], ADD_VALUES);
      MatSetValues(ptr_mat_wrapper->mat_, 1, &cur_row, nnz_offdiag_row,
                   &struct_imag.nz_col_ind_in_offdiag_row[j][0],
                   &petsc_offdiag_vals[0], ADD_VALUES);
    }
    MatAssemblyBegin(ptr_mat_wrapper->mat_, MAT_FINAL_ASSEMBLY);
    MatAssemblyEnd(ptr_mat_wrapper->mat_, MAT_FINAL_ASSEMBLY);
  }
...
template < class LAD >
void SLEPcEigenSolver< LAD >::SetupPetscMatShell(
    const OperatorType *op, const OperatorType *opT, const OperatorType *opi,
    const OperatorType *opiT, petsc::Mat_wrapper *ptr_mat_wrapper, int matrix) {
  ...
  if (matrix == 0) {
    ...
    MatCreateShell(this->comm_, op->num_rows_local(), op->num_cols_local(),
                   op->num_rows_global(), op->num_cols_global(), &opA_ptr_,
                   &ptr_mat_wrapper->mat_);
  } else {
    opB_ptr_.op_ = op;
    opB_ptr_.opT_ = opT;
    opB_ptr_.opi_ = opi;
    ...
    MatCreateShell(this->comm_, op->num_rows_local(), op->num_cols_local(),
                   op->num_rows_global(), op->num_cols_global(), &opB_ptr_,
                   &ptr_mat_wrapper->mat_);
  }
  MatShellSetOperation(ptr_mat_wrapper->mat_, MATOP_MULT,
                       (void (*)())MatVecMult< LAD >);
  MatShellSetOperation(ptr_mat_wrapper->mat_, MATOP_MULT_TRANSPOSE,
                       (void (*)())MatTransVecMult< LAD >);
}

template < class LAD >
void SLEPcEigenSolver< LAD >::GetEigenValue(int j, DataType &real,
                                            DataType &imag) {
...
template < class LAD >
void SLEPcEigenSolver< LAD >::GetEigenVector(int j, VectorType &v_real,
                                             VectorType &v_imag) {
  std::vector< int > ind;
...
/*
template <class LAD>
void SLEPcEigenSolver<LAD>::GetSingularVector(int j, VectorType& v_right,
VectorType& v_left)
{
        VecAssemblyBegin(this->ptr_vr_wrapper_[j]->vec_);
        VecAssemblyBegin(this->ptr_vi_wrapper_[j]->vec_);
...
template < class LAD > void SLEPcEigenSolver< LAD >::Clear() {
  PetscErrorCode ierr;
  ...
  if (struct_A_.mat_type == petsc::MPI_MATFREE ||
      struct_B_.mat_type == petsc::MPI_MATFREE)
    aux_vec_.Zeros();
...
template PetscErrorCode MatTransVecMult< LADescriptorPETSc >(Mat A, Vec x,
                                                             Vec y);
#endif
#ifdef WITH_HYPRE
...
template PetscErrorCode MatTransVecMult< LADescriptorHypreD >(Mat A, Vec x,
                                                              Vec y);
#endif
...
template PetscErrorCode MatTransVecMult< LADescriptorCoupledD >(Mat A, Vec x,
                                                                Vec y);
