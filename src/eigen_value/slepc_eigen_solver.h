...
template < class LAD > struct Operator_wrapper {
...
struct MatrixStructure {
  ...
  /// Column indices of nonzeros in diagonal part, obtained from
  /// SparsityStructure
  ...
  /// Column indices of nonzeros in offdiagonal part, obtained from
  /// SparsityStructure
  std::vector< int > off_diag_cols;

  /// Row indices of nonzeros in offdiagonal part, obtained from
  /// SparsityStructure
...
/// @brief Base class for all eigenvalue / singular value solvers that make use
/// of SLEPc

template < class LAD > class SLEPcEigenSolver : public EigenSolver< LAD > {
public:
  ...
  /// @param[in] opA real part of left system matrix for eigenvalue problem A*x
  /// = k*B*x / matrix for SVD
  /// @param[in] opAi imaginary part of left system matrix for eigenvalue
  /// problem A*x = k*B*x / matrix for SVD. Set to NULL, if no imaginary part is
  /// given
  ...
  void SetupOperatorA(const OperatorType *opA, const OperatorType *opAi,
                      ...
                      const std::vector< int > &d_cols,
                      const std::vector< int > &o_rows,
                      const std::vector< int > &o_cols);

  /// Setup operator for Eigenvalue / SingaluarValue problem with matrix free
  /// implementation. Note: not every eigensolver needs the transposed operator.
  /// When using such a solver, use opAt = opA.
  /// @param[in] opA real part of left system matrix for eigenvalue problem A*x
  /// = k*B*x / matrix for SVD
  /// @param[in] opAt real part of transposed left system matrix for eigenvalue
  /// problem A*x = k*B*x / matrix for SVD
  /// @param[in] opAi (optional) imag part of left system matrix for eigenvalue
  /// problem A*x = k*B*x / matrix for SVD
  /// @param[in] opAit (optional) imag part of transposed left system matrix for
  /// eigenvalue problem A*x = k*B*x / matrix for SVD
  void SetupOperatorA(const OperatorType *opA, const OperatorType *opAt,
                      const OperatorType *opAi, const OperatorType *opAit,
                      const LaCouplings &cp);

  /// Setup operator for Eigenvalue / SingaluarValue problem
  /// @param[in] opB real part of right system matrix for eigenvalue problem A*x
  /// = k*B*x / matrix for SVD
  /// @param[in] opBi (optional) imaginary part of right system matrix for
  /// eigenvalue problem A*x = k*B*x / matrix for SVD. Set to NULL, if no
  ...
  /// @param[in] opB real part of right system matrix for eigenvalue problem A*x
  /// = k*B*x / matrix for SVD
  /// @param[in] opBt real part of transposed right system matrix for eigenvalue
  /// problem A*x = k*B*x / matrix for SVD
  /// @param[in] opBi (optional) imag part of right system matrix for eigenvalue
  /// problem A*x = k*B*x / matrix for SVD
  /// @param[in] opBit (optional) imag part of transposed right system matrix
  /// for eigenvalue problem A*x = k*B*x / matrix for SVD
  void SetupOperatorB(const OperatorType *opB, const OperatorType *opBt,
                      const OperatorType *opBi, const OperatorType *opBit,
                      const LaCouplings &cp);

  /// Call this functions after setting all operators defining the eigenvalue
  /// problem
  ...
  /// @param[in] j Desired eigenvalue, sorting order corresponding to specified
  /// eigenvalue type
  ...
  /// @param[in] j Desired eigenvector, sorting order corresponding to specified
  /// eigenvalue type
  ...
  /// @param[in] j Desired singular value, sorting order corresponding to
  /// specified singular value type
  /// @param[out] sig j-th singular value
  void GetSingularValue(int j, DataType &sig);

  /// Return computed left and right singular vectors
  /// @param[in] j Desired singular vector, sorting order corresponding to
  /// specified eigenvalue type
...
protected:
  ...
  void SetupMatrixStructure(const LaCouplings &cp,
                            const std::vector< int > &d_rows,
                            const std::vector< int > &d_cols,
                            const std::vector< int > &o_rows,
                            const std::vector< int > &o_cols,
  ...
  /// Setup PETSC matrix object for operator in Eigenvalue / SingaluarValue
  /// problem
  /// @param[in] op (Hiflow) system matrix to be converted into PETSc matrix
  /// object
  /// @param[in] opi (optionla) (Hiflow) system matrix to be converted into
  /// PETSc matrix object, imag part
  /// @param[out] ptr_mat_wrapper PETSc matrix object
  void SetupPetscMat(const OperatorType *op, const OperatorType *opi,
                     slepc::MatrixStructure struct_real,
                     slepc::MatrixStructure struct_imag,
  ...
  /// @param[in] op (Hiflow) system matrix to be converted into PETSc matrix
  /// object
  /// @param[in] opT transposed (Hiflow) system matrix to be converted into
  /// PETSc matrix object
  /// @param[in] opi (optional) (Hiflow) system matrix to be converted into
  /// PETSc matrix object, imag part
  /// @param[in] opiT (optional) transposed (Hiflow) system matrix to be
  /// converted into PETSc matrix object, imag part
  /// @param[out] ptr_mat_wrapper PETSc matrix object
  void SetupPetscMatShell(const OperatorType *op, const OperatorType *opT,
                          const OperatorType *opi, const OperatorType *opiT,
                          petsc::Mat_wrapper *ptr_mat_wrapper, int matrix);

  /// Init auxiliary vector for performing matrix vector product in matrx-free
  /// implementation
  void InitVec(const LaCouplings &cp);
  ...
  /// Wrapper object for performing matrix free matrix-vector multiplication
  /// with A
