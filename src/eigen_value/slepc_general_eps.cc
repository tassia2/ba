...
SLEPcGeneralEPS< LAD >::SLEPcGeneralEPS(slepc::EpsType eps_type)
    : SLEPcEigenSolver< LAD >(new slepc::EPS_wrapper),
      eps_type_(slepc::KRYLOVSCHUR), problem_type_(slepc::NHEP),
      eval_type_(slepc::L_REAL), conv_type_(slepc::ABS),
      error_type_(slepc::ABS), ptr_st_wrapper_(new slepc::ST_wrapper),
      ptr_st_ksp_wrapper_(new petsc::KSP_wrapper),
      ptr_st_pc_wrapper_(new petsc::PC_wrapper) {
  this->eps_type_ = eps_type;
}

template < class LAD > SLEPcGeneralEPS< LAD >::~SLEPcGeneralEPS() {
  SLEPcGeneralEPS< LAD >::Clear();
}

template < class LAD > void SLEPcGeneralEPS< LAD >::Init(const MPI_Comm &comm) {
...
template < class LAD >
void SLEPcGeneralEPS< LAD >::InitControl(int nev, int ncv, int mpd, int maxits,
                                         double tol, slepc::EpsConv conv_type,
                                         slepc::EpsConv error_type,
                                         bool true_res_norm) {

  EPSSetTolerances(this->ptr_eps_wrapper_->eps_, tol, maxits);
  EPSSetConvergenceTest(this->ptr_eps_wrapper_->eps_,
                        slepc::get_conv_type(conv_type));

  if (this->eval_type_ != slepc::ALL) {
    if (ncv == 0) {
      if (mpd == 0)
        EPSSetDimensions(this->ptr_eps_wrapper_->eps_, nev, PETSC_DEFAULT,
                         PETSC_DEFAULT);
      else
...
template < class LAD > void SLEPcGeneralEPS< LAD >::PassOperatorsToSolver() {
  assert(this->mat_A_set_);

  if (this->mat_A_set_ && this->mat_B_set_)
    EPSSetOperators(this->ptr_eps_wrapper_->eps_, this->ptr_A_wrapper_->mat_,
                    this->ptr_B_wrapper_->mat_);
...
void SLEPcGeneralEPS< LAD >::SetProblemType(slepc::EpsProblemType type) {
  EPSSetProblemType(this->ptr_eps_wrapper_->eps_,
                    slepc::get_problem_type(type));
...
void SLEPcGeneralEPS< LAD >::ComputeExtremalEigenValue(slepc::EpsWhich type) {
  EPSSetWhichEigenpairs(this->ptr_eps_wrapper_->eps_,
                        slepc::get_eigenvalue_type(type));
...
template < class LAD >
void SLEPcGeneralEPS< LAD >::ComputeTargetEigenValue(DataType target,
                                                     slepc::EpsMeasure type) {
  EPSSetWhichEigenpairs(this->ptr_eps_wrapper_->eps_,
                        slepc::get_target_type(type));
...
void SLEPcGeneralEPS< LAD >::ComputeIntervalEigenValue(DataType a, DataType b) {
  ...
  if (this->ncv_ == 0) {
    if (this->mpd_ == 0)
      EPSKrylovSchurSetDimensions(this->ptr_eps_wrapper_->eps_, this->nev_,
                                  PETSC_DEFAULT, PETSC_DEFAULT);
    else
      EPSKrylovSchurSetDimensions(this->ptr_eps_wrapper_->eps_, this->nev_,
                                  PETSC_DEFAULT, this->mpd_);
  } else {
    if (this->mpd_ == 0)
      EPSKrylovSchurSetDimensions(this->ptr_eps_wrapper_->eps_, this->nev_,
                                  this->ncv_, PETSC_DEFAULT);
    else
      EPSKrylovSchurSetDimensions(this->ptr_eps_wrapper_->eps_, this->nev_,
                                  this->ncv_, this->mpd_);
  }
...
template < class LAD > int SLEPcGeneralEPS< LAD >::CheckSolver() {
  ...
  if (this->eps_type_ == slepc::SUBSPACE) {
    if (this->eval_type_ != slepc::L_MAG) {
      LOG_ERROR(
          "Eigensolver: SubSpace method only supports EigenvalueType L_MAG");
      return -1;
    }
  }
  if (this->eps_type_ == slepc::LANCZOS) {
    if (this->problem_type_ != slepc::HEP &&
        this->problem_type_ != slepc::GHEP) {
      LOG_ERROR("Eigensolver: Lanczos method only supports problem type HEP "
                "and GHEP");
      return -1;
    }
    /*
    if (this->mat_type == petsc::MPI_MATFREE)
    {
            if ( !this->mat_At_set_ )
            {
                    LOG_ERROR("Eigensolver: Lanczos method requires transposed
    operator A"); return -1;
            }
            if ( this->problem_type != HEP && this->problem_type != NHEP )
            {
                    if ( !this->mat_Bt_set_ )
                    {
                            LOG_ERROR("Eigensolver: Lanczos method requires
    transposed operator B"); return -1;
                    }
            }
    }
     */
  ...
  if (this->eps_type_ == slepc::RQCG) {
    ...
    if (this->problem_type_ != slepc::HEP &&
        this->problem_type_ != slepc::GHEP) {
      LOG_ERROR(
          "Eigensolver: RQCG method only supports problem type HEP and GHEP");
      return -1;
    }
  }
  return 0;
...
template < class LAD > EigenSolverState SLEPcGeneralEPS< LAD >::Solve() {
  ...
  for (int j = 0; j < this->n_conv_; j++) {
    EPSComputeError(this->ptr_eps_wrapper_->eps_, j,
                    slepc::get_error_type(this->error_type_), &this->res_[j]);
  }
  ...
  for (int j = 0; j < this->n_conv_; j++) {
    ...
    MatCreateVecs(this->ptr_A_wrapper_->mat_, NULL,
                  &this->ptr_v_wrapper_[j]->vec_);
    EPSGetEigenpair(this->ptr_eps_wrapper_->eps_, j, &this->eig_val_[j].val_,
                    NULL, this->ptr_v_wrapper_[j]->vec_, NULL);
    ...
    MatCreateVecs(this->ptr_A_wrapper_->mat_, NULL,
                  &this->ptr_vr_wrapper_[j]->vec_);
    MatCreateVecs(this->ptr_A_wrapper_->mat_, NULL,
                  &this->ptr_vi_wrapper_[j]->vec_);

    EPSGetEigenpair(this->ptr_eps_wrapper_->eps_, j, &this->eig_val_[j].real_,
                    &this->eig_val_[j].imag_, this->ptr_vr_wrapper_[j]->vec_,
                    this->ptr_vi_wrapper_[j]->vec_);
#endif
    if (this->print_level_ > 0) {
...
template < class LAD > void SLEPcGeneralEPS< LAD >::SetSTShift(double shift) {
...
template < class LAD > petsc::KSP_wrapper *SLEPcGeneralEPS< LAD >::GetSTKSP() {
  return this->ptr_st_ksp_wrapper_;
}

template < class LAD > petsc::PC_wrapper *SLEPcGeneralEPS< LAD >::GetSTPC() {
  return this->ptr_st_pc_wrapper_;
}

template < class LAD >
void SLEPcGeneralEPS< LAD >::SetSTKSP(slepc::KSPType ksp_type, int max_iter,
                                      int max_size, double abs_tol,
                                      double rel_tol) {
  double div_tol = 1e6;
  KSPSetTolerances(this->ptr_st_ksp_wrapper_->ksp_, rel_tol, abs_tol, div_tol,
                   max_iter);
...
template < class LAD >
void SLEPcGeneralEPS< LAD >::SetSTPC(slepc::PcType pc_type,
                                     slepc::SolverPackage package) {
  switch (pc_type) {
  case (slepc::NONE):
  ...
  case (slepc::LU):
    ...
    switch (package) {
    case (slepc::MUMPS):
      PCFactorSetMatSolverType(this->ptr_st_pc_wrapper_->pc_,
                               MATSOLVERMUMPS);
      break;
    case (slepc::UMFPACK):
      PCFactorSetMatSolverType(this->ptr_st_pc_wrapper_->pc_,
...
template < class LAD > void SLEPcGeneralEPS< LAD >::SetupSTSolver() {
  PCSetUp(this->ptr_st_pc_wrapper_->pc_);
  KSPSetUp(this->ptr_st_ksp_wrapper_->ksp_);
}

template < class LAD > void SLEPcGeneralEPS< LAD >::Clear() {
  if (this->initialized_) {
    EPSDestroy(&this->ptr_eps_wrapper_->eps_);
    this->initialized_ = false;
  }
  SLEPcEigenSolver< LAD >::Clear();
}
