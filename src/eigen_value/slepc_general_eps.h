...
enum KSPType { GMRES, FGMRES, CG, PREONLY };

enum PcType { LU, ILU, NONE, JACOBI, BJACOBI };

enum SolverPackage { MUMPS, UMFPACK, NOPACKAGE };
...
template < class LAD > class SLEPcGeneralEPS : public SLEPcEigenSolver< LAD > {
public:
  ...
  /// SLEPc documentation: " The parameters ncv and mpd are intimately related,
  /// so that the user is advised to set one of them at most. Normal usage is
  /// that (a) in cases where nev is small, the user sets ncv (a reasonable
  /// default is 2*nev); and (b) in cases where nev is large, the user sets mpd.
  ...
  /// otherwise a smaller value should be used." Possible choices for conv_type
  /// and error_type: ABS : absolute value |r| REL : relative to eigenvalue |r|
  /// / |nu| NORM: relative to matrix norm |r| / (|A| + |nu| |B|)
  /// @param[in] nev          number of eigenvalues to be computed
  /// @param[in] ncv        largest dimension of working subspace, if set to 0:
  /// use SLEPc default
  /// @param[in] mpd        maximum projected dimension, if set to 0: use SLEPc
  /// default
  ...
  /// @param[in] true_res_norm use true (explicitly) calculated residual norm,
  /// instead of update formula, default: true
  void InitControl(int nev, int ncd, int mpd, int maxits, double tol,
                   slepc::EpsConv conv_type, slepc::EpsConv error_type,
                   bool true_res_norm);
  ...
  void SetSTKSP(slepc::KSPType, int max_iter, int max_size, double abs_tol,
                double rel_tol);

  /// Set Preconditioner type in KSP object in ST object
  void SetSTPC(slepc::PcType, slepc::SolverPackage);

  /// Setup Solver object of ST, needs to be called after parameters of solver
  /// are set
  void SetupSTSolver();
