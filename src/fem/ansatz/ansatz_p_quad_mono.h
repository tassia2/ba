...
///
/// \class AugPQuadMono ansatz_p_quad_mono.h
/// \brief \bar{P}_k with {P}_k = [\prod_{i=1}^d x_i^{n_i}, \sum_i n_i <= k] 
/// \author Jonas Roller
///

template < class DataType, int DIM >
class AugPQuadMono final : public virtual AnsatzSpace< DataType, DIM > 
{
public:
  using Coord = typename StaticLA<DIM, DIM, DataType>::ColVectorType;

  /// Default Constructor
  AugPQuadMono(CRefCellSPtr<DataType, DIM> ref_cell);

  /// Default Destructor
  virtual ~AugPQuadMono()
  {
  }
...
template < class DataType, int DIM > 
AugPQuadMono< DataType, DIM >::AugPQuadMono(CRefCellSPtr<DataType, DIM> ref_cell)
  : AnsatzSpace<DataType, DIM>(ref_cell)
{
  this->tdim_ = 2;
  this->name_ = "Aug_P_Quad_Mono";
...
template < class DataType, int DIM > 
void AugPQuadMono< DataType, DIM >::init ( size_t degree )
{
  assert (DIM == 2);
  this->tdim_ = 2;
  this->nb_comp_ = 2;
  ...
  this->xk_.clear();
  this->yk_.clear();
  this->xk_.resize(this->deg_ + 1, 1.);
  this->yk_.resize(this->deg_ + 1, 1.);
}
