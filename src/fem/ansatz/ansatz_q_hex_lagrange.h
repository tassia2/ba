...
class QHexLag final: public virtual AnsatzSpace< DataType, DIM > 
{
public:
  using Coord = typename StaticLA<DIM, DIM, DataType>::ColVectorType;

  /// Default Constructor
  QHexLag (CRefCellSPtr<DataType, DIM> ref_cell);
...
template < class DataType, int DIM > 
QHexLag< DataType, DIM >::QHexLag(CRefCellSPtr<DataType, DIM> ref_cell)
