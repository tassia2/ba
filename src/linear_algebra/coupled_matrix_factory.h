...
template < class DataType > class CoupledMatrixFactory {
  typedef std::map< std::string, CoupledMatrixCreator< DataType > * >
      products_t;
  products_t products;
...
public:
  ...
  CoupledMatrixCreator< DataType > *Get(const std::string &id) const {
    ...
    if (it != products.end())
      ...
      LOG_ERROR(
          "CoupledMatrixFactory::Get: No format of this name registered.");
      return NULL;
    }
  }
};
