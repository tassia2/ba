...
void LaCouplings::InitializeCouplings(const std::vector< int > &global_offsets,
  ...
  assert(static_cast< int >(ghost_offsets.size()) == nb_procs + 1);
  assert(static_cast< int >(ghost_dofs.size()) == ghost_offsets.back());
  ...
  // border variable = variable which I own but (several) other processes need
  // it
  //                 = ghost variable of these other processes

  std::vector< std::set< int > > ghost_per_id(nb_procs);

  // collect global ghost indices (i.e. the column indices in the offdiagonal
  // part); ghost_dofs may contain duplicates, these are skipped by
  // std::map::insert
  for (int id = 0; id < nb_procs; ++id) {
    for (int j = ghost_offsets[id], e_j = ghost_offsets[id + 1]; j < e_j; j++) {
      ghost_per_id[id].insert(ghost_dofs[j]);
    }
  }
  ...
  for (int id = 0; id < nb_procs; ++id) {
    if (id != this->my_rank_) {
      nb_ghost_values[id] =
          this->ghost_offsets_[id + 1] - this->ghost_offsets_[id];
      info = MPI_Isend(&(nb_ghost_values[id]), 1, MPI_INT, id, tag, this->comm_,
                       &(mpi_req_send[id]));
      assert(info == MPI_SUCCESS);

      info = MPI_Irecv(&(nb_border_values[id]), 1, MPI_INT, id, tag,
  ...
  for (int id = 1; id <= nb_procs; ++id) {
    this->border_offsets_[id] =
        this->border_offsets_[id - 1] + nb_border_values[id - 1];
  }
  ...
  for (int id = 0; id < nb_procs; ++id) {
    if (id != this->my_rank_) {
      info = MPI_Isend(&(ghost_global_ind[this->ghost_offsets_[id]]),
                       nb_ghost_values[id], MPI_INT, id, tag, this->comm_,
                       &(mpi_req_send[id]));
      assert(info == MPI_SUCCESS);

      info = MPI_Irecv(&(this->border_indices_[this->border_offsets_[id]]),
                       nb_border_values[id], MPI_INT, id, tag, this->comm_,
                       &(mpi_req_recv[id]));
      assert(info == MPI_SUCCESS);
    }
  }
  ...
  for (size_t i = 0, e_i = this->border_indices_.size(); i != e_i; ++i) {
    ...
    assert(this->border_indices_[i] <
           this->global_offsets_[this->my_rank_ + 1]);
    this->border_indices_[i] -= this->global_offsets_[this->my_rank_];
  }

  // create global2offdiag mapping
  this->CompressOffdiagonal(ghost_dofs);
...
void LaCouplings::CompressOffdiagonal(const std::vector< int > &ghost_dofs) {
  // sort by global column id that order matches with order of border indices
  std::vector< int > ghost_dofs_sorted(ghost_dofs.size());
  std::copy(ghost_dofs.begin(), ghost_dofs.end(), ghost_dofs_sorted.begin());
  std::sort(ghost_dofs_sorted.begin(), ghost_dofs_sorted.end());

  int ind = 0;
  for (size_t k = 0, e_k = ghost_dofs_sorted.size(); k != e_k; ++k) {
    // insert column index to map if not done so far
    if (this->global2offdiag_.find(ghost_dofs_sorted[k]) ==
        this->global2offdiag_.end()) {
      this->global2offdiag_.insert(
          std::pair< int, int >(ghost_dofs_sorted[k], ind));
      this->offdiag2global_.insert(
          std::pair< int, int >(ind, ghost_dofs_sorted[k]));
      ind++;
    }
  }
