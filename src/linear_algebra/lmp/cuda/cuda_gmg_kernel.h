...
__global__ void kernel_gmg_compute_residual(
    ...
    const int nrows, const DataType *A_offdiag, const int *col_A_offdiag,
    const int *row_A_offdiag, const DataType *sol, const DataType *rhs,
    DataType *res, const DataType *sol_ghost) {
...
__global__ void kernel_gmg_compute_restriction(
    ...
    const int nrows, const DataType *R_offdiag, const int *col_R_offdiag,
    const int *row_R_offdiag, DataType *res, const DataType *res_ghost,
    const DataType *tmp) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  int j;
  DataType result;
...
__global__ void kernel_gmg_compute_restriction_no_offdiag(
    const DataType *R_diag, const int *col_R_diag, const int *row_R_diag,
    const int nrows, DataType *res, const DataType *tmp) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  int j;
  DataType result;
