...
void bind2tex(const double *x) { cudaBindTexture(NULL, tex_double, x); }

void unbind2tex(const double *x) { cudaUnbindTexture(tex_double); }

void bind2tex(const float *x) { cudaBindTexture(NULL, tex_float, x); }

void unbind2tex(const float *x) { cudaUnbindTexture(tex_float); }
...
__global__ void kernel_spmv_csr_scalar_tex(ValueType *val, int *col, int *row,
...
template < typename ValueType >
__global__ void kernel_add_spmv_csr_scalar_tex(ValueType *val, int *col,
                                               int *row, ValueType *x,
                                               ValueType *y, int n) {
  int index = blockIdx.x * blockDim.x + threadIdx.x;
  int j;
...
template < typename ValueType >
__global__ void kernel_spmvnd_csr_scalar_tex(const ValueType *val,
