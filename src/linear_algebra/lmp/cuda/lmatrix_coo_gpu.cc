...
template < typename ValueType > void GPU_COO_lMatrix< ValueType >::Clear() {
...
template < typename ValueType > void GPU_COO_lMatrix< ValueType >::Zeros(void) {
  memsetdev(this->matrix.val, 0, this->get_nnz(), sizeof(ValueType));
}

template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::set_thread_blocks(int thread_block,
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::Init(const int init_nnz,
...
template < typename ValueType >
lMatrix< ValueType > &GPU_COO_lMatrix< ValueType >::
  ...
  if (this != &mat2) {
    ...
    if (const GPU_COO_lMatrix< ValueType > *casted_mat =
      ...
      if (const CPU_COO_lMatrix< ValueType > *casted_mat =
        ...
        // unsupported type
        LOG_ERROR("ERROR GPU_COO_lMatrix<ValueType>::CopyFrom() unsupported "
                  "matrix type");
        this->print();
        mat2.print();
        exit(-1);
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CopyStructureFrom(
  ...
  if (this != &mat2) {
    ...
    if (const GPU_COO_lMatrix< ValueType > *casted_mat =
      ...
      if (const CPU_COO_lMatrix< ValueType > *casted_mat =
        ...
        // copy
        if (this->get_nnz() > 0) {
          memcpy2host(this->matrix.col, casted_mat->matrix.col,
                      this->get_nnz());
          memcpy2host(this->matrix.row, casted_mat->matrix.row,
                      this->get_nnz());
        }
        ...
        // unsupported type
        LOG_ERROR("GPU_COO_lMatrix<ValueType>::CopyStructureFrom unsupported "
                  "matrix type");
        this->print();
        mat2.print();
        exit(-1);
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CopyStructureTo(
  ...
  if (this != &mat2) {
    ...
    if (const GPU_COO_lMatrix< ValueType > *casted_mat =
      ...
      if (const CPU_COO_lMatrix< ValueType > *casted_mat =
        ...
        LOG_ERROR("GPU_COO_lMatrix<ValueType>::CopyStructureTo unsupported "
                  "matrix type");
        this->print();
        mat2.print();
        exit(-1);
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::ConvertFrom(
    const lMatrix< ValueType > &mat2) {
  // unsupported type
  LOG_ERROR(
      "ERROR GPU_COO_lMatrix<ValueType>::ConvertFrom unsupported matrix type");
  this->print();
  mat2.print();
  exit(-1);
}
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CastFrom(
  ...
  if (const COO_lMatrix< double > *other_coo =
    ...
    if (const GPU_COO_lMatrix< double > *other_gpu_coo =
      ...
      this->CastFrom(*other_gpu_coo);
                   ...
                   dynamic_cast< const CPU_COO_lMatrix< double > * >(
                       other_coo)) {
      // GPU from CPU
      this->CastFrom(*other_cpu_coo);
    } else {
      LOG_ERROR("GPU_COO_lMatrix<ValueType>::CastFrom<double> called with "
  ...
  if (const COO_lMatrix< float > *other_coo =
    ...
    if (const GPU_COO_lMatrix< float > *other_gpu_coo =
      ...
      this->CastFrom(*other_gpu_coo);
                   ...
                   dynamic_cast< const CPU_COO_lMatrix< float > * >(
                       other_coo)) {
      // GPU from CPU
      this->CastFrom(*other_cpu_coo);
    } else {
      LOG_ERROR("GPU_COO_lMatrix<ValueType>::CastFrom<float> called with "
                "non-GPU and non-CPU matrix argument.");
      this->print();
      other.print();
      exit(-1);
    ...
    LOG_ERROR("GPU_COO_lMatrix<ValueType>::CastFrom<float> called with non-COO "
              "matrix argument.");
    this->print();
    other.print();
    exit(-1);
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CastFrom(
    const GPU_COO_lMatrix< double > &other) {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CastFrom(
    const GPU_COO_lMatrix< float > &other) {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CastTo(
  ...
  if (COO_lMatrix< double > *other_coo =
    ...
    if (GPU_COO_lMatrix< double > *other_gpu_coo =
      ...
      LOG_ERROR("GPU_COO_lMatrix<ValueType>::CastTo<double> called with "
                "non-GPU and non-CPU matrix argument.");
      this->print();
      other.print();
      exit(-1);
    ...
    LOG_ERROR("GPU_COO_lMatrix<ValueType>::CastTo<double> called with non-COO "
              "matrix argument.");
    this->print();
    other.print();
    exit(-1);
...
void GPU_COO_lMatrix< ValueType >::CastTo(
  ...
  if (COO_lMatrix< float > *other_coo =
    ...
    if (GPU_COO_lMatrix< float > *other_gpu_coo =
      ...
      LOG_ERROR("GPU_COO_lMatrix<ValueType>::CastTo<float> called with non-GPU "
    ...
    LOG_ERROR("GPU_COO_lMatrix<ValueType>::CastTo<float> called with non-COO "
              "matrix argument.");
    this->print();
    other.print();
    exit(-1);
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CastTo(
    GPU_COO_lMatrix< double > &other) const {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
...
template < typename ValueType >
void GPU_COO_lMatrix< ValueType >::CastTo(
    GPU_COO_lMatrix< float > &other) const {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
