...
public:
  ...
  hiflow::la::CSR_lMatrixType< ValueType >
      matrix; // it will be allocated on the device
  // in side:
  //  ValueType *val; // value
  //  int *col; // col index
  //  int *row; // row pointer
...
public:
  ...
  /// of the matrix
  GPUscalar_CSR_lMatrix(int init_nnz, int init_num_row, int init_num_col,
...
public:
  ...
  /// The constructor call the Init() function to the do allocation
  /// of the matrix
  GPUscalartex_CSR_lMatrix(int init_nnz, int init_num_row, int init_num_col,
                           std::string init_name);
