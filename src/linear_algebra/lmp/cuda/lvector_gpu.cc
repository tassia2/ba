...
template < typename ValueType > GPU_lVector< ValueType >::GPU_lVector() {
...
template < typename ValueType > void GPU_lVector< ValueType >::Clear() {
...
template < typename ValueType > void GPU_lVector< ValueType >::Zeros() {
...
template < typename ValueType >
void GPU_lVector< ValueType >::set_thread_blocks(int thread_block,
...
template < typename ValueType >
void GPU_lVector< ValueType >::add_values(const int *indices, int length,
...
template < typename ValueType >
void GPU_lVector< ValueType >::SetValues(const int *index, const int size,
                                         const ValueType *values) {
...
template < typename ValueType >
void GPU_lVector< ValueType >::GetValues(const int *index, const int size,
...
void GPU_lVector< ValueType >::GetIndexedValues(ValueType *values) const {
  ...
  if (this->get_indexset_size() > 0) {
    ...
    cudagetvalues(this->indexset_, this->indexset_size_, dev_values,
                  this->buffer, thread_block_size);
...
void GPU_lVector< ValueType >::SetIndexedValues(const ValueType *values) {
  ...
  if (this->get_indexset_size() > 0) {
    ...
    cudasetvalues(this->indexset_, this->indexset_size_, dev_values,
                  this->buffer, thread_block_size);
...
void GPU_lVector< ValueType >::SetBlockValues(const int start_i,
  ...
  if ((size > 0) && (this->get_size() > 0)) {
    ...
    cudasetblockvalues(start_i, 0, end_i - start_i, dev_values, this->buffer,
                       thread_block_size);
...
void GPU_lVector< ValueType >::GetBlockValues(const int start_i,
  ...
  if (size > 0) {
    ...
    cudagetblockvalues(start_i, end_i, dev_values, this->buffer,
                       thread_block_size);
...
template < typename ValueType >
lVector< ValueType > &GPU_lVector< ValueType >::
  ...
  if (this != &vec) {
    ...
    if (const GPU_lVector< ValueType > *casted_vec =
      ...
      if (const CPU_lVector< ValueType > *casted_vec =
        ...
        // unsupported type
        LOG_ERROR(
            "ERROR GPU_lVector<ValueType>::CopyFrom() unsupported vector type");
        this->print();
        vec.print();
        exit(-1);
      }
  ...
  if (this != &vec) {
    ...
    if (const GPU_lVector< ValueType > *casted_vec =
      ...
      if (const CPU_lVector< ValueType > *casted_vec =
        ...
        // unsupported type
        LOG_ERROR(
            "ERROR GPU_lVector<ValueType>::CopyTo() unsupported vector type");
        this->print();
        vec.print();
        exit(-1);
      }
...
template < typename ValueType >
lVector< ValueType > *
GPU_lVector< ValueType >::extract_subvector(const int start_i,
  ...
  if (const GPU_lVector< ValueType > *casted_vec =
    ...
    cudagetblockvalues(start_i, end_i, casted_vec->buffer, this->buffer,
                       thread_block_size);

    // actually this cannot happen
  } else {
    LOG_ERROR("ERROR in GPU_lVector<ValueType>::extract_subvector(); "
...
template < typename ValueType >
void GPU_lVector< ValueType >::partial_replace_subvector(
  ...
  if (const GPU_lVector< ValueType > *casted_vec =
    ...
    LOG_ERROR("ERROR in GPU_lVector<ValueType>::partial_replace_subvector() "
              "unsupported vector type");
    this->print();
    sub_vec.print();
    exit(-1);
...
void GPU_lVector< ValueType >::partial_replace_subvector(
  ...
  if (size > 0)
    cudasetblockvalues(start_i, start_sub_vec, size, sub_vec.buffer,
                       this->buffer, thread_block_size);
}

template < typename ValueType >
void GPU_lVector< ValueType >::partial_add_subvector(
    const int start_i, const int start_sub_vec, const int size,
  ...
  if (const GPU_lVector< ValueType > *casted_vec =
    ...
    LOG_ERROR("ERROR in GPU_lVector<ValueType>::partial_add_subvector() "
              "unsupported vector type");
    this->print();
    sub_vec.print();
    exit(-1);
...
void GPU_lVector< ValueType >::partial_add_subvector(
    const int start_i, const int start_sub_vec, const int size,
    const ValueType weight, const GPU_lVector< ValueType > &sub_vec) {
  assert(start_i >= 0);
  assert(start_sub_vec >= 0);
  assert(size > 0);
  assert(start_sub_vec + size <= sub_vec.get_size());
  ...
  if (this->get_size() > 0)
    cudaaddblockvalues(start_i, start_sub_vec, size, sub_vec.buffer,
                       this->buffer, weight, thread_block_size);
}
...
template < typename ValueType >
void GPU_lVector< ValueType >::ElementWiseMult(
  ...
  if (const GPU_lVector< ValueType > *casted_vec =
    ...
    LOG_ERROR("ERROR in CPU_lVector<ValueType>::ElementWiseMult() unsupported "
              "vector type");
    this->print();
    vec.print();
    exit(-1);
...
void GPU_lVector< ValueType >::ElementWiseMult(
  ...
  if (this->get_size() > 0)
    cudamultvalues(this->get_size(), vec.buffer, this->buffer,
                   thread_block_size);
}

template < typename ValueType >
void GPU_lVector< ValueType >::CastFrom(
    const hiflow::la::lVector< double > &other) {
  ...
  if (const GPU_lVector< double > *other_gpu_vec =
    ...
    LOG_ERROR("GPU_lVector<ValueType>::CastFrom<double> called with "
              "unsupported vector argument.");
    this->print();
    other.print();
  }
}
...
void GPU_lVector< ValueType >::CastFrom(const GPU_lVector< double > &other) {
  ...
  cudacastfromdouble(this->get_size(), this->buffer, other.buffer,
                     this->thread_block_size_);
}
...
template < typename ValueType >
void GPU_lVector< ValueType >::CastFrom(
    const hiflow::la::lVector< float > &other) {
  assert(this->get_size() == other.get_size());
  ...
  if (const GPU_lVector< float > *other_gpu_vec =
    ...
    LOG_ERROR("GPU_lVector<ValueType>::CastFrom<float> called with unsupported "
              "vector argument.");
    this->print();
    other.print();
  }
...
void GPU_lVector< ValueType >::CastFrom(const GPU_lVector< float > &other) {
  ...
  cudacastfromfloat(this->get_size(), this->buffer, other.buffer,
                    this->thread_block_size_);
}
...
template < typename ValueType >
void GPU_lVector< ValueType >::CastTo(
    hiflow::la::lVector< double > &other) const {
  assert(this->get_size() == other.get_size());
  ...
  if (GPU_lVector< double > *other_gpu_vec =
    ...
    LOG_ERROR("GPU_lVector<ValueType>::CastTo<double> called with unsupported "
              "vector argument.");
    this->print();
    other.print();
  }
...
void GPU_lVector< ValueType >::CastTo(GPU_lVector< double > &other) const {
  ...
  cudacasttodouble(this->get_size(), other.buffer, this->buffer,
                   this->thread_block_size_);
}
...
template < typename ValueType >
void GPU_lVector< ValueType >::CastTo(
    hiflow::la::lVector< float > &other) const {
  assert(this->get_size() == other.get_size());
  ...
  if (GPU_lVector< float > *other_gpu_vec =
    ...
    LOG_ERROR("GPU_lVector<ValueType>::CastTo<float> called with unsupported "
              "vector argument.");
    this->print();
    other.print();
  }
...
void GPU_lVector< ValueType >::CastTo(GPU_lVector< float > &other) const {
  ...
  cudacasttofloat(this->get_size(), other.buffer, this->buffer,
                  this->thread_block_size_);
}
