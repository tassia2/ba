...
public:
  ...
  /// computes the row sums and stores result in values
  /// @param values - the result, one entry per row
  /// NOTE: values is not set to zero before summation is done
  ...
  virtual void set_diagelements(const ValueType* vals);
  virtual void get_diagelements(ValueType* vals) const;
