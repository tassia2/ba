...
template < typename ValueType > void CPU_COO_lMatrix< ValueType >::Clear() {
...
template < typename ValueType > void CPU_COO_lMatrix< ValueType >::Zeros(void) {
...
void CPU_COO_lMatrix< ValueType >::ReadFile(const char *filename) {
  ...
  if (!((mm_is_real(matcode) || mm_is_integer(matcode) ||
    ...
    printf("Only sparse real-valued or pattern coordinate matrices are "
           "supported\n");
    exit(1);
  }
...
void CPU_COO_lMatrix< ValueType >::WriteFile(const char *filename) const {
  ...
  mm_write_mtx_crd_size(fid, this->get_num_row(), this->get_num_col(),
                        this->get_nnz()); // row, col, nnz
  ...
  for (i = 0; i < this->get_nnz(); ++i)
    fprintf(fid, "%d %d %24.14e\n", this->matrix.col[i] + 1,
            this->matrix.row[i] + 1, this->matrix.val[i]);
...
template < typename ValueType >
lMatrix< ValueType > &CPU_COO_lMatrix< ValueType >::
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::CastFrom(
  ...
  if (const COO_lMatrix< double > *other_coo =
    ...
    LOG_ERROR("CPU_COO_lMatrix<ValueType>::CastFrom<double> called with "
              "non-COO matrix argument.");
    this->print();
    other.print();
    quit_program();
  }
...
void CPU_COO_lMatrix< ValueType >::CastFrom(
  ...
  if (const COO_lMatrix< double > *other_coo =
    ...
    LOG_ERROR("CPU_COO_lMatrix<ValueType>::CastFrom<float> called with non-COO "
              "matrix argument.");
    this->print();
    other.print();
    quit_program();
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::CastTo(
  ...
  if (COO_lMatrix< double > *other_coo =
    ...
    LOG_ERROR("CPU_COO_lMatrix<ValueType>::CastTo<double> called with non-COO "
              "matrix argument.");
    this->print();
    other.print();
    quit_program();
...
void CPU_COO_lMatrix< ValueType >::CastTo(
  ...
  if (COO_lMatrix< double > *other_coo =
    ...
    LOG_ERROR("CPU_COO_lMatrix<ValueType>::CastTo<float> called with non-COO "
              "matrix argument.");
    this->print();
    other.print();
    quit_program();
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::CopyStructureFrom(
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::CopyStructureTo(
    lMatrix< ValueType > &mat2) const {
  mat2.CopyStructureFrom(*this);
}

template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::ConvertFrom(
    const lMatrix< ValueType > &mat2) {
  ...
  if (const CPU_CSR_lMatrix< ValueType > *casted_mat =
    ...
    if (this->get_nnz() > 0)
      this->TransformFromCSR(casted_mat->matrix.row, casted_mat->matrix.col,
                             casted_mat->matrix.val, this->get_num_row(),
                             this->get_num_col(), this->get_nnz());

  } else {
    LOG_ERROR(
        "CPU_COO_lMatrix<ValueType>::ConvertFrom; unsupported matrix type");
    this->print();
    mat2.print();
    quit_program();
  }
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::Psgauss_seidel(
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_coo_matrix<ValueType>::Psgauss_seidel unsupported in "
              "or out vector");
    this->print();
    invec.print();
    outvec->print();
  ...
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    for (int j = 0; j < this->get_nnz(); ++j) {
      ...
      if ((this->matrix.row[j] == i) &&
          (this->matrix.col[j] == this->matrix.row[j]))
        last_j = j;

      if ((this->matrix.row[j] == i) &&
          (this->matrix.col[j] < this->matrix.row[j]))
        casted_outvec->buffer[i] -=
            this->matrix.val[j] * casted_outvec->buffer[this->matrix.col[j]];
    }
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::Pssor(const ValueType omega,
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR(
        "CPUsimple_coo_matrix<ValueType>::Pssor unsupported in or out vector");
    this->print();
    invec.print();
    outvec->print();
    quit_program();
  ...
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    for (int j = 0; j < this->get_nnz(); ++j) {
      ...
      if ((this->matrix.row[j] == i) &&
          (this->matrix.col[j] < this->matrix.row[j]))
        casted_outvec->buffer[i] -= omega * this->matrix.val[j] *
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::Psor(const ValueType omega,
                                        const lVector< ValueType > &invec,
                                        lVector< ValueType > *outvec) const {
  const CPU_lVector< ValueType > *casted_invec =
      dynamic_cast< const CPU_lVector< ValueType > * >(&invec);
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR(
        "CPUsimple_coo_matrix<ValueType>::Psor unsupported in or out vector");
    this->print();
    invec.print();
    outvec->print();
    quit_program();
  ...
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    casted_outvec->buffer[i] =
        casted_outvec->buffer[i] * omega / (this->matrix.val[diag_j]);
  }
}

template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::Pgauss_seidel(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec) const {
  const CPU_lVector< ValueType > *casted_invec =
      dynamic_cast< const CPU_lVector< ValueType > * >(&invec);
  CPU_lVector< ValueType > *casted_outvec =
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_coo_matrix<ValueType>::Pgauss_seidel unsupported in "
              "or out vector");
    this->print();
    invec.print();
    outvec->print();
...
template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::Pjacobi(const lVector< ValueType > &invec,
                                           lVector< ValueType > *outvec) const {
  const CPU_lVector< ValueType > *casted_invec =
      dynamic_cast< const CPU_lVector< ValueType > * >(&invec);
  CPU_lVector< ValueType > *casted_outvec =
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_coo_matrix<ValueType>::Pjacobi unsupported in or out "
              "vector");
    this->print();
    invec.print();
    outvec->print();
  ...
  for (int i = 0; i < this->get_nnz(); ++i)
    if (this->matrix.row[i] == this->matrix.col[i]) {
      assert(this->matrix.val[i] != 0.0);
      casted_outvec->buffer[this->matrix.row[i]] =
          casted_invec->buffer[this->matrix.row[i]] / this->matrix.val[i];
    }
}

template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::TransformFromCSR(
    const int *Ap, const int *Aj, const ValueType *Ax, const int num_rows,
  ...
  csr_2_coo <ValueType, int> (csr_data, num_rows, num_cols, num_nonzeros, false, this->matrix);
}

template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::VectorMultAdd(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec) const {
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPU_COO_lMatrix<ValueType>::VectorMultAdd unsupported in or out "
              "vector");
    this->print();
    invec.print();
    outvec->print();
...
void CPU_COO_lMatrix< ValueType >::VectorMultAdd(
  ...
  if (this->get_nnz() > 0)
    for (int i = 0; i < this->get_nnz(); ++i)
      outvec->buffer[this->matrix.row[i]] +=
          this->matrix.val[i] * invec.buffer[this->matrix.col[i]];
}

template < typename ValueType >
void CPU_COO_lMatrix< ValueType >::VectorMultNoDiag(
  ...
  if (cpu_in && cpu_out) {
    ...
    LOG_ERROR("CPU_COO_lMatrix<ValueType>::VectorMultNoDiag called with "
...
void CPU_COO_lMatrix< ValueType >::VectorMultNoDiag(
  ...
  for (int i = 0; i < this->get_nnz(); ++i) {
    if (this->matrix.row[i] != this->matrix.col[i]) {
      out->buffer[this->matrix.row[i]] +=
          this->matrix.val[i] * in.buffer[this->matrix.col[i]];
    }
  }
}

template < typename ValueType > void CPU_COO_lMatrix< ValueType >::Sort(void) {
  ...
  LOG_DEBUG(
      3,
      "CPU_COO_lMatrix<ValueType>::Sort() bubble sort - Sorting the cols....");
  ...
  LOG_DEBUG(
      3,
      "CPU_COO_lMatrix<ValueType>::Sort() bubble sort - Sorting the  rows....");
  // bubble sort (by rows)
  for (int i = 0; i < this->get_nnz() - 1; ++i)
    for (int j = 0; j < this->get_nnz() - i - 1; ++j)
  ...
  LOG_DEBUG(
      3, "CPU_COO_lMatrix<ValueType>::Sort() bubble sort - Sorting - done...");
}
