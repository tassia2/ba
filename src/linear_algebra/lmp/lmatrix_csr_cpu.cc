...
template < typename ValueType > void CPU_CSR_lMatrix< ValueType >::Clear() {
  ...
  if (this->diag_ptr_ != nullptr)
    delete[] this->diag_ptr_;
  this->diag_ptr_ = nullptr;
...
{
  int new_nnz = -1;
  int old_nnz = this->get_nnz();
  compress_csr<ValueType, int>(this->matrix, new_nnz, this->get_num_row(), this->get_nnz());

  assert (new_nnz >= 0);
  assert (new_nnz <= this->get_nnz());

  this->nnz_ = new_nnz;

  if (new_nnz != old_nnz)
  {
    get_diag_ptr<ValueType, int> (this->matrix,
                                  this->get_num_row(),
                                  this->get_num_col(),
                                  this->get_nnz(),
                                  true,
...
void CPU_CSR_lMatrix< ValueType >::Scale(const ValueType alpha) {
  ...
  const size_t j_e = this->get_nnz();
#ifdef OMP_VECTORIZE
  #pragma omp simd
#endif
  for (size_t j = 0; j != j_e; ++j)
...
void CPU_CSR_lMatrix< ValueType >::TransformFromCOO(
  ...
  std::vector<int> index_row(num_rows, 0);

  coo_2_csr <ValueType, int> (coo_data, num_rows, num_cols, num_nonzeros, &index_row[0], false, this->matrix);
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::add_values(int num_entries, 
                                              const int *rows, 
                                              const int *cols,
                                              const ValueType *vals) 
{
  ...
  for (size_t i = 0; i != num_entries; ++i)
  {
    ...
    if (!this->found_entry(row, col, vals[i]))
    {
      quit_program(); 
    }
#endif
    ...
    for (size_t j = this->matrix.row[row], 
         e_j = this->matrix.row[row + 1];
         j != e_j; ++j) 
    {
      if (col == this->matrix.col[j]) 
      {
        this->matrix.val[j] += vals[i];
        break;
      }
    }
  }
...
template < typename ValueType >
int CPU_CSR_lMatrix< ValueType >::get_actual_nnz() const {
  int res = 0;
  const int nnz = this->get_nnz();
  for (int i=0; i != nnz; ++i)
  {
    if(this->matrix.val[i] != 0.)
    {
      res++;
    }
  }
...
template < typename ValueType >
int CPU_CSR_lMatrix< ValueType >::get_row_nnz(const int row) const {

  assert(this->get_nnz() > 0);
  assert(this->get_num_row() > 0);
  assert(this->get_num_col() > 0);
  
  return this->matrix.row[row + 1] - this->matrix.row[row];
}

template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::get_row_values(const int row, ValueType *val) const {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::get_add_row_sums(ValueType *sums) const 
{
  assert(this->get_nnz() > 0);
  assert(this->get_num_row() > 0);
  assert(this->get_num_col() > 0);
  ...
  { 
    ValueType res = 0.;
    const int e_j = this->matrix.row[i + 1];
#ifdef OMP_VECTORIZE
    #pragma omp simd reduction(+:res)
#endif
    for (int j = this->matrix.row[i]; j != e_j; ++j) 
    {
      res += this->matrix.val[j];
    }
    sums[i] += res;
  }
}
...
void CPU_CSR_lMatrix< ValueType >::VectorMultAdd(
  ...
  if (this->get_nnz() > 0) {
    ...
    {
      ValueType res = 0.;
      const size_t e_j = this->matrix.row[i + 1];
#ifdef OMP_VECTORIZE
      #pragma omp simd reduction (+:res)
#endif
      for (size_t j = this->matrix.row[i]; j != e_j; ++j) 
      {
        res += this->matrix.val[j] * invec.buffer[this->matrix.col[j]];
      }
      outvec->buffer[i] += res;
...
void CPU_CSR_lMatrix< ValueType >::extract_diagelements(
  ...
  for (int i = start_i; i < end_i; ++i)
  {
    ...
    tmp_buff[i - start_i] = this->matrix.val[this->diag_ptr_[i]];
  }
...
void CPU_CSR_lMatrix< ValueType >::extract_invdiagelements(
  ...
  for (int i = start_i; i < end_i; ++i)
  {
    ...
    assert (std::abs(this->matrix.val[this->diag_ptr_[i]]) > 0.);
    
    tmp_buff[i - start_i] = 1. / this->matrix.val[this->diag_ptr_[i]];
  }
  vec->SetBlockValues(0, end_i - start_i, tmp_buff);
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::extract_diagelements(
    const int start_i, const int end_i, ValueType *vals) const 
{
  assert (this->diag_ptr_ != nullptr);
  ...
  for (int i = start_i; i < end_i; ++i)
  {
    vals[i - start_i] = this->matrix.val[this->diag_ptr_[i]];
  }

}

template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::get_diagelements(ValueType *vals) const 
{
  assert (this->diag_ptr_ != nullptr);
  ...
  for (int i = 0; i < this->num_row_; ++i)
  {
    vals[i] = this->matrix.val[this->diag_ptr_[i]];
  }
}
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::set_diagelements(const ValueType* vals) {
  assert(this->get_nnz() > 0);
  assert(this->get_num_row() > 0);
  assert(this->get_num_col() > 0);
  ...
  for (int i = 0; i < this->get_num_row(); ++i)
  {
    this->matrix.val[this->diag_ptr_[i]] = vals[i];
  } 
...
void CPU_CSR_lMatrix< ValueType >::delete_diagonal() {
  ...
  for (int i = 0; i < this->get_num_row(); ++i)
  {
    this->matrix.val[this->diag_ptr_[i]] = 0.0;
  } 
/*
  for (int i = 0; i < this->get_num_row(); ++i)
    for (int j = this->matrix.row[i]; j < this->matrix.row[i + 1]; ++j)
