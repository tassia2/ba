...
public:
  ...
  //  in side:
  //  ValueType *val; values of the dense m x n matrix elements. Entries are
  //  stored in one array. int *col; col index int *row; row index
  ...
  virtual void ilusp(const int p, const int ncolors, const int *color_sizes,
  ...
  virtual hiflow::la::lMatrix< ValueType > *
  MatrixMult(const CPU_DENSE_lMatrix< ValueType > &inmat) const;
...
public:
  ...
  /// of the matrix
  CPUsimple_DENSE_lMatrix(int init_nnz, int init_num_row, int init_num_col,
