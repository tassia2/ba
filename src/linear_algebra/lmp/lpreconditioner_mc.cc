...
void lPreconditioner_MultiColoring< ValueType >::Init() {
  LOG_ERROR("lPreconditioner_MultiColoring<ValueType>::Init() - wrong "
            "initialization call");
  quit_program();
}

template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::SetupPermVectors(
    hiflow::la::lVector< ValueType > *x,
...
template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::SetupVector(
...
template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::SetRelax(
    const ValueType omega_relax) {
...
template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::SetupPermOperator(
    hiflow::la::lMatrix< ValueType > &op) {
...
void lPreconditioner_MultiColoring< ValueType >::levelscheduling() {
  ...
  LU_cpu_->Levelscheduling(this->ncolors_, &(this->color_sizes_),
                           &(this->permut_index_));

  LOG_DEBUG(1, "lprecond mc number of levels: " << this->ncolors_);

  for (int k = 0; k < this->ncolors_; ++k)
    LOG_DEBUG(2, "lprecond mc level=" << k
...
void lPreconditioner_MultiColoring< ValueType >::multicoloring() {
  ...
  aux_mat_analysis_cpu_->Multicoloring(this->ncolors_, &(this->color_sizes_),
                                       &(this->permut_index_));

  LOG_DEBUG(1, "lprecond mc number of colours: " << this->ncolors_);

  for (int k = 0; k < this->ncolors_; ++k)
    LOG_DEBUG(2, "lprecond mc color=" << k
                                      << " ;number of elements in this color= "
                                      << this->color_sizes_[k]);
...
template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::PermuteBack(
...
template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::reInitOperator(
...
void lPreconditioner_MultiColoring< ValueType >::extract_mat() {
  ...
  this->L_ =
      new lMatrix< ValueType > *[this->ncolors_ * (this->ncolors_ - 1) / 2];
  this->R_ =
      new lMatrix< ValueType > *[this->ncolors_ * (this->ncolors_ - 1) / 2];
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    //    D matrices
    this->D_[i] = this->LU_->extract_submatrix(offset, offset,
    ...
    for (int j = 0; j < this->ncolors_; ++j) {
      ...
      if (j > i) {
        ...
        LOG_DEBUG(7, "lPreconditioner_MultiColoring<ValueType>::extract_mat() "
                     "Extracting R LU ("
                         << this->ncolors_ * (this->ncolors_ - 1) / 2 << ","
                         << R_index << ") with sizes " << offset << ","
                         << offset_col << " x "
                         << offset + this->color_sizes_[i] << ","
                         << offset_col + this->color_sizes_[j]);

        this->R_[R_index] = this->LU_->extract_submatrix(
            offset, offset_col, offset + this->color_sizes_[i],
            offset_col + this->color_sizes_[j]);
      ...
      if (j < i) {
        ...
        LOG_DEBUG(7, "lPreconditioner_MultiColoring<ValueType>::extract_mat() "
                     "Extracting L LU ("
                         << this->ncolors_ * (this->ncolors_ - 1) / 2 << ","
                         << L_index << ") with sizes " << offset << ","
                         << offset_col << " x "
                         << offset + this->color_sizes_[i] << ","
                         << offset_col + this->color_sizes_[j]);

        this->L_[L_index] = this->LU_->extract_submatrix(
            offset, offset_col, offset + this->color_sizes_[i],
            offset_col + this->color_sizes_[j]);
...
void lPreconditioner_MultiColoring< ValueType >::allocate_vec() {
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    if (this->x_const_ != NULL) {
      this->output_chunks_[i] = this->x_const_->extract_subvector(
          offset, offset + this->color_sizes_[i]);
    } else {

      assert(this->x_ != NULL);
      this->output_chunks_[i] =
          this->x_->extract_subvector(offset, offset + this->color_sizes_[i]);
    }
  }
}
...
void lPreconditioner_MultiColoring< ValueType >::convert_Dmat2vec() {
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    // extract them
    this->LU_cpu_->extract_diagelements(offset, offset + this->color_sizes_[i],
                                        this->Dv_[i]);

    this->LU_cpu_->extract_invdiagelements(
        offset, offset + this->color_sizes_[i], this->iDv_[i]);
  }
}
...
void lPreconditioner_MultiColoring< ValueType >::check_for_dropoffs() {
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    if ((this->D_[i]->get_nnz() != this->D_[i]->get_num_row()) ||
        (this->D_[i]->get_nnz() != this->D_[i]->get_num_col())) {
      LOG_INFO("lprecond mc",
               " D Matrix " << i << " has wrong nnz!!!!!!!!!!!!!");
      LOG_INFO("lprecond mc", "Extracting D LU ("
                                  << this->ncolors_ << "/" << i
                                  << ") with sizes " << offset << "," << offset
                                  << " x " << offset + this->color_sizes_[i]
                                  << "," << offset + this->color_sizes_[i]);
    ...
    for (int j = mat_cpu->matrix.row[i]; j < mat_cpu->matrix.row[i + 1]; ++j) {
      ...
      for (int ic = 0; ic < this->ncolors_; ++ic) {
        ...
        if ((i >= offset) && (i < offset + color_sizes_[ic]))
          if ((mat_cpu->matrix.col[j] >= offset) &&
              (mat_cpu->matrix.col[j] < offset + color_sizes_[ic]))
...
template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::ApplylPreconditioner(
...
template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::splitvectors(
    const hiflow::la::lVector< ValueType > &input) {
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    this->output_chunks_[i]->partial_replace_subvector(
        0, offset, this->color_sizes_[i], input);
  }
}

template < typename ValueType >
void lPreconditioner_MultiColoring< ValueType >::concatenatevectors(
    hiflow::la::lVector< ValueType > *output) {
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    // output = ouput_chunks
    output->partial_replace_subvector(offset, 0, this->color_sizes_[i],
                                      *this->output_chunks_[i]);
  }
}
