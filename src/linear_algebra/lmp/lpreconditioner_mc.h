...
/// @author Dimitar Lukarski
///
/// Provides full toolchain for analysing, building and solving LU types of
/// preconditioner

template < typename ValueType >
class lPreconditioner_MultiColoring
...
public:
  ...
  /// Build function: Analyse() + (permute) + factorize + dropoff + LS +
  /// Prepare_LU
  ...
  /// Contains extract_mat + allocate_vec + convert_Dmat2vec + scale_D +
  /// check_for_dropoffs + cleanup
  virtual void Prepare_LU(void);
  /// Extract D[], L[], R[] sub matrices from LU (wrt to the specific platform
  /// of Operator)
  ...
  /// If a global permutation is necessary to be done (instead of local), this
  /// function return the permutation mapping which is allocated from the
  /// multicoloring() function.
