...
lPreconditioner_MultiColoring_ILUp<
    ValueType >::lPreconditioner_MultiColoring_ILUp() {
  this->ilu_p_ = 0;
  this->mat_power_ = 1;
...
template < typename ValueType >
void lPreconditioner_MultiColoring_ILUp< ValueType >::set_ext_power_pattern(
    const lMatrix< ValueType > &mat) {
...
template < typename ValueType >
void lPreconditioner_MultiColoring_ILUp< ValueType >::set_ext_ilup_pattern(
    const lMatrix< ValueType > &mat) {
...
template < typename ValueType >
void lPreconditioner_MultiColoring_ILUp< ValueType >::Init(
    const int ilu_p, const int mat_power, const bool mc, const bool dropoff_mc,
    const bool ls) {
...
void lPreconditioner_MultiColoring_ILUp< ValueType >::build_aux_matrix() {
  ...
  if (this->ext_power_pat_) {
    ...
    if (this->mat_power_ == 1) {
      ...
      CPUsimple_CSR_lMatrix< ValueType > *matrix_cpu =
          new CPUsimple_CSR_lMatrix< ValueType >;
      matrix_cpu->CloneFrom(*this->Operator_);

      this->aux_mat_analysis_cpu_ =
          matrix_cpu->MatrixSupSPower(this->mat_power_);
...
void lPreconditioner_MultiColoring_ILUp< ValueType >::factorize() {
  ...
  if (this->ilu_p_ == 0) {
    ...
    //    this->LU_cpu_->ilusp(this->ilu_p_, this->ncolors_, this->color_sizes_,
    //    this->permut_index_); this->LU_cpu_->WriteFile("LU.mtx");
  }
...
void lPreconditioner_MultiColoring_ILUp< ValueType >::forwardstep() {
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    for (int j = 0; j < i; ++j) {
      this->L_[l_index]->VectorMultAdd(*this->output_chunks_[j],
