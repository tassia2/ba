...
lPreconditioner_MultiColoring_SymmetricGaussSeidel<
    ValueType >::lPreconditioner_MultiColoring_SymmetricGaussSeidel() {
...
template < typename ValueType >
void lPreconditioner_MultiColoring_SymmetricGaussSeidel<
    ValueType >::scale_D() {
  ...
  for (int i = 0; i < this->ncolors_; ++i) {
    ...
    // relaxation
    this->Dv_[i]->Scale(
        1.0 / (this->omega_relax_ *
               (2.0 - this->omega_relax_))); // for 1.0/(omega(2.0-omega))
