...
template < typename ValueType > lVector< ValueType >::lVector() {
  this->size_ = 0;
  this->name_ = "";
  this->platform_name_ = "";
...
lVector< ValueType > *lVector< ValueType >::CloneWithoutContent() const {
  ...
  lVector< ValueType > *new_vector = init_vector< ValueType >(
      this->get_size(), cloned_name, this->get_platform(),
...
void lVector< ValueType >::print(std::ostream &out) const {
  ...
  LOG_INFO("lVector",
           "name='" << this->get_name() << "', Elements=" << this->get_size()
...
template < typename ValueType > void lVector< ValueType >::Sync(void) const {
  // do nothing
}
