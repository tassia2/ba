...
public:
  ...
  inline void add_value(const int i, ValueType val)
  {
    assert(0 <= i);
  ...
  inline void SetValue(const int index, const ValueType value)
  {
    assert(index >= 0);
  ...
  inline ValueType GetValue(const int index) const 
  {
    assert(index >= 0);
    assert(index < this->get_size());
  ...
  virtual bool ContainsNaN() const
  {
    return hiflow::contains_nan<ValueType>( buffer, this->get_size());
  }
