...
template < typename ValueType > void OPENCL_CSR_lMatrix< ValueType >::Clear() {
#ifdef WITH_OPENCL
...
void OPENCL_CSR_lMatrix< ValueType >::Zeros(void) {
  ...
  if (this->get_nnz() > 0)
    openclzeros(this->val, this->get_nnz(), my_manager->get_kernel(2),
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::set_thread_blocks(cl_device_id my_device,
                                                        const int size) {
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::Init(const int init_nnz,
  ...
  // allocate
  opencl_memallocdev< ValueType >(this->val, init_nnz,
                                  my_manager->get_context());

  opencl_memallocdev< int >(this->col, init_nnz, my_manager->get_context());

  opencl_memallocdev< int >(this->row, init_num_row + 1,
                            my_manager->get_context());
...
template < typename ValueType >
lMatrix< ValueType > &OPENCL_CSR_lMatrix< ValueType >::
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::CopyStructureFrom(
  ...
  if (this != &mat2) {
    ...
    if (const OPENCL_CSR_lMatrix< ValueType > *casted_mat =
      ...
      if (this->get_nnz() > 0) {
        opencl_memcpydev< int >(this->col, casted_mat->col, mat2.get_nnz(),
                                my_manager->get_command_queue());
        opencl_memcpydev< int >(this->row, casted_mat->row,
                                mat2.get_num_row() + 1,
                                my_manager->get_command_queue());
      }
      ...
      if (const CPU_CSR_lMatrix< ValueType > *casted_mat2 =
        ...
        // copy
        if (this->get_nnz() > 0) {
          opencl_memcpy2dev< int >(this->col, casted_mat2->matrix.col,
                                   mat2.get_nnz(),
                                   my_manager->get_command_queue());
          opencl_memcpy2dev< int >(this->row, casted_mat2->matrix.row,
                                   mat2.get_num_row() + 1,
                                   my_manager->get_command_queue());
        }
        ...
        // unsupported type
        LOG_ERROR("ERROR OPENCL_CSR_lMatrix<ValueType>::CopyStructureFrom "
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::CopyFrom(
    const lMatrix< ValueType > &mat2) {
#ifdef WITH_OPENCL
  ...
  if (this != &mat2) {
    ...
    if (const OPENCL_CSR_lMatrix< ValueType > *casted_mat =
      ...
      if (this->get_nnz() > 0) {
        opencl_memcpydev< ValueType >(this->val, casted_mat->val,
                                      mat2.get_nnz(),
                                      my_manager->get_command_queue());
      }
    } else {
      ...
      if (const CPU_CSR_lMatrix< ValueType > *casted_mat2 =
        ...
        // copy
        if (this->get_nnz() > 0) {
          opencl_memcpy2dev< ValueType >(this->val, casted_mat2->matrix.val,
                                         mat2.get_nnz(),
                                         my_manager->get_command_queue());
        }
      } else {
        // unsupported type
        LOG_ERROR("ERROR OPENCL_CSR_lMatrix<ValueType>::operator= ; "
                  "unsupported matrix type");
        this->print();
        mat2.print();
        exit(-1);
...
void OPENCL_CSR_lMatrix< ValueType >::CopyTo(lMatrix< ValueType > &mat2) const {
  if (this != &mat2) {
    ...
    if (const OPENCL_CSR_lMatrix< ValueType > *casted_mat =
      ...
      if (this->get_nnz() > 0) {
#ifdef WITH_OPENCL
        opencl_memcpydev< ValueType >(casted_mat->val, this->val,
                                      this->get_nnz(),
                                      my_manager->get_command_queue());
#else
        ERROR;
      ...
      if (const CPU_CSR_lMatrix< ValueType > *casted_mat2 =
        ...
        if (this->get_nnz() > 0) {
#ifdef WITH_OPENCL
          opencl_memcpy2host< ValueType >(casted_mat2->matrix.val, this->val,
                                          this->get_nnz(),
                                          my_manager->get_command_queue());
#else
          ERROR;
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::CopyStructureTo(
    lMatrix< ValueType > &mat2) const {
#ifdef WITH_OPENCL
  assert(manager_initialized);
  if (this != &mat2) {

    mat2.Init(this->get_nnz(), this->get_num_row(), this->get_num_col(),
              this->get_name());
    ...
    if (const OPENCL_CSR_lMatrix< ValueType > *casted_mat =
      ...
      if (this->get_nnz() > 0) {
        opencl_memcpydev< int >(casted_mat->col, this->col, this->get_nnz(),
                                my_manager->get_command_queue());
        opencl_memcpydev< int >(casted_mat->row, this->row,
                                this->get_num_row() + 1,
                                my_manager->get_command_queue());
      }
      ...
      if (const CPU_CSR_lMatrix< ValueType > *casted_mat2 =
        ...
        // copy
        if (this->get_nnz() > 0) {
          opencl_memcpy2host< int >(casted_mat2->matrix.col, this->col,
                                    this->get_nnz(),
                                    my_manager->get_command_queue());
          opencl_memcpy2host< int >(casted_mat2->matrix.row, this->row,
                                    this->get_num_row() + 1,
                                    my_manager->get_command_queue());
        }
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::ConvertFrom(
    const lMatrix< ValueType > &mat2) {
  // unsupported type
  LOG_ERROR(
      "ERROR OPENCL_CSR_lMatrix<ValueType>::ConvertFrom not implemented yet");
  this->print();
  mat2.print();
  exit(-1);
}
...
template < typename ValueType >
lMatrix< ValueType > *OPENCL_CSR_lMatrix< ValueType >::extract_submatrix(
    const int start_row, const int start_col, const int end_row,
  ...
  opencl_sub_matrix->Init(cpu_sub_matrix->get_nnz(),
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::VectorMultAdd(
    const OPENCL_lVector< ValueType > &invec,
  ...
  if (this->get_nnz() > 0)
    openclcsrvectormult(this->val, this->col, this->row, invec.buffer,
                        outvec->buffer, this->get_num_row(),
                        my_manager->get_kernel(9),
                        my_manager->get_command_queue(), this->global_threads,
                        this->local_threads);
...
void OPENCL_CSR_lMatrix< ValueType >::VectorMultAdd(
  ...
  const OPENCL_lVector< ValueType > *casted_invec =
      dynamic_cast< const OPENCL_lVector< ValueType > * >(&invec);
  OPENCL_lVector< ValueType > *casted_outvec =
      dynamic_cast< OPENCL_lVector< ValueType > * >(outvec);

  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("ERROR OPENCL_CSR_lMatrix<ValueType>::VectorMultAdd unsupported "
              "in or out vector");
    this->print();
    invec.print();
    outvec->print();
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::VectorMult(
    const OPENCL_lVector< ValueType > &invec,
    OPENCL_lVector< ValueType > *outvec) const {
#ifdef WITH_OPENCL
...
void OPENCL_CSR_lMatrix< ValueType >::VectorMult(
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("ERROR OPENCL_CSR_lMatrix<ValueType>::VectorMult unsupported in "
              "or out vector");
    this->print();
    invec.print();
    outvec->print();
...
template < typename ValueType >
lMatrix< ValueType > *
OPENCL_CSR_lMatrix< ValueType >::CloneWithoutContent() const {
#ifdef WITH_OPENCL
  ...
  lMatrix< ValueType > *new_matrix =
      new OPENCL_CSR_lMatrix< ValueType >(this->my_manager);
  new_matrix->Init(this->get_nnz(), this->get_num_row(), this->get_num_col(),
                   cloned_name);
  new_matrix->Zeros();
  return new_matrix;
...
void OPENCL_CSR_lMatrix< ValueType >::CastFrom(const lMatrix< double > &other) {
  ...
  LOG_ERROR(
      "OPENCL_CSR_lMatrix<ValueType>::CastFrom<double> not yet implemented.");
  this->print();
  other.print();
  exit(-1);
}
...
void OPENCL_CSR_lMatrix< ValueType >::CastFrom(const lMatrix< float > &other) {
  ...
  LOG_ERROR(
      "OPENCL_CSR_lMatrix<ValueType>::CastFrom<float> not yet implemented.");
  this->print();
  other.print();
  exit(-1);
}
...
void OPENCL_CSR_lMatrix< ValueType >::CastTo(lMatrix< double > &other) const {
  ...
  LOG_ERROR(
      "OPENCL_CSR_lMatrix<ValueType>::CastTo<double> not yet implemented.");
  this->print();
  other.print();
  exit(-1);
}
...
void OPENCL_CSR_lMatrix< ValueType >::CastTo(lMatrix< float > &other) const {
  ...
  LOG_ERROR(
      "OPENCL_CSR_lMatrix<ValueType>::CastTo<float> not yet implemented.");
  this->print();
  other.print();
  exit(-1);
}
...
void OPENCL_CSR_lMatrix< ValueType >::SwapDiagElementsToRowFront(void) {
  LOG_ERROR("OPENCL_CSR_lMatrix<ValueType>::SwapDiagElementsToRowFront() not "
            "yet implemented.");
  this->print();
  exit(-1);
}
...
template < typename ValueType >
void OPENCL_CSR_lMatrix< ValueType >::VectorMultNoDiag(
    const lVector< ValueType > &in, lVector< ValueType > *out) const {
  LOG_ERROR(
      "OPENCL_CSR_lMatrix<ValueType>::VectorMultNoDiag not yet implemented.");
  this->print();
  exit(-1);
}
