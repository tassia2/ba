...
template < typename ValueType > OPENCL_lVector< ValueType >::OPENCL_lVector() {
...
template < typename ValueType > void OPENCL_lVector< ValueType >::Clear() {
#ifdef WITH_OPENCL
...
template < typename ValueType > void OPENCL_lVector< ValueType >::Zeros() {
#ifdef WITH_OPENCL

  int size = this->get_size();

  if (size > 0)
    openclzeros(this->buffer, size, my_manager->get_kernel(2),
                my_manager->get_command_queue(), this->global_threads,
                this->local_threads);
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::set_thread_blocks(
    cl_device_id my_device, const int size) // TODO, so far only for GPU
{
...
void OPENCL_lVector< ValueType >::Init(const int size, const std::string name) {
  ...
  if (size > 0)
    opencl_memallocdev< ValueType >(this->buffer, size,
                                    my_manager->get_context());
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::add_values(const int *indices, int length,
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::SetValues(const int *index, const int size,
                                            const ValueType *values) {
#ifdef WITH_OPENCL
  ...
  opencl_memcpy2dev< int >(index_buffer, index, size,
                           my_manager->get_command_queue());

  // copy to the device the values
  cl_mem value_buffer;
  opencl_memallocdev< ValueType >(value_buffer, size,
                                  my_manager->get_context());
  opencl_memcpy2dev< ValueType >(value_buffer, values, size,
                                 my_manager->get_command_queue());
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::GetValues(const int *index, const int size,
  ...
  opencl_memcpy2host< ValueType >(values, value_buffer, size,
                                  my_manager->get_command_queue());
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::GetBlockValues(const int start_i,
                                                 const int end_i,
                                                 ValueType *values) const {
#ifdef WITH_OPENCL
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::SetBlockValues(const int start_i,
                                                 const int end_i,
                                                 const ValueType *values) {
#ifdef WITH_OPENCL
  ...
  if ((size > 0) && (this->get_size() > 0)) {
    ...
    openclsetblockvalues(this->buffer, value_buffer, start_i, 0,
                         end_i - start_i, my_manager->get_kernel(12),
                         my_manager->get_command_queue(), this->global_threads,
                         this->local_threads);
...
template < typename ValueType >
lVector< ValueType > *
OPENCL_lVector< ValueType >::extract_subvector(const int start_i,
  ...
  if (const OPENCL_lVector< ValueType > *casted_vec =
    ...
    LOG_ERROR("ERROR in OPENCL_lVector<ValueType>::extract_subvector(); "
...
template < typename ValueType >
lVector< ValueType > &OPENCL_lVector< ValueType >::
operator=(const lVector< ValueType > &vec) {
  if (this == &vec)
    return *this;
...
void OPENCL_lVector< ValueType >::CopyFrom(const lVector< ValueType > &vec) {
  ...
  if (this != &vec) {
    ...
    if (const OPENCL_lVector< ValueType > *casted_vec =
      ...
      if (this->get_size() > 0) {
        opencl_memcpydev< ValueType >(this->buffer, casted_vec->buffer,
                                      this->get_size(),
                                      my_manager->get_command_queue());
      }
    } else {
      ...
      if (const CPU_lVector< ValueType > *casted_vec =
        ...
        if (this->get_size() > 0) {
          opencl_memcpy2dev< ValueType >(this->buffer, casted_vec->buffer,
                                         this->get_size(),
                                         my_manager->get_command_queue());
        }
      } else {
        // unsupported type
        LOG_ERROR("ERROR OPENCL_lVector<ValueType>::operator= unsupported "
                  "vector type");
        exit(-1);
        this->print();
        vec.print();
...
void OPENCL_lVector< ValueType >::CastFrom(const lVector< double > &vec) {
  LOG_ERROR(
      "ERROR OPENCL_lVector<ValueType>::CastFrom<double> not implemented yet")
  this->print();
  ;
  exit(-1);
}
...
void OPENCL_lVector< ValueType >::CastFrom(const lVector< float > &vec) {
  LOG_ERROR(
      "ERROR OPENCL_lVector<ValueType>::CastFrom<float> not implemented yet");
  this->print();
  exit(-1);
}
...
void OPENCL_lVector< ValueType >::CastTo(lVector< double > &vec) const {
  LOG_ERROR(
      "ERROR OPENCL_lVector<ValueType>::CastTo<double> not implemented yet")
  this->print();
  ;
  exit(-1);
}
...
void OPENCL_lVector< ValueType >::CastTo(lVector< float > &vec) const {
  LOG_ERROR(
      "ERROR OPENCL_lVector<ValueType>::CastTo<float> not implemented yet");
  this->print();
  exit(-1);
}

template < typename ValueType >
void OPENCL_lVector< ValueType >::ElementWiseMult(
  ...
  if (const OPENCL_lVector< ValueType > *casted_vec =
    ...
    std::cerr << "ERROR in CPU_lVector<ValueType>::ElementWiseMult() "
                 "unsupported vector type\n";
    this->print();
    vec.print();
    exit(-1);
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::ElementWiseMult(
    const OPENCL_lVector< ValueType > &vec) const {
#ifdef WITH_OPENCL
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Axpy(const OPENCL_lVector< ValueType > &vec,
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Axpy(const lVector< ValueType > &vec,
                                       const ValueType scalar) {
#ifdef WITH_OPENCL
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::ScaleAdd(
    const ValueType scalar, const OPENCL_lVector< ValueType > &vec) {
#ifdef WITH_OPENCL
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::ScaleAdd(const ValueType scalar,
...
template < typename ValueType >
ValueType
OPENCL_lVector< ValueType >::Dot(const OPENCL_lVector< ValueType > &vec) const {
  ValueType result = 0;
  ...
  if (this->get_size() > 0)
    result = opencldot< ValueType >(
        this->buffer, vec.buffer, this->get_size(), my_manager->get_context(),
        my_manager->get_kernel(11), my_manager->get_kernel(10),
        my_manager->get_command_queue(), this->global_threads,
        this->local_threads, this->global_blocks, my_manager->kerneltype,
...
template < typename ValueType >
ValueType
OPENCL_lVector< ValueType >::Dot(const lVector< ValueType > &vec) const {
  ValueType result = 0;
...
ValueType OPENCL_lVector< ValueType >::Norm2(void) const {
  ...
  if (this->get_size() > 0)
    result = openclnrm2< ValueType >(
        this->buffer, this->get_size(), my_manager->get_context(),
        my_manager->get_kernel(11), my_manager->get_kernel(10),
        my_manager->get_command_queue(), this->global_threads,
        this->local_threads, this->global_blocks, my_manager->kerneltype,
        my_manager->cpuFinalReduction, my_manager->cpuFinalThreshold,
...
void OPENCL_lVector< ValueType >::CopyTo(lVector< ValueType > &vec) const {
  ...
  if (const OPENCL_lVector< ValueType > *casted_vec =
    ...
    assert(this->get_size() == casted_vec->get_size());
    opencl_memcpydev< ValueType >(casted_vec->buffer, this->buffer,
                                  casted_vec->get_size(),
                                  my_manager->get_command_queue());
    ...
    if (const CPU_lVector< ValueType > *casted_vec =
      ...
      assert(this->get_size() == casted_vec->get_size());
      opencl_memcpy2host< ValueType >(casted_vec->buffer, this->buffer,
                                      casted_vec->get_size(),
                                      my_manager->get_command_queue());
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::partial_replace_subvector(
  ...
  if (const OPENCL_lVector< ValueType > *casted_vec =
    ...
    std::cerr
        << "ERROR in OPENCL_lVector<ValueType>::partial_replace_subvector() "
           "unsupported vector type\n";
    this->print();
    sub_vec.print();
    exit(-1);
...
void OPENCL_lVector< ValueType >::partial_replace_subvector(
  ...
  if (size > 0)
    openclsetblockvalues(this->buffer, sub_vec.buffer, start_i, start_sub_vec,
                         size, my_manager->get_kernel(12),
                         my_manager->get_command_queue(), this->global_threads,
                         this->local_threads);
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::partial_add_subvector(
    const int start_i, const int start_sub_vec, const int size,
    const ValueType weight, const lVector< ValueType > &sub_vec) {
  LOG_ERROR("ERROR OPENCL_lVector<ValueType>::partial_add_subvector not "
            "implemented yet");
  exit(-1);
}
...
void OPENCL_lVector< ValueType >::partial_add_subvector(
    const int start_i, const int start_sub_vec, const int size,
    const ValueType weight, const OPENCL_lVector< ValueType > &sub_vec) {
  LOG_ERROR("ERROR OPENCL_lVector<ValueType>::partial_add_subvector not "
            "implemented yet");
  exit(-1);
}
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::set_indexset(const int *indexset,
                                               const int size) {
#ifdef WITH_OPENCL
...
void OPENCL_lVector< ValueType >::get_indexset(int *indexset) const {
  ...
  if (this->get_indexset_size() > 0) {
    ...
    opencl_memcpy2host< int >(indexset, this->indexset_,
                              this->get_indexset_size(),
                              this->my_manager->get_command_queue());
...
void OPENCL_lVector< ValueType >::GetIndexedValues(ValueType *values) const {
  ...
  if (this->get_indexset_size() > 0) {
    ...
    opencl_memallocdev< ValueType >(dev_values, this->indexset_size_,
                                    my_manager->get_context());

    // get the values from the vector on the values buffer (on the device)
    openclgetvalues(this->buffer, this->indexset_, dev_values,
                    this->indexset_size_, this->my_manager->get_kernel(1),
                    this->my_manager->get_command_queue(), this->global_threads,
                    this->local_threads);
...
void OPENCL_lVector< ValueType >::SetIndexedValues(const ValueType *values) {
  ...
  if (this->get_indexset_size() > 0) {
    ...
    opencl_memcpy2dev< ValueType >(dev_values, values, this->indexset_size_,
                                   this->my_manager->get_command_queue());

    // set the values from the vector on the values buffer
    openclsetvalues(this->buffer, this->indexset_, dev_values,
                    this->indexset_size_, this->my_manager->get_kernel(0),
                    this->my_manager->get_command_queue(), this->global_threads,
                    this->local_threads);
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Rot(lVector< ValueType > *vec,
                                      const ValueType &sc,
                                      const ValueType &ss) {
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Rot(OPENCL_lVector< ValueType > *vec,
                                      const ValueType &sc,
                                      const ValueType &ss) {
  LOG_ERROR("ERROR OPENCL_lVector<ValueType>::Rot not implemented yet");
  exit(-1);
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Rotg(ValueType *sa, ValueType *sb,
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Rotm(lVector< ValueType > *vec,
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Rotm(OPENCL_lVector< ValueType > *vec,
                                       const ValueType &sparam) {
  LOG_ERROR("ERROR OPENCL_lVector<ValueType>::Rotm not implemented yet");
  exit(-1);
}
...
template < typename ValueType >
void OPENCL_lVector< ValueType >::Rotmg(ValueType *sd1, ValueType *sd2,
...
void OPENCL_lVector< ValueType >::CastFrom(
    const hiflow::la::lVector< OtherValueType > &other) {
  if (const OPENCL_lVector< OtherValueType > *other_opencl_vec =
    ...
    this->CastFrom(*other_opencl_vec);
                 ...
                 dynamic_cast< const CPU_lVector< OtherValueType > * >(
                     &other)) {
    this->CastFrom(*other_cpu_vec);
  } else {
    LOG_ERROR("OPENCL_lVector<ValueType>::CastFrom<OtherValueType> unsupported "
              "other vector");
    this->print();
    other.print();
  }
...
template < typename OtherValueType >
void OPENCL_lVector< ValueType >::CastFrom(
    const OPENCL_lVector< OtherValueType > &other) {
  ValueType *vals = new ValueType[this->get_size()];
  OtherValueType *other_vals = new OtherValueType[other.get_size()];

  opencl_memcpy2host< OtherValueType >(other_vals, other.buffer,
                                       other.get_size(),
                                       other.my_manager->get_command_queue());
...
template < typename OtherValueType >
void OPENCL_lVector< ValueType >::CastFrom(
    const CPU_lVector< OtherValueType > &other) {
  ValueType *vals = new ValueType[this->get_size()];
