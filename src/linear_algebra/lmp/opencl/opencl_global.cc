...
opencl_manager::opencl_manager() { this->initialized = false; }
...
void opencl_manager::init(bool is_double) {
  ...
  if (num_opencl_platforms == 0) {
    LOG_ERROR("ERROR - no available opencl platform(s)"); // is this actually
                                                          // required here?
    exit(-1);
  }
  ...
  for (cl_uint i = 0; i < num_opencl_platforms; i++) {
    status = clGetPlatformInfo(cl_platforms[i], CL_PLATFORM_NAME, 1024,
                               &info_text, NULL);
  ...
  status = clGetDeviceIDs(cl_platforms[num_opencl_platforms],
                          CL_DEVICE_TYPE_ALL, 0, NULL, &num_opencl_devices);
  checkOpenCLErr(status, "Error: clGetDeviceIDs");
  if (num_opencl_devices == 0) {
    LOG_ERROR(
        "ERROR - no available opencl device(s) found on this platform"); // is
                                                                         // this
                                                                         // actually
                                                                         // required
                                                                         // here?
  ...
  status =
      clGetDeviceIDs(cl_platforms[num_opencl_platforms], CL_DEVICE_TYPE_ALL,
                     num_opencl_devices, cl_devices, NULL);
  checkOpenCLErr(status, "Error: clGetDeviceIDs");
  printf("List of available OpenCL Devices for Platform %d\n",
         num_opencl_platforms);
  for (cl_uint i = 0; i < num_opencl_devices; i++) {
    clGetDeviceInfo(cl_devices[i], CL_DEVICE_NAME, sizeof(info_text),
                    &info_text, NULL);
    printf("\t(%d)\tDevice %d:\t%s\n", i, i, info_text);
    cl_device_type type;
  ...
  status = clGetDeviceInfo(cl_devices[num_opencl_devices],
  ...
  opencl_context = clCreateContext(0, 1, &cl_devices[num_opencl_devices], NULL,
                                   NULL, &status);
  checkOpenCLErr(status, "Error: Creating Context. (clCreateContext)");

  // First, get the size of device list data
  status = clGetContextInfo(opencl_context, CL_CONTEXT_DEVICES, 0, NULL,
                            &deviceListSize);
  checkOpenCLErr(
      status,
  ...
  status = clGetContextInfo(opencl_context, CL_CONTEXT_DEVICES, deviceListSize,
                            devices, NULL);
  ...
  clGetDeviceInfo(cl_devices[num_opencl_devices], CL_DEVICE_TYPE, sizeof(type),
                  &type, NULL);
  clGetDeviceInfo(cl_devices[num_opencl_devices], CL_DEVICE_VENDOR,
  ...
  if (type & CL_DEVICE_TYPE_CPU) {
    if (is_double) {
      source = load_program_source(
          "/home/dlukarski/hiflow3/hiflow/src/linear_algebra/lmp/opencl/"
          "kernels/kernels_cpu_double.cl");
      LOG_INFO("opencl_global", "Loading CPU Kernel for double precision...");
    } else {
      source = load_program_source(
          "/home/dlukarski/hiflow3/hiflow/src/linear_algebra/lmp/opencl/"
          "kernels/kernels_cpu.cl");
  ...
  if (type & CL_DEVICE_TYPE_GPU) {
    if (vendor[0] == 'N') {
      if (is_double) {
        source = load_program_source(
            "/home/dlukarski/hiflow3/hiflow/src/linear_algebra/lmp/opencl/"
            "kernels/kernels_nvidia_double.cl");
        LOG_INFO("opencl_global",
                 "Loading NVIDIA GPU Kernel for double precision...");
      } else {
        source = load_program_source(
            "/home/dlukarski/hiflow3/hiflow/src/linear_algebra/lmp/opencl/"
            "kernels/kernels_nvidia.cl");
        LOG_INFO("opencl_global",
                 "Loading NVIDIA GPU Kernel for single precision...");
      }
      kerneltype = 0;
    }
    if (vendor[0] == 'A') {
      if (is_double) {
        source = load_program_source(
            "/home/dlukarski/hiflow3/hiflow/src/linear_algebra/lmp/opencl/"
            "kernels/kernels_ati_double.cl");
        LOG_INFO("opencl_global",
                 "Loading ATI GPU Kernel for double precision...");
      } else {
        source = load_program_source(
            "/home/dlukarski/hiflow3/hiflow/src/linear_algebra/lmp/opencl/"
            "kernels/kernels_ati.cl");
        LOG_INFO("opencl_global",
                 "Loading ATI GPU Kernel for single precision...");
      }
      kerneltype = 1;
    }
  }
  ...
  my_program = clCreateProgramWithSource(opencl_context, 1,
                                         (const char **)&source, NULL, &status);
  checkOpenCLErr(
      status,
      "Error: Loading Binary into cl_program (clCreateProgramWithBinary)");

  status = clBuildProgram(my_program, 1, devices,
                          "-w -Werror -cl-mad-enable -cl-strict-aliasing", NULL,
                          NULL);
  if (status != CL_SUCCESS) {
    size_t length;
    char build_log[2048];
    printf("Error: Failed to build program executable!\n");
    clGetProgramBuildInfo(my_program, *devices, CL_PROGRAM_BUILD_LOG,
  ...
  checkOpenCLErr(status,
                 "Error: Creating Kernel from program (opencl_setvalues)");
  my_kernels[1] = clCreateKernel(my_program, "opencl_getvalues", &status);
  checkOpenCLErr(status,
                 "Error: Creating Kernel from program (opencl_getvalues)");
  ...
  checkOpenCLErr(status,
                 "Error: Creating Kernel from program (opencl_csr_vectormult)");
  my_kernels[9] =
      clCreateKernel(my_program, "opencl_csr_vectormultadd", &status);
  checkOpenCLErr(
      status, "Error: Creating Kernel from program (opencl_csr_vectormultadd)");
  ...
  checkOpenCLErr(status,
                 "Error: Creating Kernel from program (opencl_setblockvalues)");
  my_kernels[13] = clCreateKernel(my_program, "opencl_getblockvalues", &status);
  checkOpenCLErr(status,
                 "Error: Creating Kernel from program (opencl_getblockvalues)");
  my_kernels[14] = clCreateKernel(my_program, "opencl_multvalues", &status);
  checkOpenCLErr(status,
                 "Error: Creating Kernel from program (opencl_multvalues)");
...
cl_context opencl_manager::get_context() { return opencl_context; }

cl_command_queue opencl_manager::get_command_queue() { return commandQueue; }

cl_kernel opencl_manager::get_kernel(int id) { return my_kernels[id]; }

cl_device_id opencl_manager::get_device() { return devices[0]; }
#endif
