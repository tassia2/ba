...
void print_platform_opencl_info(void) {
  ...
  if (num_opencl_platforms == 0) {
    LOG_ERROR(
        "ERROR print_platform_opencl_info() - no available opencl platform(s)");
    exit(-1);
  }
  ...
  for (cl_uint i = 0; i < num_opencl_platforms; i++) {
    ...
    status = clGetDeviceIDs(cl_platforms[i], CL_DEVICE_TYPE_ALL, 0, NULL,
                            &num_opencl_devices);
    checkOpenCLErr(
        status,
        "Error: clGetDeviceIDs"); // TODO evtl ohne Error check wegen 0 devices
    if (num_opencl_devices == 0) {
      ...
      status =
          clGetDeviceIDs(cl_platforms[i], CL_DEVICE_TYPE_ALL,
                         num_opencl_devices, cl_devices, &num_opencl_devices);

      for (int device = 0; device < num_opencl_devices; device++) {
        printf("Device %d of platform %d:\n", device, i);
        // CL_DEVICE_NAME
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_NAME, sizeof(info_text),
                        &info_text, NULL);
        printf("  CL_DEVICE_NAME:   %s\n", info_text);

        // CL_DEVICE_VENDOR
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_VENDOR, sizeof(info_text),
                        &info_text, NULL);
        printf("  CL_DEVICE_VENDOR:   %s\n", info_text);

        // CL_DRIVER_VERSION
        clGetDeviceInfo(cl_devices[device], CL_DRIVER_VERSION,
                        sizeof(info_text), &info_text, NULL);
        printf("  CL_DRIVER_VERSION:   %s\n", info_text);

        // CL_DEVICE_INFO
        cl_device_type type;
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_TYPE, sizeof(type), &type,
                        NULL);
        ...
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_MAX_COMPUTE_UNITS,
                        sizeof(compute_units), &compute_units, NULL);
        printf("  CL_DEVICE_MAX_COMPUTE_UNITS:\t\t%u\n", compute_units);

        // CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS
        size_t workitem_dims;
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
        ...
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_MAX_WORK_ITEM_SIZES,
                        sizeof(workitem_size), &workitem_size, NULL);
        printf("  CL_DEVICE_MAX_WORK_ITEM_SIZES:\t%u / %u / %u \n",
               (unsigned int)workitem_size[0], (unsigned int)workitem_size[1],
               (unsigned int)workitem_size[2]);

        // CL_DEVICE_MAX_WORK_GROUP_SIZE
        unsigned int workgroup_size;
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_MAX_WORK_GROUP_SIZE,
                        sizeof(workgroup_size), &workgroup_size, NULL);
        printf("  CL_DEVICE_MAX_WORK_GROUP_SIZE:\t%u\n", workgroup_size);

        // CL_DEVICE_MAX_CLOCK_FREQUENCY
        cl_uint clock_frequency;
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_MAX_CLOCK_FREQUENCY,
                        sizeof(clock_frequency), &clock_frequency, NULL);
        printf("  CL_DEVICE_MAX_CLOCK_FREQUENCY:\t%u MHz\n", clock_frequency);

        // CL_DEVICE_ADDRESS_BITS
        cl_uint addr_bits;
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_ADDRESS_BITS,
                        sizeof(addr_bits), &addr_bits, NULL);
        printf("  CL_DEVICE_ADDRESS_BITS:\t\t%u\n", addr_bits);

        // CL_DEVICE_MAX_MEM_ALLOC_SIZE
        cl_ulong max_mem_alloc_size;
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_MAX_MEM_ALLOC_SIZE,
        ...
        cl_ulong mem_size;
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_GLOBAL_MEM_SIZE,
        ...
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_ERROR_CORRECTION_SUPPORT,
        ...
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_LOCAL_MEM_TYPE,
                        sizeof(local_mem_type), &local_mem_type, NULL);
        printf("  CL_DEVICE_LOCAL_MEM_TYPE:\t\t%s\n",
               local_mem_type == 1 ? "local" : "global");

        // CL_DEVICE_LOCAL_MEM_SIZE
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_LOCAL_MEM_SIZE,
                        sizeof(mem_size), &mem_size, NULL);
        printf("  CL_DEVICE_LOCAL_MEM_SIZE:\t\t%u KByte\n",
               (unsigned int)(mem_size / 1024));

        // CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_MAX_CONSTANT_BUFFER_SIZE,
                        sizeof(mem_size), &mem_size, NULL);
        ...
        clGetDeviceInfo(cl_devices[device], CL_DEVICE_QUEUE_PROPERTIES,
...
void init_platform_opencl(struct SYSTEM &my_system, int opencl_plat,
                          int opencl_dev) {
#ifdef WITH_OPENCL
