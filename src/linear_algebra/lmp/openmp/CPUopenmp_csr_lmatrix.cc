...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::CloneFrom(
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::VectorMult(
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUopenmp_CSR_lMatrix<ValueType>::VectorMult unsupported in or "
              "out vector");
    this->print();
    invec.print();
    outvec->print();
...
#pragma omp parallel for
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    for (int j = this->matrix.row[i]; j != this->matrix.row[i + 1]; ++j)
      outvec->buffer[i] +=
          this->matrix.val[j] * invec.buffer[this->matrix.col[j]];
  }
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::VectorMultAdd(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec) const {
#ifdef WITH_OPENMP
  ...
  if ((casted_invec == NULL) || (casted_outvec == NULL)) {
    LOG_ERROR("CPUopenmp_CSR_lMatrix<ValueType>::VectorMultAdd unsupported in "
              "or out vector");
    this->print();
    invec.print();
    outvec->print();
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::VectorMultNoDiag(
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::BlocksPsgauss_seidel(
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::CastFrom(
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::get_add_values(
...
#pragma omp parallel for
  for (int i = 0; i < num_rows; ++i) {
    ...
    for (int j = this->matrix.row[rows[i]];
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::VectorMultAdd_submatrix(
    const int *rows, int num_rows, const int *cols, int num_cols,
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::VectorMultAdd_submatrix_vanka(
...
#pragma omp parallel for
  for (int i = 0; i < num_rows; ++i) {
    for (int j = this->matrix.row[rows[i]]; j < this->matrix.row[rows[i] + 1];
         ++j) {
...
void CPUopenmp_CSR_lMatrix< ValueType >::VectorMultAdd_submatrix_vanka(
  ...
  if (casted_invec == NULL) {
    LOG_ERROR("CPUopenmp_CSR_lMatrix<ValueType>::VectorMultAdd_submatrix_vanka "
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::add_values(const int *rows,
                                                    int num_rows,
                                                    const int *cols,
                                                    int num_cols,
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::set_values(const int *rows,
                                                    int num_rows,
                                                    const int *cols,
                                                    int num_cols,
                                                    const ValueType *values) {
...
template < typename ValueType >
void CPUopenmp_CSR_lMatrix< ValueType >::CastTo(
