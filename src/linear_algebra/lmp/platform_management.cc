...
void select_platform(struct SYSTEM &my_system, enum PLATFORM &v_platform,
                     enum IMPLEMENTATION &v_implementation,
                     enum PLATFORM &m_platform,
                     enum IMPLEMENTATION &m_implementation,
                     enum MATRIX_FORMAT &m_format,
                     enum MATRIX_FREE_PRECOND &m_precond) {
  ...
  std::cout
      << "lVector Implementaton: (CPU)NAIVE=0, (CPU/CBLAS,GPU/CUBLAS)BLAS=1, "
         "(CPU)OpenMP=2, (CPU)MKL=3, (CPU/GPU)OPENCL=6 >";
  std::cin >> input;
  v_implementation = IMPLEMENTATION(input);

  std::cout << "lMatrix Implementaton:"
            << " (CPU)NAIVE=0, (none)BLAS=1 (CPU)OpenMP=2"
            << " (CPU)MKL=3, (GPU)SCALAR=4, (GPU)SCALAR+TEX_CACHE=5, "
               "(CPU/GPU)OPENCL=6 >";
  std::cin >> input;
  ...
  //            << " No Precond=0, Jacobi=1, Gauss Seidel=2, Symmetric Gauss
  //            Seidel=3, SOR=4, SSOR=5 >";
  //  std::cin >> input ;
...
void init_platform(struct SYSTEM &my_system) {
  ...
  if (my_system.Platform == CPU) {
    ...
    if (my_system.rank < 0) {
      ...
      init_platform_gpu(my_system.GPU_CUBLAS,
                        my_system.rank % num_gpu_per_node);
    }
  }
#endif
#ifdef WITH_OPENCL
    ...
    if (my_system.rank < 0) {
      ...
      init_platform_opencl(my_system, opencl_platform,
                           my_system.rank % opencl_devices);
    }
