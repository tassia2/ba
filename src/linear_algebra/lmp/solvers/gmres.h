...
template < typename ValueType, bool Timer = false > struct GMRESFunctor {
  // Default Constructor
...
int GMRESFunctor< ValueType, Timer >::
operator()(lVector< ValueType > *x, const lVector< ValueType > *b,
           const lMatrix< ValueType > *matrix, const ValueType rel_eps,
           const ValueType abs_eps, int &max_iter, const int print,
  ...
  if (print > -1) {
    LOG_INFO("GMRESFunctor",
             " with " << (lPrecond ? "" : "no") << " preconditioner");
...
template < typename ValueType, bool Timer >
void GMRESFunctor< ValueType, Timer >::apply_plane_rotation(
    ValueType cs, ValueType sn, ValueType *dx, ValueType *dy) const {
...
void GMRESFunctor< ValueType, Timer >::update_solution(
    ...
    int k, lVector< ValueType > *x) const {
