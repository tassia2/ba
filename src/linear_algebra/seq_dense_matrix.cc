...
#include "mesh/types.h" // for NOT_YET_IMPLEMENTED
...
template<class DataType>
SeqDenseMatrix< DataType >::SeqDenseMatrix (const SeqDenseMatrix<DataType>& rhs)
{
  this->Clear();
  

  this->platform_name_ = rhs.platform_name_;
  this->platform_id_ = rhs.platform_id_;

  this->implementation_name_ = rhs.implementation_name_;
  this->implementation_id_ = rhs.implementation_id_;

  this->block_ = rhs.block_;

  const int nrow = rhs.nrows();
  const int ncol = rhs.ncols();
...
template<class DataType>
SeqDenseMatrix<DataType>& SeqDenseMatrix< DataType >::operator= (const SeqDenseMatrix<DataType>& rhs)
{
  this->Clear();
  this->platform_name_ = rhs.platform_name_;
  this->platform_id_ = rhs.platform_id_;
...
void SeqDenseMatrix< DataType >::Resize(int m, int n) {
  ...
  // NOTE (Philipp G): savest but possibly not most efficient choice for block size
  // lower values for block_: less rows are searched for pivot element
  // this->block_ = m;
}
...
void SeqDenseMatrix< DataType >::Zeros() {
  ...
  std::fill(this->val_.begin(), this->val_.end(), static_cast< DataType >(0.0));
  this->is_zero_ = true;
}
...
template < class DataType >
DataType SeqDenseMatrix< DataType >::abs() const {
...
bool SeqDenseMatrix< DataType >::Factorize_simple() 
{
  ...
  for (int i = 0; i < nrows_; i += block_) {
    //std::cout << "i = " << i << " in (0," << nrows_ << ")" << std::endl;
    ...
    //std::cout << " j = " << j << ", blockend = " << blockend << std::endl;
    
    // gaussian elimination on the diagonal block
    for (int k = i; k < blockend; ++k) {
      //std::cout << "k = " << k << " in (" << i << "," << blockend << ")" << std::endl;
      ...
      //std::cout << "max value " << max_value << " at index " << max_index << " found in column k=" << k << ", searched from row " << k+1 << " to " << blockend-1 << std::endl;
