...
template < class MatrixType > struct InitFunctor {
...
template < class T > struct InitFunctor< hiflow::la::CoupledMatrix< T > > {

  void operator()(hiflow::la::CoupledMatrix< T > &matrix, int nnz, int num_row,
                  int num_col, std::string const &namee,
                  hiflow::la::CoupledMatrix< T > const &ref_matrix) const {
    matrix.Init(ref_matrix.comm(), ref_matrix.row_couplings(),
