 ...
 */
template < class LevelGenerator > class StandardCoarsening {
public:
  ...
  ConnectionType operator()(ResultType &result, MatrixType const &Af) const {
    ...
    while (!U.empty()) {
      ...
      size_t next_C =
          std::max_element(U.begin(), U.end(), SecondLessThan())->first;
      ...
      for (IndexSet::const_iterator iter_cur(ST[next_C].begin()),
           iter_end(ST[next_C].end());
           iter_cur != iter_end; ++iter_cur) {
        if (U.find(*iter_cur) == U.end())
          continue;
#ifdef PRINT_DEBUG
      ...
      for (IndexSet::iterator iter_cur(update_list.begin());
           iter_cur != update_list.end();) {
        if (U.find(*iter_cur) == U.end())
          update_list.erase(iter_cur++);
        else
          ++iter_cur;
      }
      ...
      // Update lambda
      for (IndexSet::const_iterator iter_cur(update_list.begin()),
           iter_end(update_list.end());
           iter_cur != iter_end; ++iter_cur) {
        U[*iter_cur] =
            intersection_size(ST[*iter_cur], U) +
    ...
    assert(num_variables == connection.fine_variables.size() +
                                connection.coarse_variables.size());
...
private:
  ...
  void create_strong_connections(AdjacencyList &S, MatrixType const &Af) const {
    ...
    for (size_t i = 0; i != num_variables; ++i) {
      ...
      for (size_t p = Af.matrix.row[i]; p != Af.matrix.row[i + 1]; ++p) {
        if (Af.matrix.col[p] != i and Af.matrix.val[p] < 0 and
            std::abs(Af.matrix.val[p]) > abs_max_neg_value)
  ...
  AdjacencyList transpose(AdjacencyList const &S) const {
    ...
    for (AdjacencyList::const_iterator iter_row_cur(S.begin()),
         iter_row_end(S.end());
         iter_row_cur != iter_row_end; ++iter_row_cur) {
      for (AdjacencyList::value_type::const_iterator
               iter_col_cur(iter_row_cur->begin()),
           iter_col_end(iter_row_cur->end());
           iter_col_cur != iter_col_end; ++iter_col_cur) {
  ...
  /// Functor returning true if absolute value of v1 is lesser than absolute
  /// value of v2. Should be a lambda function using C++11.
  ...
  /// Functor returning true if second value of v1 is lesser than the second
  /// value of v2. Should be a lambda function using C++11.
