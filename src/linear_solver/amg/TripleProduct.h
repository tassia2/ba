...
template < class LevelGenerator > class TripleProduct {
public:
  ...
  TripleProduct(Settings const &settings = Settings()) : settings_(settings) {}

  void operator()(ResultType &result, MatrixType const &Af) const {
    TripleProductImpl< MatrixType >()(result.ptr_coarse_matrix,
                                      *result.ptr_restriction_matrix, Af,
