...
/// Functor returning true if value of v1 is equal to first value of v2 or vice
/// versa. Should be a lambda function using C++11.
...
size_t intersection_size(T1 const &s1, std::map< T2, T3 > const &s2) {
  Counter c;
  std::set_intersection(s1.begin(), s1.end(), s2.begin(), s2.end(),
                        std::back_inserter(c), MapFirstLessThan());
  return c.count;
}
