...
public:
  ...
  int n_gpus(void) const { return n_gpus_; }

  DataType *dev_b_ptr(const int dev) { return b_gpu[dev]; }

  DataType *host_b_ptr(const int dev) { return b_host[dev]; }

  DataType *dev_x_ptr(const int dev) { return x_gpu[dev]; }

  DataType *host_x_ptr(const int dev) { return x_host[dev]; }

  DataType *dev_xg_ptr(const int dev) { return xg_gpu[dev]; }

  DataType *host_xg_ptr(const int dev) { return xg_host[dev]; }
...
private:
  ...
  AsynchronousIterationGPU< LAD > &
  operator=(const AsynchronousIterationGPU< LAD > &other);
};
