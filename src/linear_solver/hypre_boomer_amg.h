...
template < class LAD > class HypreBoomerAMG : public HypreLinearSolver< LAD > {
public:
  ...
  /// Sets the size of the system of PDEs, if using the systems version. The
  /// default is 1, i.e. a scalar system.
  ...
  /// Sets the mapping that assigns the function to each variable, if using the
  /// systems version. If no assignment is made and the number of functions is k
  /// > 1, the mapping generated is (0,1,...,k-1,0,1,...,k-1,...).
  ...
  /// Sets AMG strength threshold. The default is 0.25. For 2d Laplace
  /// operators, 0.25 is a good value, for 3d Laplace operators, 0.5 or 0.6 is a
  /// better value. For elasticity problems, a large strength threshold, such as
  /// 0.9, is often better.
  ...
  /// Sets whether to use the nodal systems coarsening. Should be used for
  /// linear systems generated from systems of PDEs. \param[in] nodal type of
  /// coarsening algorithm
  // CAUTION: Setting this function my lead to a segmentation fault in HYPRE routines !!
  ...
  /// Sets whether to give special treatment to diagonal elements in the nodal
  /// systems version. The default is 0. If set to 1, the diagonal entry is set
  /// to the negative sum of all off diagonal entries. If set to 2, the signs of
  /// all diagonal entries are inverted.
  // CAUTION: Setting this function my lead to a segmentation fault in HYPRE routines !!
  ...
  /// Defines the maximal number of elements per row for the interpolation. The
  /// default is 0.
  ...
  /// Defines whether separation of weights is used for standard or multipass
  /// interpolation. The default is 0.

  void SetSepWeight(int sep_weight) {
    this->sep_weight_ = sep_weight;
    this->SetModifiedParam(true);
  }

  /// Defines the smoother to be used. It uses the given smoother on the fine
  /// grid, the up and the down cycle and sets the solver on the coarsest level
  /// to Gaussian elimination.

  void SetRelaxType(int relax_type) {
    this->cycle_relax_type_[1] = relax_type;
    this->cycle_relax_type_[2] = relax_type;
    this->SetModifiedParam(true);
  }

  /// Defines the relaxation weight for smoothed Jacobi and hybrid SOR on all
  /// levels.
  ...
  /// Defines the relaxation weight for Schwarz smoothing on all levels.

  void SetSchwarzRlxWt(DataType schwarz_relax_weight) {
    this->schwarz_relax_weight_ = schwarz_relax_weight;
    this->SetModifiedParam(true);
  }
  ...
  /// The smoothers, as defined by SetSmoothType, will be used on level 0 (the
  /// finest level) through level smooth num levels-1. The default is 0, i.e. no
  /// complex smoothers are used.
  ...
  /// Defines name of an input file for Euclid parameters. For further
  /// explanation see description of Euclid.
  ...
  /// Defines number of levels for ILU(k) in Euclid. For further explanation see
  /// description of Euclid.
  ...
  /// Defines filter for ILU(k) for Euclid. For further explanation see
  /// description of Euclid.
  ...
  /// Defines use of block jacobi ILUT for Euclid. For further explanation see
  /// description of Euclid.
  ...
  /// AMG logging: if > 1, than exact residual is calculated at the end of
  /// BommerAMGSolve
  void SetLogging(int logging) {
    this->amg_logging_ = logging;
    this->SetModifiedParam(true);
  }

  /// If set to 1, the local interpolation transposes will be saved to use more
  /// efficient matvecs instead of matvecTs
...
private:
  ...
  int P_max_elem_;
  /// Sets whether separation of weights is used for standard or multipass
  /// interpolation
  int sep_weight_;
  /// Relaxation weight
  DataType relax_weight_;
  /// Schwarz Relaxation weight
  DataType schwarz_relax_weight_;
