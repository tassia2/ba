...
public:
  ...
  /// uses ILUT and defines a drop tolerance relative to the largest absolute
  /// value of any entry in the row being factored.
...
protected:
  ...
  void BuildImpl(VectorType const *b, VectorType *x) {
    ...
    //              HYPRE_EuclidSetup ( this->solver_, *(
    //              this->op_->GetParCSRMatrix ( ) ), *( b.GetParVector ( ) ),
    //              *( x->GetParVector ( ) ) );
