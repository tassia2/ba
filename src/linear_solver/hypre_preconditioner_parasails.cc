...
#include "linear_solver/hypre_preconditioner_parasails.h"
...
template < class LAD >
LinearSolverState
HyprePreconditionerParaSails< LAD >::SolveImpl(const VectorType &b,
                                               VectorType *x) {
#ifdef WITH_HYPRE
  HYPRE_ParCSRParaSailsSolve(this->solver_, *(this->op_->GetParCSRMatrix()),
 ...
 */
template < class LAD > void HyprePreconditionerParaSails< LAD >::Clear() {
#ifdef WITH_HYPRE
