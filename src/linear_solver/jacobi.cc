...
template < class LAD >
Jacobi< LAD >::Jacobi()
    : LinearSolver< LAD >(), solve_mode_(0), smooth_iter_(1), inner_iter_(1),
      w_(1.0), async_(false), inv_diag_(0), csr_mat_(0), y_(0) {}

template < class LAD > Jacobi< LAD >::~Jacobi() {
  if (inv_diag_ != 0)
    delete inv_diag_;
  if (y_ != 0)
    delete y_;
...
// TODO: check whether this has to be moved to BuildImpl
template < class LAD >
void Jacobi< LAD >::Prepare(OperatorType &op, const VectorType &sample) {
  ...
  op.diagonal().extract_invdiagelements(0, op.num_rows_local(), inv_diag_);

  this->SetupOperator(op);
}

template < class LAD >
void Jacobi< LAD >::BuildImpl(VectorType const *b, VectorType *x) {
  
  assert(this->op_!= nullptr);
  // try to cast to a CSR matrix
  csr_mat_ = dynamic_cast< CSR_lMatrix< DataType > * >(&((this->op_)->diagonal()));
  ...
  // create local vectors from sample
  inv_diag_ = b->interior().CloneWithoutContent();
  y_ = b->interior().CloneWithoutContent();

  (this->op_)->diagonal().extract_invdiagelements(0, (this->op_)->num_rows_local(), inv_diag_);
}

template < class LAD > void Jacobi< LAD >::SetSolveMode(const int mode) {
  assert(mode >= 0);
  assert(mode <= 3);
...
template < class LAD > void Jacobi< LAD >::SetNumIter(const int iter) {
  assert(iter > 0);

  smooth_iter_ = iter;
}

template < class LAD > void Jacobi< LAD >::SetInnerIter(const int iter) {
  assert(iter > 0);
...
template < class LAD >
LinearSolverState Jacobi< LAD >::SolveImpl(const VectorType &b, VectorType *x) {
  assert(x != 0);
...
template < class LAD >
LinearSolverState Jacobi< LAD >::SolveNormal(const VectorType &b,
                                             VectorType *x) {
  assert(x != 0);
  assert(this->op_ != 0);
  assert(inv_diag_ != 0);
  ...
  IterateControl::State iter_ctrl =
...
template < class LAD >
LinearSolverState Jacobi< LAD >::SolveDamped(const VectorType &b,
                                             VectorType *x) {
  assert(x != 0);
  assert(this->op_ != 0);
  assert(inv_diag_ != 0);
...
template < class LAD >
LinearSolverState Jacobi< LAD >::SmoothNormal(const VectorType &b,
                                              VectorType *x) {
...
template < class LAD >
LinearSolverState Jacobi< LAD >::SmoothDamped(const VectorType &b,
                                              VectorType *x) {
  assert(smooth_iter_ > 0);

  for (int n = 0; n < this->maxits_; ++n) {
    if (async_)
      x->UpdateGhost();
    for (int i = 0; i < inner_iter_; ++i) {
      DoDampedIteration(b, x);
