...
MPIR< LAD_high, LAD_low >::MPIR()
    : LinearSolver< LAD_high >(), max_ech_(0), max_ecl_(0), n_ech_(0),
      n_ecl_(0), max_iter_preh_(0), max_iter_posth_(0), max_iter_prel_(0),
      max_iter_postl_(0), max_iter_ech_(0), max_iter_ecl_(0),
      accu_iter_preh_(0), accu_iter_posth_(0), accu_iter_prel_(0),
      accu_iter_postl_(0), accu_iter_ech_(0), accu_iter_ecl_(0),
      abs_tol_mpir_(0.0), abs_tol_preh_(0.0), abs_tol_posth_(0.0),
      abs_tol_prel_(0.0), abs_tol_postl_(0.0), abs_tol_ech_(0.0),
      abs_tol_ecl_(0.0), rel_tol_mpir_(0.0), rel_tol_preh_(0.0),
      rel_tol_posth_(0.0), rel_tol_prel_(0.0), rel_tol_postl_(0.0),
      rel_tol_ech_(0.0), rel_tol_ecl_(0.0), div_tol_mpir_(0.0),
      div_tol_preh_(0.0), div_tol_posth_(0.0), div_tol_prel_(0.0),
      div_tol_postl_(0.0), div_tol_ech_(0.0), div_tol_ecl_(0.0),
      pre_smoother_high_(0), post_smoother_high_(0), pre_smoother_low_(0),
      post_smoother_low_(0), error_correction_solver_high_(0),
      error_correction_solver_low_(0), Ah_(0), Al_(0), resh_(0), corh_(0),
      resl_(0), corl_(0), res0_(0.0), use_pre_smoother_high_(false),
      use_post_smoother_high_(false), use_pre_smoother_low_(false),
...
template < class LAD_high, class LAD_low >
void MPIR< LAD_high, LAD_low >::SetupPreconditioner(
    Preconditioner< LAD_high > &precond) {}

template < class LAD_high, class LAD_low >
LinearSolverState MPIR< LAD_high, LAD_low >::SolveImpl(const VectorType_high &b,
                                                       VectorType_high *x) {
  assert(x != 0);
  assert(Ah_ != 0);
  assert(Al_ != 0);
  ...
  while (ContinueIteration()) {
    ...
    // error correction
    if ((n_ecl_ < max_ecl_) && (this->res_ > abs_tol_ecl_) &&
        (this->res_ > res0_ * rel_tol_ecl_) && (this->res_ < div_tol_ecl_))
    ...
    } else if ((n_ech_ < max_ech_) && (this->res_ > abs_tol_ech_) &&
    ...
    if (this->print_level_ > 1) {
      LOG_INFO("MPIR",
               " residual norm " << this->res_ << " after error correction");
      LOG_INFO("MPIR", " ------------------------------------------------------"
                       "-------------");
    }
  }
  ...
  if (this->print_level_ > 0) {
    ...
    LOG_INFO(
        ...
        " -------------------------------------------------------------------");
    LOG_INFO("MPIR",
             " number of low precision error correction loops: " << n_ecl_);
    LOG_INFO("MPIR", " accumulated low precision pre-smoother iterations: "
                         << accu_iter_prel_);
    LOG_INFO("MPIR",
             " accumulated low precision error correction solver iterations: "
                 << accu_iter_ecl_);
    LOG_INFO("MPIR", " accumulated low precision post-smoother iterations: "
                         << accu_iter_postl_);
    LOG_INFO(
        "MPIR",
        " -------------------------------------------------------------------");
    LOG_INFO("MPIR",
             " number of high precision error correction loops: " << n_ech_);
    LOG_INFO("MPIR", " accumulated high precision pre-smoother iterations: "
                         << accu_iter_preh_);
    LOG_INFO("MPIR",
             " accumulated high precision error correction solver iterations: "
                 << accu_iter_ech_);
    LOG_INFO("MPIR", " accumulated high precision post-smoother iterations: "
                         << accu_iter_posth_);
    LOG_INFO(
        "MPIR",
        " ===================================================================");
...
bool MPIR< LAD_high, LAD_low >::ContinueIteration() const {
  return (this->res_ > abs_tol_mpir_) && (this->res_ > res0_ * rel_tol_mpir_) &&
         (this->res_ < div_tol_mpir_) &&
         (((n_ech_ < max_ech_) && (this->res_ > abs_tol_ech_) &&
           (this->res_ > res0_ * rel_tol_ech_) &&
           (this->res_ < div_tol_ech_)) ||
          ((n_ecl_ < max_ecl_) && (this->res_ > abs_tol_ecl_) &&
           (this->res_ > res0_ * rel_tol_ecl_) && (this->res_ < div_tol_ecl_)));
}
...
template < class LAD_high, class LAD_low >
void MPIR< LAD_high, LAD_low >::SetHighPrecPreSmootherParameters(
    const int max_iter, const DataType_high abs_tol,
    const DataType_high rel_tol, const DataType_high div_tol) {
...
void MPIR< LAD_high, LAD_low >::ResetHighPrecPreSmootherParameters() {
  if (pre_smoother_high_ != 0) {
    pre_smoother_high_->InitControl(max_iter_preh_, abs_tol_preh_,
                                    rel_tol_preh_, div_tol_preh_);
  }
}

template < class LAD_high, class LAD_low >
void MPIR< LAD_high, LAD_low >::SetHighPrecPostSmootherParameters(
    const int max_iter, const DataType_high abs_tol,
    const DataType_high rel_tol, const DataType_high div_tol) {
...
void MPIR< LAD_high, LAD_low >::ResetHighPrecPostSmootherParameters() {
  if (post_smoother_high_ != 0) {
    post_smoother_high_->InitControl(max_iter_posth_, abs_tol_posth_,
                                     rel_tol_posth_, div_tol_posth_);
  }
}

template < class LAD_high, class LAD_low >
void MPIR< LAD_high, LAD_low >::SetLowPrecPreSmootherParameters(
    const int max_iter, const DataType_low abs_tol, const DataType_low rel_tol,
...
void MPIR< LAD_high, LAD_low >::ResetLowPrecPreSmootherParameters() {
  if (pre_smoother_low_ != 0) {
    pre_smoother_low_->InitControl(max_iter_prel_, abs_tol_prel_, rel_tol_prel_,
                                   div_tol_prel_);
  }
}

template < class LAD_high, class LAD_low >
void MPIR< LAD_high, LAD_low >::SetLowPrecPostSmootherParameters(
    const int max_iter, const DataType_low abs_tol, const DataType_low rel_tol,
    const DataType_low div_tol) {
...
void MPIR< LAD_high, LAD_low >::ResetLowPrecPostSmootherParameters() {
  if (post_smoother_low_ != 0) {
    post_smoother_low_->InitControl(max_iter_postl_, abs_tol_postl_,
                                    rel_tol_postl_, div_tol_postl_);
  }
}

template < class LAD_high, class LAD_low >
void MPIR< LAD_high, LAD_low >::SetHighPrecErrCorSolverParameters(
    const int max_iter, const DataType_high abs_tol,
    const DataType_high rel_tol, const DataType_high div_tol) {
...
void MPIR< LAD_high, LAD_low >::ResetHighPrecErrCorSolverParameters() {
  if (error_correction_solver_high_ != 0) {
    error_correction_solver_high_->InitControl(max_iter_ech_, abs_tol_ech_,
                                               rel_tol_ech_, div_tol_ech_);
  }
}

template < class LAD_high, class LAD_low >
void MPIR< LAD_high, LAD_low >::SetLowPrecErrCorSolverParameters(
    const int max_iter, const DataType_low abs_tol, const DataType_low rel_tol,
    const DataType_low div_tol) {
...
void MPIR< LAD_high, LAD_low >::ResetLowPrecErrCorSolverParameters() {
  if (error_correction_solver_low_ != 0) {
    error_correction_solver_low_->InitControl(max_iter_ecl_, abs_tol_ecl_,
                                              rel_tol_ecl_, div_tol_ecl_);
  }
}
