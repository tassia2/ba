...
public:
  ...
  virtual void SetupOperator(MatrixType_high &op) { Ah_ = &op; }

  void SetHighPrecMatrix(MatrixType_high &mh) { Ah_ = &mh; }

  void SetLowPrecMatrix(MatrixType_low &ml) { Al_ = &ml; }

  void SetHighPrecResVector(VectorType_high &vh) { resh_ = &vh; }

  void SetHighPrecCorVector(VectorType_high &vh) { corh_ = &vh; }

  void SetLowPrecResVector(VectorType_low &vl) { resl_ = &vl; }

  void SetLowPrecCorVector(VectorType_low &vl) { corl_ = &vl; }
  ...
  void SetMpirParameters(const int max_ech, const int max_ecl,
  ...
  void UseHighPrecPreSmoother(const bool use) { use_pre_smoother_high_ = use; }

  void UseHighPrecPostSmoother(const bool use) {
    use_post_smoother_high_ = use;
  }

  void UseLowPrecPreSmoother(const bool use) { use_pre_smoother_low_ = use; }

  void UseLowPrecPostSmoother(const bool use) { use_post_smoother_low_ = use; }

protected:
  virtual LinearSolverState SolveImpl(const VectorType_high &b,
                                      VectorType_high *x);
  ...
  int max_iter_preh_;  // max iter per error correction loop for
                       // pre_smoother_high_
  int max_iter_posth_; // max iter per error correction loop for
                       // post_smoother_high_
  int max_iter_prel_;  // max iter per error correction loop for
                       // pre_smoother_low_
  int max_iter_postl_; // max iter per error correction loop for
                       // post_smoother_low_
  int max_iter_ech_;   // max iter per error correction loop for
                       // error_correction_solver_high_
  int max_iter_ecl_;   // max iter per error correction loop for
                       // error_correction_solver_low_
  ...
  int accu_iter_ech_;   // accumulated no. of iterations of
                        // error_correction_solver_high_
  int accu_iter_ecl_;   // accumulated no. of iterations of
                        // error_correction_solver_low_
  ...
  DataType_high
      abs_tol_ech_;          // absolute tolerance error_correction_solver_high_
  ...
  DataType_high
      rel_tol_ech_;          // relative tolerance error_correction_solver_high_
  ...
  DataType_high
      div_tol_ech_; // divergence tolerance error_correction_solver_high_
  DataType_low
      div_tol_ecl_; // divergence tolerance error_correction_solver_low_
  ...
  VectorType_high *resh_,
      *corh_; // high precision residual and correction vector
