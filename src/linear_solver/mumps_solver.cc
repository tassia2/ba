...
#include "linear_solver/mumps_solver.h"

#include "common/log.h"
#include "config.h"
#include "dof/dof_partition.h"
#include "linear_algebra/la_descriptor.h"
#include "linear_solver/mumps_structure.h"
...
MumpsSolver< LAD, MUMPS_STRUCTURE >::MumpsSolver() : LinearSolver< LAD >() {
...
void MumpsSolver< LAD, MUMPS_STRUCTURE >::InitParameter(const MPI_Comm &comm
  ...
  this->mumps_struc_.id_.ICNTL(1) =
      6; // output stream for error messages (default 6)
  this->mumps_struc_.id_.ICNTL(2) =
      0; // output stream for diagnostic (default 0)
  ...
  this->mumps_struc_.id_.ICNTL(11) =
      0; // if positive returns statistics (default 0)
  ...
  // this->mumps_struc_.id_.ICNTL(14) = 20; // percentage of estimated workspace
  // increase (automatic chosen)

  // 15-17 not used

  this->mumps_struc_.id_.ICNTL(18) =
      3; // = 3, distributed assembled matrix input
  this->mumps_struc_.id_.ICNTL(19) =
      0; // return Schur complement (default 0, no return)
  this->mumps_struc_.id_.ICNTL(20) =
      0; // rhs sparse pattern (0 = dense, 1 = sparse)
...
template < class LAD, class MUMPS_STRUCTURE >
void MumpsSolver< LAD, MUMPS_STRUCTURE >::BuildImpl(VectorType const *b,
                                                    VectorType *x) {
  assert(this->op_ != nullptr);
  assert(this->op_->is_square_matrix());
...
void MumpsSolver< LAD, MUMPS_STRUCTURE >::FactorizeSymbolic() {
  ...
  this->mumps_struc_.id_.n = this->op_->num_rows_global();   // global size
...
template < class LAD, class MUMPS_STRUCTURE >
LinearSolverState
MumpsSolver< LAD, MUMPS_STRUCTURE >::SolveFactorized(const VectorType &b,
                                                     VectorType *x) {
  ...
  assert(info == MPI_SUCCESS);
  int tag = 1;
  DataType *sol_recv = new DataType[this->op_->num_rows_local()];
  ...
  if (my_rank == master_rank) {
    ...
    for (int id = 1; id < size_comm; ++id) {
      ...
      info = MPI_Send(&rhs_gathered[send_index_begin],
                      send_index_end - send_index_begin, MPI_DOUBLE, id, tag,
                      this->comm());
      assert(info == MPI_SUCCESS);
    }

    // own values
    for (int i = 0; i < this->op_->num_rows_local(); ++i) {
      sol_recv[i] = rhs_gathered[i];
    }

  } else {
    MPI_Status status;
    info = MPI_Recv(sol_recv, this->op_->num_rows_local(), MPI_DOUBLE, master_rank,
                    tag, this->comm(), &status);
    assert(info == MPI_SUCCESS);
  }

  // now set values in solution vector
  x->SetLocalValues(sol_recv);
...
template < class LAD, class MUMPS_STRUCTURE >
LinearSolverState
MumpsSolver< LAD, MUMPS_STRUCTURE >::SolveImpl(const VectorType &b,
                                               VectorType *x) {
#ifdef WITH_MUMPS
...
void MumpsSolver< LAD, MUMPS_STRUCTURE >::CreateLocalMatrix() {
  ...
  // extract CSR matrix
  int *ia = new int[this->op_->num_rows_local() + 1];
