...
int CellType::add_refinement(const std::vector< int > &sub_cell_numbers) {
  ...
  BSharedPtr< RefinementTree >::Type tree =
      BSharedPtr< RefinementTree >::Type(new RefinementTree(tdim(), *this));
  assert(tree != 0);
...
void CellType::initialize_cell_types() {
  if (!is_initialized()) {
    ...
    for (int t = 0; t < CellType::NUM_CELL_TYPES; ++t) {
      ...
      cell_types_[t] = BSharedPtr< CellType >::Type(ptr);
...
std::vector< BSharedPtr< CellType >::Type > CellType::cell_types_;
