...
EntityCount Connectivity::num_entities() const { return connections_.size(); }
...
void Connectivity::clear() { connections_.resize(0); }
...
void Connectivity::transpose(const EntityCount num_d1_entities,
  ...
  std::vector< std::vector< int > > &d1_d2_conn =
      d1_d2_connectivity.connections_;
  d1_d2_conn.clear();
  d1_d2_conn.resize(num_d1_entities);

  for (size_t d2_id = 0; d2_id != connections_.size(); ++d2_id) {
    for (ConstConnectionIterator d1_it = begin(d2_id); d1_it != end(d2_id);
         ++d1_it) {
...
void Connectivity::transpose_subset(const std::vector< int > &d2_indices,
  ...
  for (IndexIterator d2_it = d2_indices.begin(); d2_it != d2_indices.end();
       ++d2_it) {
...
void ConnectivityAlgorithms::intersect_subset_equal_dimensions(
    ...
    const Connectivity &zero_d_connectivity, Connectivity &d_d_connectivity) {
  LOG_DEBUG(1, "Intersecting subset (equal dimensions)");
  typedef SortedArray< int >::const_iterator IndexIterator;

  for (IndexIterator d_it = d_indices.begin(); d_it != d_indices.end();
       ++d_it) {
    // NB d_zero_connectivity is assumed to be complete, i.e. contain all
    // entities.
    const int d_id = *d_it;
    std::vector< int > connected_d_entities;
    ...
    for (ConstConnectionIterator v_it = d_zero_connectivity.begin(d_id);
         v_it != d_zero_connectivity.end(d_id); ++v_it) {
      const int v_id = *v_it;
      // NB zero_d_connectivity is assumed to be complete - i.e. contain all
      // vertices. The v_id is then directly the index to use in this
      // connectivity.
      for (ConstConnectionIterator cd_it = zero_d_connectivity.begin(v_id);
        ...
        const bool is_already_added =
            find(connected_d_entities.begin(), connected_d_entities.end(),
                 connected_d_id) != connected_d_entities.end();

        if (connected_d_id != d_id && !is_already_added &&
...
void ConnectivityAlgorithms::intersect_subset_unequal_dimensions(
    const SortedArray< int > &d1_indices, const SortedArray< int > &d2_indices,
    const Connectivity &d1_zero_connectivity,
    const Connectivity &d2_zero_connectivity,
    const Connectivity &zero_d2_connectivity,
    Connectivity &d1_d2_connectivity) {
  ...
  for (IndexIterator d1_it = d1_indices.begin(); d1_it != d1_indices.end();
       ++d1_it) {
    const int d1_id = *d1_it;
    ...
    for (ConstConnectionIterator v_it = d1_zero_connectivity.begin(d1_id);
      ...
      for (ConstConnectionIterator d2_it = zero_d2_connectivity.begin(v_id);
        ...
        const bool is_connected = is_subset(
            d2_zero_connectivity.begin(d2_id), d2_zero_connectivity.end(d2_id),
            d1_zero_connectivity.begin(d1_id), d1_zero_connectivity.end(d1_id));

        const bool is_already_added =
            find(connected_d2_entities.begin(), connected_d2_entities.end(),
                 d2_id) != connected_d2_entities.end();

        if (is_connected && !is_already_added && d2_indices.find(d2_id, 0)) {
          connected_d2_entities.push_back(d2_id);
        }
      }
    }
