...
DistributedBuilder::DistributedBuilder(const MPI_Comm &communicator,
                                       MeshBuilder &local_builder, int sender,
                                       const std::vector< int > *partition)
    : MeshBuilder(local_builder.tdim(), local_builder.gdim()),
      communicator_(communicator), local_builder_(local_builder),
      sender_(sender), partition_(partition), num_sent_entities_(0) {
  // setup type for points
  MPI_Type_contiguous(this->gdim(), MPI_DOUBLE, &point_type_);
  MPI_Type_commit(&point_type_);
}

DistributedBuilder::~DistributedBuilder() { MPI_Type_free(&point_type_); }

// NB coordinates is empty except for on sender

VertexHandle
DistributedBuilder::add_vertex(const std::vector< Coordinate > &coordinates) {
...
DistributedBuilder::add_vertices(const std::vector< Coordinate > &coordinates) {
  assert(coordinates.empty() || is_sender());
  if (is_sender()) {
    assert(!coordinates.empty());
  }
...
EntityHandle
DistributedBuilder::add_entity(TDim tdim,
  ...
  if (sender_ != receiver) {
    if (is_sender()) {
      ...
      MPI_Send(const_cast< VertexHandle * >(vec2ptr(vertices)), entity_size,
               MPI_INT, receiver, 3, communicator_);
    } else if (is_receiver) {
      MPI_Recv(&tdim, 1, MPI_INT, sender_, 1, communicator_, MPI_STATUS_IGNORE);
      MPI_Recv(&entity_size, 1, MPI_INT, sender_, 2, communicator_,
               MPI_STATUS_IGNORE);
      received_vertices.resize(entity_size);
      MPI_Recv(vec2ptr(received_vertices), entity_size, MPI_INT, sender_, 3,
...
DistributedBuilder::add_entities(TDim tdim,
                                 const std::vector< VertexHandle > &vertices,
  ...
  std::vector< int > num_entities_on_proc; // number of entities on each process
                                           // (length = num processes)
  std::vector< int > permuted_entity_sizes; // sizes of entities in permuted
                                            // order (length = num of entities)
  std::vector< int > num_vertices_on_proc; // number of vertices on each process
                                           // (length = num processes)
  std::vector< VertexHandle >
      permuted_vertices; // vertices of entities in permuted order (length = num
                         // vertices)

  // extra data needed for sender
  std::vector< int >
      entity_offsets_on_proc; // offset into permuted_entity_sizes
                              // (length = num processes)
  std::vector< int > vertex_offsets_on_proc; // offset into permuted_vertices
                                             // (length = num processes)
  ...
  if (is_sender()) {
    ...
    compute_entity_permutation(num_entities, entity_offsets_on_proc,
                               entity_permutation);
    ...
    compute_num_vertices_on_proc(permuted_entity_sizes, entity_offsets_on_proc,
                                 num_vertices_on_proc);
    ...
    compute_permuted_vertices(sizes, num_entities_on_proc, entity_permutation,
                              vertices, permuted_vertices);
  ...
  MPI_Scatter(vec2ptr(num_entities_on_proc), 1, MPI_INT, &num_received_entities,
              1, MPI_INT, sender_, communicator_);
  ...
  MPI_Scatterv(vec2ptr(permuted_entity_sizes), vec2ptr(num_entities_on_proc),
               ...
               vec2ptr(received_entity_sizes), num_received_entities, MPI_INT,
               sender_, communicator_);

  // Scatter vertices to receiving processes
  const int num_received_vertices = std::accumulate(
      received_entity_sizes.begin(), received_entity_sizes.end(), 0);
  ...
  MPI_Scatterv(vec2ptr(permuted_vertices), vec2ptr(num_vertices_on_proc),
               ...
               vec2ptr(received_vertices), num_received_vertices, MPI_INT,
               sender_, communicator_);
...
void DistributedBuilder::set_material_number(TDim tdim, EntityHandle entity,
...
MeshPtr DistributedBuilder::build() { return local_builder_.build(); }

bool DistributedBuilder::is_sender() const { return rank() == sender_; }
...
void DistributedBuilder::broadcast_vertices(
    const std::vector< Coordinate > &coords, EntityCount num_new_vertices) {
...
void DistributedBuilder::compute_entity_permutation(
    int num_entities, const std::vector< int > &entity_offsets_for_proc,
...
void DistributedBuilder::compute_num_entities_on_proc(
    int num_entities, std::vector< int > &num_entities_on_proc) const {
  assert(partition_ != 0);
...
void DistributedBuilder::compute_num_vertices_on_proc(
    const std::vector< int > &permuted_entity_sizes,
  ...
  for (int p = 0; p < num_processes; ++p) {
    ...
    const int end_entity =
        (p + 1 == num_processes) ? num_entities : entity_offsets_on_proc[p + 1];
...
void DistributedBuilder::convert_sizes_to_offsets(
    const std::vector< int > &sizes, std::vector< int > &offsets) const {
  offsets.clear();
...
void DistributedBuilder::compute_permuted_vertices(
    const std::vector< int > &entity_sizes,
...
VertexHandle DistributedBuilder::add_local_vertex(int vertex_index) {
  ...
  if (local_vertices_[vertex_index] == -1) {
    ...
    VertexHandle v =
        local_builder_.add_vertex(std::vector< Coordinate >(begin, end));
