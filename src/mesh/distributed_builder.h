...
public:
  ...
  DistributedBuilder(const MPI_Comm &communicator, MeshBuilder &local_builder,
...
private:
  ...
  void broadcast_vertices(const std::vector< Coordinate > &coords,
                          EntityCount num_new_vertices);

  void
  compute_entity_permutation(int num_entities,
                             const std::vector< int > &entity_offsets_for_proc,
                             std::vector< int > &entity_permutation) const;

  void
  compute_num_entities_on_proc(int num_entities,
                               std::vector< int > &num_entities_on_proc) const;

  void
  compute_num_vertices_on_proc(const std::vector< int > &permuted_entity_sizes,
  ...
  void compute_permuted_vertices(
