...
bool Entity::is_descendant(Entity entity) const {
  Entity parent;
  for (Entity e = *this; e.has_parent(); e = parent) {
    parent = e.parent();
...
double Entity::diameter() const {
  assert(is_valid());
  compute_coordinates();

  const int nb_vert = this->num_vertices();
  const int gdim = this->gdim();
  double h = 0.;

  for (int i = 1; i != nb_vert; ++i)
  {
    for (int j = 0; j != i; ++j)
    {
      double dist_ij = 0.;
      ...
      {
        const double a = this->coordinates_[i * gdim + d];
        const double b = this->coordinates_[j * gdim + d];
        dist_ij += (a-b) * (a-b);
      }
      h = std::max(h, dist_ij);
    }
...
void Entity::compute_coordinates() const {
  ...
  if (coordinates_.empty()) {
    mesh_->get_coordinates(tdim_, index_, coordinates_);
  }
}
