...
public:
  ...
  bool is_descendant(Entity entity) const;
  ...
  void get_coordinates(std::vector< vec<DIM,T> > &coords) const;
  ...
  template < int DIM, typename T >
  void get_coordinates(vec<DIM,T> &coords, int vertex_number) const;
  ...
  template < int DIM, typename T >
  void get_midpoint(vec<DIM, T> &coords) const;
...
template < int DIM, typename T >
void Entity::get_coordinates(std::vector< vec<DIM, T> > &coords) const {
  assert(is_valid());
  compute_coordinates();
  ...
  coords.clear();
  coords.resize(num_vert);
  
  for (int v=0; v<num_vert; ++v)
  {
    for (int d=0; d<gdim; ++d)
    {
      coords[v].set(d, coordinates_[v*gdim+d]);
    }
  }
}
...
template < int DIM, typename T >
void Entity::get_midpoint(vec<DIM, T > &coords) const 
{
  std::vector<T> mid_coords;
  this->get_midpoint(mid_coords);
  
  for (int d=0; d<DIM; ++d)
  {
    coords.set(d, mid_coords[d]); 
  }
}
...
template < int DIM, typename T >
void Entity::get_coordinates(vec<DIM, T> &coords,
                             int vertex_number) const 
{
  assert(is_valid());
  this->compute_coordinates();

  auto begin_sub = coordinates_.begin() + DIM * vertex_number;

  for (int d=0; d!=DIM; ++d)
  {
    coords[d] = *(begin_sub + d);
  }
}
