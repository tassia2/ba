...
class MeshBuilder {
public:
  /// \brief Type for identifiers of vertices that have been added to the
  /// MeshBuilder
  typedef int VertexHandle;

  /// \brief Type for identifiers of entities that have been
  /// added to the MeshBuilder. This later becomes the Id values of the
  /// entities.
  ...
  add_vertex(const std::vector< Coordinate > &coordinates) = 0;

  /// \brief Add a list of vertices to the Mesh under construction
  /// \param coordinates   the coordinates of the vertices in interleaved format
  /// (x0 y0 z0 x1 y1 z1 ...) \return a list with the identifiers of the new
  /// vertices
  virtual std::vector< VertexHandle >
  add_vertices(const std::vector< Coordinate > &coordinates) = 0;

  /// \brief Add an entity to the Mesh under construction
  /// \param tdim       the topological dimension of the added entity
  /// \param vertices   the list of vertices in the entity, identified through
  /// the VertexHandle:s returned from add_vertex or add_vertices. \return  the
  /// identifier of the new entity
  virtual EntityHandle
  add_entity(TDim tdim, const std::vector< VertexHandle > &vertices) = 0;

  /// \brief Add a list of entities to the Mesh under construction
  /// \param tdim       the topological dimension of the added entities
  /// \param vertices   the concatenated list of vertices of the entities,
  /// identified through their VertexHandle:s. \param sizes      a list of sizes
  /// (number of vertices) of the entities. This length of this list equals the
  /// number of entities to be added. \return a list with the identifiers of the
  /// new entities
  ...
  /// entity \param entity     identifier for the entity \param align      new
  /// align number for the entity
...
MeshBuilder::MeshBuilder(TDim tdim, GDim gdim,
                         std::vector< MasterSlave > period, bool is_rectangular)
    : tdim_(tdim), gdim_(gdim), period_(period),
      is_rectangular_(is_rectangular) {}

MeshBuilder::~MeshBuilder() {}

TDim MeshBuilder::tdim() const { return tdim_; }

GDim MeshBuilder::gdim() const { return gdim_; }

bool MeshBuilder::is_rectangular() const { return is_rectangular_; }

std::vector< MasterSlave > MeshBuilder::get_period() const { return period_; }
} // namespace mesh
} // namespace hiflow
