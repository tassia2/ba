...
std::string HDF5_ALIGN_NUM = "align_numbers";
...
MeshDatabase::MeshDatabase(TDim tdim, GDim gdim)
    : tdim_(tdim), gdim_(gdim), entity_vertex_connectivities_(tdim),
...
TDim MeshDatabase::tdim() const { return tdim_; }

GDim MeshDatabase::gdim() const { return gdim_; }

/// \param point          the coordinates of the vertex (should point to an
/// array of gdim coordinate values) \returns              the id of the vertex
...
/// \param destination    the vector of the destination (should point to an
/// array of gdim coordinate values) \param id             the id of the vertex
...
/// \param displacement   the vector of the displacement (should point to an
/// array of gdim coordinate values) \param id             the id of the vertex
...
/// \param point          the coordinates of the vertex (should point to an
/// array of gdim coordinate values) \param id             output parameter
/// which contains the id of the vertex if it was found \returns true if the
/// database contains the vertex
...
void MeshDatabase::get_coordinates(Id vertex_id, std::vector< Coordinate >& coords) const {
  coords.clear();
  coords.resize(gdim_, 0.);
  const size_t offset = gdim_ * vertex_id;
  
  for (size_t l=0; l<gdim_; ++l)
  {
    coords[l] = coordinates_[offset+l];
  }
}
...
/// \returns              a vector containing the coordinates of the vertices in
/// interleaved form (in 3d: x1 y1 z1 x2 y2 z2, ... )

void
MeshDatabase::get_coordinates(const std::vector< Id > &vertex_ids,
                              std::vector< Coordinate >& coords) const 
{
  coords.clear();
  coords.resize(gdim_ * vertex_ids.size());
  int k = 0;
  for (std::vector< Id >::const_iterator it = vertex_ids.begin();
...
MeshDatabase::get_coordinates(const std::vector< Id > &vertex_ids) const {
...
void MeshDatabase::transform_vertices(const std::vector< std::vector< double> >& A, 
                                      const std::vector<double>& b)
{
  assert (A.size() == this->gdim_);
  assert (b.size() == this->gdim_);
  
  const int num_points = this->coordinates_.size() / this->gdim_;
  
  for (int i=0; i != num_points; ++i)
  {
    std::vector<double> in_vec (this->gdim_, 0.);
  
    for (int d=0; d != this->gdim_; ++d)
    {
      in_vec[d] = this->coordinates_[i * this->gdim_ +d];
    }
    
    std::vector<double> out_vec (this->gdim_, 0.);
    for (int d=0; d != this->gdim_; ++d)
    {
      for (int s=0; s != this->gdim_; ++s)
      {
        out_vec[d] += A[d][s] * in_vec[s];
      }
      out_vec[d] += b[d];
    }
    ...
    for (int d=0; d != this->gdim_; ++d)
    {
      this->coordinates_[i * this->gdim_ +d] = out_vec[d];
    }
  }
}
...
bool MeshDatabase::find_closest_vertex(const Coordinate *x, double eps,
  ...
  while (max_index - min_index > range) {
    ...
    double middle_index_distance =
        vertex_search_table_->distance(vertices_in_db[middle_index]);
  ...
  max_index = middle_index + range / 2 <= number_of_vertices_in_db - 1
                  ? middle_index + range / 2
                  : number_of_vertices_in_db - 1;
  LOG_DEBUG(4, "(min middle max) == " << min_index << " " << middle_index << " "
                                      << max_index);
  ...
  *closest_vertex = *it;
  Coordinate minimum = vertex_search_table_->distance(
      x, vertex_search_table_->get_coordinates(*closest_vertex));
  ...
  for (; it != candidates.end(); ++it) {
    ...
    LOG_DEBUG(2, "Current minimum distance of point with id "
                     << *it << " from my search point is: " << minimum);
  }
  if (minimum < eps) {
    found = true;
  }
...
/// \param id            output parameter containing the id of the entity if it
/// was found \returns             true if the database contains the entity with
/// the given vertices
...
bool MeshDatabase::has_entity(TDim entity_dim,
  ...
  return vertex_entity_connectivities_[entity_dim - 1].find_entity(
      entity_vertices, entity_dim, this, id);
}

/// \param entity_dim    the topological dimension of the sought entity
/// \param id            the id of the sought entity
/// \returns             true if an entity (entity_dim, id) exists in the
/// database
...
/// \returns             a VertexIdIterator referencing the first vertex of the
/// entity
...
/// \param entity_dim    the topological dimension of the entity
/// \param id            the id of the entity
/// \returns             a VertexIdIterator referencing one past the last vertex
/// of the entity
...
void MeshDatabase::compute_incident_entities(
    TDim d, const SortedArray< Id > &d_entities,
  ...
  ConnectivityAlgorithms::intersect_subset_equal_dimensions(
      d_entities, d_0_connectivity, zero_d_connectivity, d_d_connectivity);
}

void MeshDatabase::compute_incident_entities(
    TDim d1, TDim d2, const SortedArray< Id > &d1_entities,
  ...
  ConnectivityAlgorithms::intersect_subset_unequal_dimensions(
      d1_entities, d2_entities, d1_0_connectivity, d2_0_connectivity,
      zero_d2_connectivity, d1_d2_connectivity);
}

void MeshDatabase::vertex_entity_incidence(
    TDim d, const SortedArray< Id > &vertices,
...
/// \pre              the entity is a cell or facet or edge
/// \param d          the topological dimension of the entity
/// \param id         id of the entity
/// \returns          align number of the entity

MaterialNumber MeshDatabase::get_align_number(TDim d, Id id) const {
  assert(d == tdim_ || d == tdim_ - 1);
  assert(has_entity(d, id));

  return align_numbers_[tdim_ - d][id];
}

/// \pre              the entity is a cell or facet or edge
/// \param d          the topological dimension of the entity
/// \param id         id of the entity
/// \param align      new align number for the entity

void MeshDatabase::set_align_number(TDim d, Id id, AlignNumber align) {
  assert(d == tdim_ || d == tdim_ - 1);
  assert(has_entity(d, id));

  align_numbers_[tdim_ - d][id] = align;
}

/// \param point          the coordinates of the vertex (should point to an
/// array of gdim coordinate values) \returns              the id of the newly
/// created vertex
...
Id MeshDatabase::create_entity(TDim entity_dim,
                               const std::vector< Id > &entity_vertices) {
  assert(entity_dim >= 1);
  assert(entity_dim <= tdim_);
  ...
  entity_vertex_connectivities_[entity_dim - 1].add_connections(
      entity_vertices);
  for (std::vector< Id >::const_iterator it = entity_vertices.begin();
       it != entity_vertices.end(); ++it) {
    vertex_entity_connectivities_[entity_dim - 1].add_vertex_connection(*it,
                                                                        id);
  }
  ...
  if (entity_dim == tdim_ || entity_dim == tdim_ - 1) {
    // Allocate space for align number for entity if it is a cell or facet or
    // line Default align number is 0.
    align_numbers_[tdim_ - entity_dim].push_back(0);
  }

  LOG_DEBUG(2, "created entity (d=" << entity_dim << ", id=" << id << "): ");
  LOG_DEBUG(2,
            string_from_range(entity_vertices.begin(), entity_vertices.end()));

  return id;
}

const Connectivity &
MeshDatabase::entity_vertex_connectivity(TDim entity_dim) const {
  assert(entity_dim > 0);
  assert(entity_dim <= tdim_);
...
const Connectivity &
MeshDatabase::vertex_entity_connectivity(TDim entity_dim) const {
  assert(entity_dim > 0);
  assert(entity_dim <= tdim_);
...
void MeshDatabase::save(std::string filename, const MPI_Comm &comm) const {
  ...
  H5DatasetPtr gdim_dataset_ptr(
      new H5Dataset(group_ptr, 1, HDF5_GDIM, "w", &gdim_));
  gdim_dataset_ptr->write(static_cast< hsize_t >(is_writer), 0, &gdim_);

  H5DatasetPtr tdim_dataset_ptr(
      new H5Dataset(group_ptr, 1, HDF5_TDIM, "w", &tdim_));
  tdim_dataset_ptr->write(static_cast< hsize_t >(is_writer), 0, &tdim_);

  // WRITING COORDINATES
  write_array_parallel< Coordinate >(group_ptr, HDF5_COORDS_NAME, coordinates_,
                                     comm);
  ...
  write_array_parallel< Id >(group_ptr, HDF5_VERTEX_SEARCH_TABLE, vertex_table,
                             comm);
  ...
  for (int mnc = 0; mnc < 2; ++mnc) {
    ...
    write_array_parallel< Id >(group_ptr, mat_data_name.str(),
                               material_numbers_[mnc], comm);
  }

  // WRITING ALIGN NUMBERS
  for (int mnc = 0; mnc < 2; ++mnc) {
    std::stringstream align_data_name;
    align_data_name << HDF5_ALIGN_NUM << mnc;
    write_array_parallel< Id >(group_ptr, align_data_name.str(),
                               align_numbers_[mnc], comm);
  }
  ...
  for (int dim = 0; dim < tdim_; ++dim) {
    ...
    for (int ent = 0; ent < conny.num_entities(); ++ent) {
      int num_conns = 0;
      for (ConstConnectionIterator inc = conny.begin(ent);
           inc != conny.end(ent); ++inc) {
  ...
  for (int dim = 0; dim < tdim_; ++dim) {
    ...
    for (int ent = 0; ent < vertex_conny.num_entities(); ++ent) {
      int num_conns = 0;
      for (ConstConnectionIterator inc = vertex_conny.begin(ent);
           inc != vertex_conny.end(ent); ++inc) {
    ...
    write_array_parallel< int >(group_ptr, vertex_conn_data_name.str(),
                                vertex_conn_as_vec, comm);
    std::stringstream vertex_offset_data_name;
    vertex_offset_data_name << HDF5_VERTEX_ENTITY_OFFSETS << dim;
    write_array_parallel< int >(group_ptr, vertex_offset_data_name.str(),
                                vertex_offsets, comm);
  }
...
void MeshDatabase::load(std::string filename, const MPI_Comm &comm) {
  ...
  H5DatasetPtr gdim_dataset_ptr(
      new H5Dataset(group_ptr, 1, HDF5_GDIM, "r", &gdim_));
  gdim_dataset_ptr->read(static_cast< hsize_t >(is_reader), 0, &gdim_);
  H5DatasetPtr tdim_dataset_ptr(
      new H5Dataset(group_ptr, 1, HDF5_TDIM, "r", &tdim_));
  tdim_dataset_ptr->read(static_cast< hsize_t >(is_reader), 0, &tdim_);
  MPI_Bcast(&tdim_, 1, MPI_INT, master_rank, comm);
  MPI_Bcast(&gdim_, 1, MPI_INT, master_rank, comm);

  // READING COORDINATES
  read_array_parallel< Coordinate >(group_ptr, HDF5_COORDS_NAME, coordinates_,
                                    comm);

  // READING VERTEX SEARCH TABLE
  // pointer so we can avoid copying data
  std::vector< Id > *vertex_table = new std::vector< Id >(0);
  read_array_parallel< Id >(group_ptr, HDF5_VERTEX_SEARCH_TABLE, *vertex_table,
                            comm);
  ...
  for (int mnc = 0; mnc < 2; ++mnc) {
    std::stringstream mat_data_name;
    mat_data_name << HDF5_MATERIAL_NUM << mnc;
    read_array_parallel< Id >(group_ptr, mat_data_name.str(),
                              material_numbers_[mnc], comm);
  }

  // READING ALIGN NUMBERS
  for (int mnc = 0; mnc < 2; ++mnc) {
    std::stringstream align_data_name;
    align_data_name << HDF5_ALIGN_NUM << mnc;
    read_array_parallel< Id >(group_ptr, align_data_name.str(),
                              align_numbers_[mnc], comm);
  }
  ...
  for (int dim = 0; dim < tdim_; ++dim) {
    ...
    for (int i = 0; i < offsets.size(); ++i) {
      next_pos += offsets[i];
      entity_vertex_connectivities_[dim].add_connections(std::vector< int >(
  ...
  for (int dim = 0; dim < tdim_; ++dim) {
    ...
    std::stringstream vertex_conn_data_name;
    vertex_conn_data_name << HDF5_VERTEX_ENTITY << dim;
    read_array_parallel< int >(group_ptr, vertex_conn_data_name.str(),
                               vertex_conn_as_vec, comm);
    std::stringstream vertex_offset_data_name;
    vertex_offset_data_name << HDF5_VERTEX_ENTITY_OFFSETS << dim;
    read_array_parallel< int >(group_ptr, vertex_offset_data_name.str(),
                               vertex_offsets, comm);
    int curr_pos = 0;
    int next_pos = 0;
    for (int i = 0; i < vertex_offsets.size(); ++i) {
      next_pos += vertex_offsets[i];
      vertex_entity_connectivities_[dim].add_connections(std::vector< int >(
          vertex_conn_as_vec.begin() + curr_pos,
          vertex_conn_as_vec.begin() + curr_pos + vertex_offsets[i]));
      curr_pos = next_pos;
    }
  }
#else
...
void MeshDatabase::copy_from(const MeshDatabasePtr db) {
  ...
  // copy align numbers
  for (int l = 0; l < 2; ++l) {
    this->align_numbers_[l] = db->align_numbers_[l];
  }

  // copy entity_vertex connectivity
  LOG_DEBUG(1, "Entity_vertex_connectivity size "
                   << db->entity_vertex_connectivities_.size());

  this->entity_vertex_connectivities_.resize(
      db->entity_vertex_connectivities_.size());

  for (int ent_dim = 1; ent_dim <= tdim; ++ent_dim) {
    Connectivity &conny = db->entity_vertex_connectivities_[ent_dim - 1];
    LOG_DEBUG(1, "Size of INPUT Entity_vertex_connectivity for entdim "
                     << ent_dim << " = " << conny.num_entities());
    ...
    for (int i = 0; i < offsets.size(); ++i) {
      next_pos += offsets[i];
      this->entity_vertex_connectivities_[ent_dim - 1].add_connections(
          std::vector< int >(conn_as_vec.begin() + curr_pos,
                             conn_as_vec.begin() + next_pos));
      curr_pos = next_pos;
    }

    LOG_DEBUG(
        1,
        "Size of Entity_vertex_connectivity for entdim "
            << ent_dim << " = "
            << this->entity_vertex_connectivities_[ent_dim - 1].num_entities());
  }

  // copy vertex_entity connections
  this->vertex_entity_connectivities_.resize(
      db->vertex_entity_connectivities_.size());
  for (int ent_dim = 1; ent_dim <= tdim_; ++ent_dim) {
    VertexConnectivity &vertex_conny =
        db->vertex_entity_connectivities_[ent_dim - 1];
    // convert connectivity to vector
    std::vector< int > vertex_conn_as_vec(0);
    std::vector< int > vertex_offsets(vertex_conny.num_entities());
    for (int ent = 0; ent < vertex_conny.num_entities(); ++ent) {
      int num_conns = 0;
      for (ConnectionIterator inc = vertex_conny.begin(ent);
           inc != vertex_conny.end(ent); ++inc) {
        vertex_conn_as_vec.push_back(*inc);
        ++num_conns;
      }
    ...
    for (int i = 0; i < vertex_offsets.size(); ++i) {
      next_pos += vertex_offsets[i];
      this->vertex_entity_connectivities_[ent_dim - 1].add_connections(
          std::vector< int >(vertex_conn_as_vec.begin() + curr_pos,
                             vertex_conn_as_vec.begin() + curr_pos +
                                 vertex_offsets[i]));
      curr_pos = next_pos;
    }
...
void MeshDatabase::create_entity_package(int entity_tdim,
  ...
  for (int j = 0; j < ids.size(); ++j) {
    ...
    std::vector< Id > vertex_id =
        std::vector< Id >(this->begin_vertices(entity_tdim, ids[j]),
                          this->end_vertices(entity_tdim, ids[j]));
    MaterialNumber material_number =
        this->get_material_number(entity_tdim, ids[j]);

    AlignNumber align_number = this->get_align_number(entity_tdim, ids[j]);
    ...
    entities->align_numbers.push_back(align_number);
  }
  // last offset
  entities->offsets.push_back(entities->connections.size());
}
...
void VertexConnectivity::add_vertex_connection(Id vertex_id,
                                               Id connected_entity_id) {
  assert(vertex_id >= 0);
  assert(vertex_id < static_cast< int >(connections_.size()));

  // OLD IMPLEMENTATION
  // NB: no check that the vertex is not already connected to the entity is
  // performed
  ...
  std::vector< int >::iterator contained =
      std::find(connections_[vertex_id].begin(), connections_[vertex_id].end(),
                connected_entity_id);
  if (contained != connections_[vertex_id].end()) {
    return;
  }
  connections_[vertex_id].push_back(connected_entity_id);
  std::stable_sort(connections_[vertex_id].begin(),
                   connections_[vertex_id].end());
}

bool VertexConnectivity::find_entity(const std::vector< Id > &entity_vertices,
                                     const TDim tdim, const MeshDatabase *db,
                                     Id &id) const {
  ...
  for (Citerator v_it = entity_vertices.begin() + 1;
    ...
    for (Iterator target = intersection.begin(); target != intersection.end();
         ++target) {
      const bool found =
          std::binary_search(connected_begin, connected_end, *target);
      if (found) {
  ...
  if (!intersection.empty()) {
    ...
    if (static_cast< int >(entity_vertices.size()) ==
        static_cast< int >(db->entity_size(tdim, id))) {
      found_unique = true;
      if (intersection.size() != 1) {
        LOG_ERROR("Intersection contains " << intersection.size()
                                           << " elements:");
...
VertexSearchTable::VertexSearchTable(
    GDim gdim, const std::vector< Coordinate > &coordinates)
...
Coordinate VertexSearchTable::distance(const Coordinate *point1,
                                       const Coordinate *point2) const {
...
std::vector< Id > VertexSearchTable::vertices() const { return vertices_; }
...
bool compare_floating_point(double x1, double x2, double abs_tol,
                            double rel_tol) {
...
bool VertexSearchTable::compare_points(const Coordinate *p1,
...
void VertexSearchTable::update_vertex(Id id) {
  ...
  if (pos != -1) { // if vertex already exists
