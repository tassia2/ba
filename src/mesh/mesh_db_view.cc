...
MeshDbView::MeshDbView(TDim tdim, GDim gdim, MeshDatabasePtr db,
                       const std::vector< Id > &cells,
                       std::vector< MasterSlave > period, bool is_rectangular)
  : Mesh(tdim, gdim, period, is_rectangular),
    ...
    rec_ref_geom_fun_(&recursive_refined_geometry_fun), db_(db) {
...
IncidentEntityIterator MeshDbView::begin_incident(const Entity &entity,
  ...
  const Entity new_entity =
    Entity(ConstMeshPtr(this), entity_dim, new_entity_index);
  return IncidentEntityIterator(new_entity, connectivity.begin(index));
}

IncidentEntityIterator MeshDbView::end_incident(const Entity &entity,
    TDim entity_dim) const {
  assert(entity_dim >= 0);
  assert(entity_dim <= tdim());
...
Id MeshDbView::get_id(TDim entity_dim, EntityNumber index) const {
  ...
  if (index >= num_entities(entity_dim)) {
    LOG_DEBUG(3, "dim = " << entity_dim << ", index = " << index
              << " num_entities = " << num_entities(entity_dim));
  }
#endif
...
std::vector< Id > MeshDbView::get_vertex_ids(TDim entity_dim,
...
void MeshDbView::get_vertex_ids(TDim entity_dim,
    EntityNumber index,
    std::vector< Id >& ids) const 
{
  ...
  ids.clear();
  ids.reserve(8);
  const auto it0 = db_->begin_vertices(entity_dim, entity_id);
  const auto eit = db_->end_vertices(entity_dim, entity_id);
  
  for (auto it = it0; it != eit; ++it)
  {
    ids.push_back(*it);
  }
}

MaterialNumber MeshDbView::get_material_number(TDim entity_dim,
    EntityNumber index) const {
  return db_->get_material_number(entity_dim, get_id(entity_dim, index));
}

void MeshDbView::set_material_number(TDim entity_dim, EntityNumber index,
                                     MaterialNumber material) {
  return db_->set_material_number(entity_dim, get_id(entity_dim, index),
                                  material);
}

AlignNumber MeshDbView::get_align_number(TDim entity_dim,
    EntityNumber index) const {
  return db_->get_align_number(entity_dim, get_id(entity_dim, index));
}

void MeshDbView::set_align_number(TDim entity_dim, EntityNumber index,
                                  AlignNumber align) {
  return db_->set_align_number(entity_dim, get_id(entity_dim, index), align);
}

#if 0
std::vector< Coordinate >
MeshDbView::get_coordinates(TDim entity_dim, EntityNumber index) const {
  // only of size gdim if entity_dim == 0
  std::vector< Coordinate > coords(gdim());
  if (entity_dim == 0) {
    db_->get_coordinates(get_id(0, index), coords);
  } else {
    coords = db_->get_coordinates(get_vertex_ids(entity_dim, index));
  }

  bool displacement_x =
    has_attribute(std::string("__displacement_vertices_x__"), 0);
  if (displacement_x) {
    if (entity_dim != 0) {
      VertexIdIterator vii =
        db_->begin_vertices(entity_dim, get_id(entity_dim, index));
      int k = 0;
      for (; vii != db_->end_vertices(entity_dim, get_id(entity_dim, index));
           ++vii) {
        int vertex_index;
...
MeshDbView::get_coordinates(TDim entity_dim, EntityNumber index) const
{
  ...
  this->get_coordinates(entity_dim, 
                        index, 
                        coords);
  return coords;
}
...
void MeshDbView::get_coordinates(TDim entity_dim, 
                                 EntityNumber index, 
                                 std::vector< Coordinate >& coords) const
{
  ...
  std::vector< Id > vertex_ids;
  // only of size gdim if entity_dim == 0
  if (entity_dim == 0) 
  {
    db_->get_coordinates(get_id(0, index), coords);
    assert (coords.size() == gdim());
  } 
  else 
  {
    this->get_vertex_ids(entity_dim, index, vertex_ids);
    
    db_->get_coordinates(vertex_ids, coords);
    
    //std::cout << coords.size() << " " << vertex_ids.size() << " " << gdim() << std::endl;
    assert (coords.size() == vertex_ids.size() * gdim());
  }
...
void MeshDbView::transform_vertices (const std::vector< std::vector< double> >& A, 
                                     const std::vector<double>& b)
{
  this->db_->transform_vertices(A,b);
}
...
std::vector< Id >
MeshDbView::get_children_cell_ids(EntityNumber cell_index) const {
  NOT_YET_IMPLEMENTED;
  std::vector< Id > tmp;
  return tmp;
}
...
MeshDbView::get_sibling_cell_indices(EntityNumber cell_index) const {
  std::vector< EntityNumber > siblings(0);
  return siblings;
}

Entity MeshDbView::get_entity(TDim entity_dim,
...
EntityCount MeshDbView::num_entities(TDim entity_dim) const {
  assert(entity_dim >= 0);
  assert(entity_dim <= tdim());
  const std::vector< Id > &entities = this->get_entities(entity_dim);
  return entities.size();
}

EntityCount MeshDbView::num_incident_entities(const Entity &entity,
    TDim entity_dim) const {
  assert(entity_dim >= 0);
  assert(entity_dim <= tdim());
...
Connectivity &MeshDbView::get_incidence_connectivity(TDim d1, TDim d2) const {
  ...
  {
    // d->0 connectivities are computed in get_entity() and should not be
    // required here.
    assert(d2 != 0);
    ...
    else if (d1 == tdim() && d2 < tdim()) 
    {
      ...
      // the connectivity reference now points to the object that was computed
      // in get_entities()
    } 
    else if (d1 > d2) 
    {
    ...
    else if (d1 == d2) 
    {
      ...
      const Connectivity &d2_d1_connectivity =
        get_incidence_connectivity(d2, d1);
  ...
  if (entities_[d].empty()) 
  {
    ...
    else 
    {
      ...
      Connectivity &D_d_connectivity =
        incidence_connections_[incidence_connection_index(tdim(), d)];
      ...
      LOG_DEBUG(2, "End computing connectivity "
                << tdim() << ", " << d << " -> index = "
                << incidence_connection_index(tdim(), d));
    }
  }
...
bool MeshDbView::has_computed_connectivity(TDim d1, TDim d2) const {
  return incidence_connections_[incidence_connection_index(d1, d2)]
         .num_entities() != 0;
}
...
void MeshDbView::compute_entity_vertex_connectivity(TDim d) const {
  ...
  for (SortedArray< Id >::const_iterator it = entities.begin();
    ...
    for (std::vector< Id >::iterator v_it = entity_vertices.begin();
         v_it != entity_vertices.end(); ++v_it) {
...
bool MeshDbView::find_entity(TDim entity_dim, Id id,
                             EntityNumber *entity_number) const {
  const SortedArray< Id > &entities = get_entities(entity_dim);
  return entities.find(id, entity_number);
}

bool MeshDbView::find_vertex(const std::vector< Coordinate > &coords,
...
MeshPtr MeshDbView::refine(std::vector< int > &refinements) const {
  ...
  for (int c = 0; c < num_cells; ++c) {
    ...
    if (r > 0) { // refinement (subdivision)
      LOG_DEBUG(2, "Refining cell " << c << " with refinement " << r);

      const RefinementTree *tree =
        current_cell.cell_type().refinement_tree(r - 1);

      std::vector< int > local_sub_cell_numbers;
      const std::vector< Id > children_cells =
        compute_refined_cells(current_cell, tree, local_sub_cell_numbers);
      new_cells.insert(new_cells.end(), children_cells.begin(),
                       children_cells.end());
      sub_cell_numbers.insert(sub_cell_numbers.end(),
                              local_sub_cell_numbers.begin(),
                              local_sub_cell_numbers.end());
  ...
  // create refined mesh
  MeshPtr refined_mesh = MeshPtr(new RefinedMeshDbView(
                                   td, gd, db_, new_cells, refined_history, parent_history_indices,
                                   parent_cell_indices, sub_cell_numbers, period_, this->is_rectangular_));
  return refined_mesh;
}
...
bool MeshDbView::is_boundary_facet(EntityNumber facet_index) const {
  ...
  const std::vector< Entity > incident_cells(
    begin_incident(facet_entity, tdim()), end_incident(facet_entity, tdim()));
  LOG_DEBUG(3, "Incident cells: " << string_from_range(incident_cells.begin(),
            incident_cells.end()));
  ...
  if (incident_cells.size() == 1) {
    ...
    LOG_DEBUG(3, "Facet with index " << facet_index
...
get_cell_facet_materials(const MeshDatabasePtr &db, const Entity &cell) {
  ...
  const std::vector< Id > cell_vertices(cell.begin_vertex_ids(),
                                        cell.end_vertex_ids());
  ...
  for (int f = 0; f < num_facets; ++f) {
    ...
    Id facet_id;
    if (db->has_entity(facet_tdim, facet_vertices,
                       facet_id)) { // does facet exist in database?
      const MaterialNumber material =
        db->get_material_number(facet_tdim, facet_id);

      if (material >= 0) { // does it have non-negative material id?
        if (material_numbers.empty()) { // create vector only if a valid
          // material id was found
...
std::vector< AlignNumber > get_cell_facet_aligns(const MeshDatabasePtr &db,
  ...
  for (int f = 0; f < num_facets; ++f) {
    ...
    if (db->has_entity(facet_tdim, facet_vertices,
                       facet_id)) { // does facet exist in database?
      const AlignNumber align = db->get_align_number(facet_tdim, facet_id);

      if (align >= 0) {              // does it have non-negative material id?
        if (align_numbers.empty()) { // create vector only if a valid material
          // id was found
          align_numbers.resize(num_facets, -1);
        }
        align_numbers[f] = align;
      }
    }
  }

  return align_numbers;
}
...
void inherit_facet_material_numbers(
  ...
  const CellType &cell_type =
    CellType::get_instance(cell_tdim, sub_cell_vertices.size());
  const int num_facets = cell_type.num_regular_entities(facet_tdim);

  // local facet numbers of the sub-cell in the parent cell type
  const std::vector< int > facets_in_parent =
  ...
  for (int f = 0; f < num_facets; ++f) {
    ...
    const MaterialNumber parent_material_number =
      parent_facet_materials[parent_facet_number];

    // Check whether the parent facet material number has been set, and should
    // be inherited
    if (parent_material_number >= 0) {

      // get vertices of sub-facet
      std::vector< Id > sub_facet_vertices =
        cell_type.vertices_of_entity(facet_tdim, f, sub_cell_vertices);
...
void inherit_facet_align_numbers(
  ...
  for (int f = 0; f < num_facets; ++f) {
    ...
    const AlignNumber parent_align_number =
      parent_facet_aligns[parent_facet_number];

    // Check whether the parent facet material number has been set, and should
    // be inherited
    if (parent_align_number >= 0) {
      ...
      // set material number for sub-facet
      db->set_align_number(facet_tdim, sub_facet_id, parent_align_number);
    }
  }
}
//        }
...
std::vector< Id >
MeshDbView::compute_refined_cells(const Entity &cell,
                                  const RefinementTree *tree,
  ...
  ::hiflow::mesh::compute_refined_cells(cell, *tree, refined_cell_coordinates,
                                        refined_cell_sizes, sub_cell_numbers,
                                        ref_geom_fun_);
...
void MeshDbView::replace_vertex(const std::vector< Coordinate > &destination,
                                Id vertex_index) {
  assert(!destination.empty());
  assert(static_cast< int >(destination.size()) == gdim());
  // the local vertex_index has to be mapped to the global vertex_id with the
  // help of entities
  db_->replace_vertex(vec2ptr(destination), entities_[0][vertex_index]);
}

void MeshDbView::move_vertex(const std::vector< Coordinate > &displacement,
                             Id vertex_index) {
...
void MeshDbView::move_vertices(const std::vector< Coordinate > &displacements) {
  ...
  for (int v = 0; v < num_verts; ++v) {
    const std::vector< Coordinate > pt =
      std::vector< Coordinate >(displacements.begin() + gdim() * v,
...
MeshPtr MeshDbView::extract_boundary_mesh() const {
  ...
  std::vector< Id >
  boundary_facets; // id:s of boundary facets (used to create boundary mesh)
  std::vector< EntityNumber >
  mesh_facet_indices; // indices of boundary facets in current mesh (used to
  // map from bdy mesh to current mesh)
  ...
  if (!boundary_facets.empty()) {
    ...
    std::vector< EntityNumber > sorted_mesh_facet_indices(
      boundary_facets.size());
    permute_vector(boundary_facet_permutation, mesh_facet_indices,
                   sorted_mesh_facet_indices);
    ...
    MeshPtr bdy_mesh(
      new MeshDbView(mesh_tdim - 1, gdim(), db_, boundary_facets));
...
MeshPtr MeshDbView::extract_facet_mesh() const {
  const TDim mesh_tdim = tdim();
...
void MeshDbView::set_recursive_refined_geometry_function(
  RecursiveRefinedGeometryFunction f) {
  if (f != nullptr) {
    rec_ref_geom_fun_ = f;
  } else {
    rec_ref_geom_fun_ = &recursive_refined_geometry_fun;
...
void MeshDbView::convert_connectivity_to_indices(
  TDim d, Connectivity &connectivity) const {
  typedef Connectivity::ConnectionIterator CIterator;
  for (int i = 0; i < connectivity.num_entities(); ++i) {
    for (CIterator it = connectivity.begin(i); it != connectivity.end(i);
         ++it) {
...
bool MeshDbView::cell_is_local(EntityNumber index) const {
  ...
  int remote_index = -1;
  this->get_attribute_value("_remote_index_", this->tdim(), index,
                            &remote_index);
  return remote_index < 0;
}

void MeshDbView::set_mesh(ConstMeshPtr mesh_ptr, int history_index) {
  ...
  std::pair< std::map< int, ConstMeshPtr >::iterator, bool > ret =
...
ConstMeshPtr MeshDbView::get_mesh(int history_index) const {
  assert(history_index >= 0);
  assert(history_index <= mesh_history_.size());
  assert(this->mesh_history_.find(history_index) != this->mesh_history_.end());
  return this->mesh_history_.find(history_index)->second;
}
...
void MeshDbView::save(std::string filename, const MPI_Comm &comm) const {
  ...
  for (int dim = 0; dim < tdim() + 1; ++dim) {
    ...
    write_array_parallel< Id >(group_ptr, entity_name.str(),
                               entities_[dim].data(), comm);
  }
  ...
  for (int i = 0; i < (tdim() + 1) * (tdim() + 1); ++i) {
    ...
    for (int ent = 0; ent < conny.num_entities(); ++ent) {
      int num_conns = 0;
      for (ConnectionIterator inc = conny.begin(ent); inc != conny.end(ent);
           ++inc) {
    ...
    write_array_parallel< int >(group_ptr, conn_data_name.str(), conn_as_vec,
                                comm);
    std::stringstream offset_data_name;
    offset_data_name << "incidence_connections_offsets" << i;
    write_array_parallel< int >(group_ptr, offset_data_name.str(), offsets,
                                comm);
  }
#else
...
void MeshDbView::load(std::string filename, const MPI_Comm &comm) {
  ...
  for (int dim = 0; dim < tdim() + 1; ++dim) {
    std::stringstream entity_name;
    entity_name << "entities" << dim;
    read_array_parallel< Id >(group_ptr, entity_name.str(),
                              entities_[dim].data(), comm);
  }
  ...
  for (int i = 0; i < (tdim() + 1) * (tdim() + 1); ++i) {
    ...
    std::stringstream conn_data_name;
    conn_data_name << "incidence_connections" << i;
    read_array_parallel< int >(group_ptr, conn_data_name.str(), conn_as_vec,
                               comm);
    std::stringstream offset_data_name;
    offset_data_name << "incidence_connections_offsets" << i;
    read_array_parallel< int >(group_ptr, offset_data_name.str(), offsets,
                               comm);
    int curr_pos = 0;
    int next_pos = 0;
    ...
    for (int j = 0; j < offsets.size(); ++j) {
      next_pos += offsets[j];
      incidence_connections_[i].add_connections(std::vector< int >(
            conn_as_vec.begin() + curr_pos, conn_as_vec.begin() + next_pos));
      curr_pos = next_pos;
    }
  }
#else
...
void MeshDbView::copy_from(ConstMeshPtr mesh) {
  ...
  boost::intrusive_ptr< const MeshDbView > mesh_dbview =
    boost::static_pointer_cast< const MeshDbView >(mesh);
...
//  this->geometric_search_ = mesh_dbview->geometric_search_;
  ...
  for (int i = 0; i < (tdim + 1) * (tdim + 1); ++i) {
    ...
    for (int j = 0; j < offsets.size(); ++j) {
      next_pos += offsets[j];
      this->incidence_connections_[i].add_connections(std::vector< int >(
            conn_as_vec.begin() + curr_pos, conn_as_vec.begin() + next_pos));
      curr_pos = next_pos;
    }
  }
...
void MeshDbView::deep_copy_from(ConstMeshPtr mesh) {
  // this loop has to be called to avoid later errors due to missing entities in
  // input mesh. num_entities creates all necessary entities in input mesh
  const int tdim = mesh->tdim();
  for (int dim = 0; dim < tdim + 1; ++dim) {
  ...
  boost::intrusive_ptr< const MeshDbView > mesh_dbview =
    boost::static_pointer_cast< const MeshDbView >(mesh);
  tmp_db->copy_from(mesh_dbview->get_db());
  ...
  for (int h = 0; h < this->history_index_; ++h) {
    ...
    MeshPtr tmp_mesh(new MeshDbView(this->tdim(), this->gdim()));
    ...
    boost::intrusive_ptr< MeshDbView > tmp_mesh_dbview =
      boost::static_pointer_cast< MeshDbView >(tmp_mesh);
    tmp_mesh_dbview->set_db(tmp_db);
...
MeshDbViewBuilder::MeshDbViewBuilder(MeshDatabasePtr db,
                                     std::vector< MasterSlave > period,
                                     bool is_rectangular)
  : MeshBuilder(db->tdim(), db->gdim(), period, is_rectangular), db_(db) {
  assert(db_ != 0);
}

MeshDbViewBuilder::MeshDbViewBuilder(TDim tdim, GDim gdim,
                                     std::vector< MasterSlave > period,
                                     bool is_rectangular)
  : MeshBuilder(tdim, gdim, period, is_rectangular),
    db_(new MeshDatabase(tdim, gdim)) {
  assert(db_ != 0);
}

MeshDbViewBuilder::MeshDbViewBuilder(TDim tdim, GDim gdim, int flag,
                                     std::vector< MasterSlave > period,
                                     bool is_rectangular)
  : MeshBuilder(tdim, gdim, period, is_rectangular) {}

MeshDbViewBuilder::MeshDbViewBuilder(const MeshDbView &mesh)
  : MeshBuilder(mesh.tdim(), mesh.gdim(), mesh.get_period(),
                mesh.is_rectangular()),
    db_(mesh.db_) {
...
MeshDbViewBuilder::MeshDbViewBuilder ( TDim tdim, GDim gdim,
                                       DatabaseConnectionPtr connection )
  : MeshBuilder ( tdim, gdim ), db_ ( new MeshDatabase ( tdim, gdim,
                                      connection ) ) {
  assert ( db_ != 0 );
}
#endif
...
VertexHandle
MeshDbViewBuilder::add_vertex(const std::vector< Coordinate > &coordinates) {
  assert(!coordinates.empty());
  assert(static_cast< int >(coordinates.size()) == gdim());
  if (!period_.empty()) {
    return db_->add_vertex(vec2ptr(
                             hiflow::mesh::periodify_slave_to_master(coordinates, gdim(), period_)));
  }
  return db_->add_vertex(vec2ptr(coordinates));
}

std::vector< VertexHandle >
MeshDbViewBuilder::add_vertices(const std::vector< Coordinate > &coordinates) {
  ...
  for (int v = 0; v < num_verts; ++v) {
    const std::vector< Coordinate > pt =
      std::vector< Coordinate >(coordinates.begin() + gdim() * v,
                                coordinates.begin() + gdim() * (v + 1));
    if (!period_.empty()) {
      vertices[v] = db_->add_vertex(
                      vec2ptr(periodify_slave_to_master(pt, gdim(), period_)));
    } else {
...
EntityHandle
MeshDbViewBuilder::add_entity(TDim tdim,
...
MeshDbViewBuilder::add_entities(TDim tdim,
                                const std::vector< VertexHandle > &vertices,
  ...
  for (int e = 0; e < num_entities; ++e) {
    ...
    entities[e] = db_->add_entity(
                    tdim,
                    std::vector< VertexHandle >(vertices.begin() + offset,
...
void MeshDbViewBuilder::set_material_number(TDim tdim, EntityHandle entity,
    MaterialNumber material) {
  db_->set_material_number(tdim, entity, material);
}

void MeshDbViewBuilder::set_align_number(TDim tdim, EntityHandle entity,
    AlignNumber align) {
  db_->set_align_number(tdim, entity, align);
}
...
MeshPtr MeshDbViewBuilder::build() {
  if (!cells_.empty()) {
    return MeshPtr(new MeshDbView(db_->tdim(), db_->gdim(), db_, cells_,
                                  period_, is_rectangular_));
  }
  return MeshPtr(0);
}
} // namespace mesh
