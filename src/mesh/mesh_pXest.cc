...
/// \brief Function which is called for each coarsenbale family of quadrants.
/// This function checks whether the given family should be coarsened, but does NOT coarsen it actually
/// @param[in] forest pointer to underlying forest object
/// @param[in] tree global id of current tree object
/// @param[in] quads pointers to quadrants forming a family
/// @return true if family of quads shouzld be coarsened

int pXest_coarsen_check_fn(p4est_t *forest, p4est_topidx_t tree,
                           p4est_quadrant_t *quads[]) {
  // get pointer to external user data
  ForestCoarsenData *i_ptr =
      static_cast< ForestCoarsenData * >(forest->user_pointer);
...
int pXest_coarsen_check_fn(p8est_t *forest, p4est_topidx_t tree,
                           p8est_quadrant_t *quads[]) {
  // get pointer to external user data
  ForestCoarsenData *i_ptr =
      static_cast< ForestCoarsenData * >(forest->user_pointer);
...
bool MeshPXest::check_for_coarsening(std::vector< EntityNumber > &refinements) const 
{
#ifdef WITH_P4EST
  ...
  coarsen_data.did_any_coarsening = false;
  ...
  if (tdim == 2) 
  {
    ...
    p4est_coarsen_ext(forest, 0, 0, pXest_coarsen_check_fn,
                      pXest_coarsen_init_fn, pXest_coarsen_replace_fn);

    if (coarsen_data.did_any_coarsening)
    {
      return true;
    }
  }
  else if (tdim == 3) 
  {
    ...
    pXest_set_forest_user_ptr(forest, &coarsen_data);
    p8est_coarsen_ext(forest, 0, 0, pXest_coarsen_check_fn,
                      pXest_coarsen_init_fn, pXest_coarsen_replace_fn);
...
void MeshPXest::create_interface_list(InterfaceList *list) const {
  ...
  ConstMeshPtr tmp_mesh = ConstMeshPtr(this);
  list->determine_interface_types(tmp_mesh);
#endif
}
...
Id MeshPXest::get_ancestor_cell_id(Id cell_id, int level_diff) const {
  ...
  // REDO
  LOG_DEBUG(3, "id " << cell_id << " pos " << pos << " level " << level << " x " << x << " y "
                     << y << " z " << z << " tree " << tree_id);
  if (level < 1) {
    return -1;
  }
