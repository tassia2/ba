...
void MpiBroadcast::communicate(/*const*/ EntityPackage *sent_entities,
  ...
  received_entities->align_numbers.resize(num_entities, 0);

  // Swap data into received_entities.

  // sent_entities is only temporarily modified, and will be
  // reset to its original state before end of function (assuming no exceptions
  // occur).
  if (is_root()) {
    ...
    received_entities->align_numbers = sent_entities->align_numbers;
  }

  // Broadcast vectors.
  MPI_Bcast(vec2ptr(received_entities->coords), num_coords, MPI_DOUBLE, root_,
            mpi_comm_);
  MPI_Bcast(vec2ptr(received_entities->offsets), num_entities + 1, MPI_INT,
            root_, mpi_comm_);
  MPI_Bcast(vec2ptr(received_entities->connections), num_connections, MPI_INT,
            root_, mpi_comm_);
  MPI_Bcast(vec2ptr(received_entities->material_numbers), num_entities, MPI_INT,
            root_, mpi_comm_);
  MPI_Bcast(vec2ptr(received_entities->align_numbers), num_entities, MPI_INT,
            root_, mpi_comm_);

  MPI_Barrier(mpi_comm_);
}

bool MpiBroadcast::is_root() const { return rank() == root_; }

int MpiBroadcast::rank() const { return rank_; }

//////////////// MpiScatter ////////////////

MpiScatter::MpiScatter(const MPI_Comm &mpi_communicator, int sender,
                       const std::vector< EntityCount > &num_entities_on_proc)
    : sender_(sender), mpi_comm_(mpi_communicator),
      num_entities_on_proc_(num_entities_on_proc) {
  if (is_sender()) {
    int size = -1;
    MPI_Comm_size(mpi_comm_, &size);
    if (static_cast< int >(num_entities_on_proc.size()) != size) {
      LOG_DEBUG(2, "num_entities on proc = " << num_entities_on_proc.size()
                                             << ", num procs = " << size);
    }
...
void MpiScatter::communicate(/*const*/ EntityPackage *sent_entities,
  ...
  LOG_DEBUG(2, "GDim = " << received_entities->gdim << ", TDim = "
                         << received_entities->tdim << " on proc " << rank());
  ...
  LOG_DEBUG(3, "On proc " << rank() << ", " << num_coordinates
                          << " coordinates will be received");

  // broadcast coordinates vector
  LOG_DEBUG(2, "Broadcasting coordinates");
  if (is_sender()) {
    // TODO(Staffan): can we avoid copy here, while still keeping the code
    // clean?
    received_entities->coords = sent_entities->coords;
  }
  MPI_Bcast(vec2ptr(received_entities->coords), num_coordinates, MPI_DOUBLE,
            sender_, mpi_comm_);

  // scatter number of received entities for each proc
  int num_received_entities;
  EntityCount *num_ent_on_proc = const_cast< EntityCount * >(
      vec2ptr(num_entities_on_proc_)); // work-around C weakness
  LOG_DEBUG(2, "Scattering number of received entities");
  MPI_Scatter(num_ent_on_proc, 1, MPI_INT, &num_received_entities, 1, MPI_INT,
              sender_, mpi_comm_);
  ...
  MPI_Scatterv(vec2ptr(sent_entities->offsets), num_ent_on_proc,
               vec2ptr(entity_offset_on_proc), MPI_INT,
               vec2ptr(received_entities->offsets), num_received_entities,
               MPI_INT, sender_, mpi_comm_);

  // scatter the material_numbers vector
  received_entities->material_numbers.resize(num_received_entities);
  MPI_Scatterv(vec2ptr(sent_entities->material_numbers), num_ent_on_proc,
               vec2ptr(entity_offset_on_proc), MPI_INT,
               vec2ptr(received_entities->material_numbers),
               num_received_entities, MPI_INT, sender_, mpi_comm_);

  // scatter the align_numbers vector
  received_entities->align_numbers.resize(num_received_entities);
  MPI_Scatterv(vec2ptr(sent_entities->align_numbers), num_ent_on_proc,
               vec2ptr(entity_offset_on_proc), MPI_INT,
               vec2ptr(received_entities->align_numbers), num_received_entities,
               MPI_INT, sender_, mpi_comm_);
  ...
  if (is_sender()) {
    ...
    for (int p = 0; p < num_processes; ++p) {
      ...
      if (next_entity == static_cast< int >(sent_entities->offsets.size())) {
        num_connections_on_proc[p] = sent_entities->connections.size() -
                                     sent_entities->offsets[curr_entity];
      } else {
        num_connections_on_proc[p] = sent_entities->offsets[next_entity] -
                                     sent_entities->offsets[curr_entity];
      }
  ...
  MPI_Scatter(vec2ptr(process_offsets), 1, MPI_INT, &proc_offset, 1, MPI_INT,
              sender_, mpi_comm_);
  ...
  LOG_DEBUG(3, "On process " << rank() << ", " << num_received_connections
  ...
  std::transform(received_entities->offsets.begin(),
                 received_entities->offsets.end(),
                 received_entities->offsets.begin(),
                 std::bind2nd(std::minus< int >(), proc_offset));

  // set ending offset
  received_entities->offsets.push_back(num_received_connections);

  LOG_DEBUG(3, "Received offset vector on process "
                   << rank() << " \n"
                   << string_from_range(received_entities->offsets.begin(),
                                        received_entities->offsets.end()));

  // scatter connections vector
  std::vector< int > connection_offset_on_proc;
  if (is_sender()) {
    convert_sizes_to_offsets(num_connections_on_proc,
                             connection_offset_on_proc);
  }
  LOG_DEBUG(2, "Scattering connections");
  MPI_Scatterv(vec2ptr(sent_entities->connections),
               vec2ptr(num_connections_on_proc),
               vec2ptr(connection_offset_on_proc), MPI_INT,
               vec2ptr(received_entities->connections),
               num_received_connections, MPI_INT, sender_, mpi_comm_);
...
void MpiScatter::communicate(std::vector< EntityPackage > &sent_entities,
  ...
  MPI_Scatter(vec2ptr(num_coordinates_sent), 1, MPI_INT, &num_coordinates_recv,
              1, MPI_INT, sender_, mpi_comm_);

  received_entities.coords.resize(num_coordinates_recv);
  LOG_DEBUG(3, "On proc " << rank() << ", " << num_coordinates_recv
                          << " coordinates will be received");
  ...
  MPI_Scatterv(vec2ptr(coordinates_sent), vec2ptr(num_coordinates_sent),
               vec2ptr(sdispl), MPI_DOUBLE, vec2ptr(received_entities.coords),
               num_coordinates_recv, MPI_DOUBLE, sender_, mpi_comm_);
  ...
  MPI_Scatter(vec2ptr(offset_sizes_sent), 1, MPI_INT, &offset_sizes_recv, 1,
              MPI_INT, sender_, mpi_comm_);

  received_entities.offsets.resize(offset_sizes_recv);
  LOG_DEBUG(3, "On proc " << rank() << ", " << offset_sizes_recv
  ...
  if (is_sender()) {
    for (int i = 0; i < num_procs; ++i) {
      sent_offsets.insert(sent_offsets.end(), sent_entities[i].offsets.begin(),
                          sent_entities[i].offsets.end());
      sdispl[i + 1] = sdispl[i] + sent_entities[i].offsets.size();
    }
  }

  MPI_Scatterv(vec2ptr(sent_offsets), vec2ptr(offset_sizes_sent),
               vec2ptr(sdispl), MPI_INT, vec2ptr(received_entities.offsets),
               offset_sizes_recv, MPI_INT, sender_, mpi_comm_);
  ...
  MPI_Scatter(vec2ptr(mat_sizes_sent), 1, MPI_INT, &mat_sizes_recv, 1, MPI_INT,
              sender_, mpi_comm_);

  received_entities.material_numbers.resize(mat_sizes_recv);
  LOG_DEBUG(3, "On proc " << rank() << ", " << mat_sizes_recv
  ...
  if (is_sender()) {
    for (int i = 0; i < num_procs; ++i) {
      sent_mat.insert(sent_mat.end(), sent_entities[i].material_numbers.begin(),
                      sent_entities[i].material_numbers.end());
      sdispl[i + 1] = sdispl[i] + sent_entities[i].material_numbers.size();
    }
  }

  MPI_Scatterv(vec2ptr(sent_mat), vec2ptr(mat_sizes_sent), vec2ptr(sdispl),
               MPI_INT, vec2ptr(received_entities.material_numbers),
               mat_sizes_recv, MPI_INT, sender_, mpi_comm_);
  mat_sizes_sent.clear();
  sent_mat.clear();

  // scatter number of received entities for each proc
  LOG_DEBUG(2, "Scatter align_number sizes");
  std::vector< int > al_sizes_sent(num_procs, 0);
  int al_sizes_recv = -1;

  if (is_sender()) {
    for (int i = 0; i < num_procs; ++i) {
      al_sizes_sent[i] = sent_entities[i].align_numbers.size();
    }
  }

  MPI_Scatter(vec2ptr(al_sizes_sent), 1, MPI_INT, &al_sizes_recv, 1, MPI_INT,
              sender_, mpi_comm_);

  received_entities.align_numbers.resize(al_sizes_recv);
  LOG_DEBUG(3, "On proc " << rank() << ", " << al_sizes_recv
                          << " align numbers will be received");

  LOG_DEBUG(2, "Scatter align_number vector");

  sdispl.clear();
  sdispl.resize(num_procs + 1, 0);
  std::vector< int > sent_al;
  if (is_sender()) {
    for (int i = 0; i < num_procs; ++i) {
      sent_al.insert(sent_al.end(), sent_entities[i].align_numbers.begin(),
                     sent_entities[i].align_numbers.end());
      sdispl[i + 1] = sdispl[i] + sent_entities[i].align_numbers.size();
    }
  }

  MPI_Scatterv(vec2ptr(sent_al), vec2ptr(al_sizes_sent), vec2ptr(sdispl),
               MPI_INT, vec2ptr(received_entities.align_numbers), al_sizes_recv,
               MPI_INT, sender_, mpi_comm_);
  al_sizes_sent.clear();
  sent_al.clear();
  ...
  MPI_Scatter(vec2ptr(conn_sizes_sent), 1, MPI_INT, &conn_sizes_recv, 1,
              MPI_INT, sender_, mpi_comm_);

  received_entities.connections.resize(conn_sizes_recv);
  LOG_DEBUG(3, "On proc " << rank() << ", " << conn_sizes_recv
                          << " connections will be received");
  ...
  if (is_sender()) {
    for (int i = 0; i < num_procs; ++i) {
      sent_conn.insert(sent_conn.end(), sent_entities[i].connections.begin(),
                       sent_entities[i].connections.end());
      sdispl[i + 1] = sdispl[i] + sent_entities[i].connections.size();
    }
  }

  MPI_Scatterv(vec2ptr(sent_conn), vec2ptr(conn_sizes_sent), vec2ptr(sdispl),
               MPI_INT, vec2ptr(received_entities.connections), conn_sizes_recv,
               MPI_INT, sender_, mpi_comm_);
...
bool MpiScatter::is_sender() const { return rank() == sender_; }
...
MpiNonBlockingPointToPoint::MpiNonBlockingPointToPoint(
    const MPI_Comm &communicator, int sender, int receiver)
    : sender_(sender), receiver_(receiver), comm_(communicator),
      is_communicating_(false) {}

void MpiNonBlockingPointToPoint::communicate(
    /*const*/ EntityPackage *sent_entities,
    EntityPackage *received_entities) const {
  // tags
  ...
  if (is_sender()) {
    ...
    MPI_Isend(&local_buf_[0], 5, MPI_INT, receiver_, LOCAL_BUF_TAG, comm_,
              &local_buf_request_);

    // send coords
    LOG_DEBUG(2, "sending coords from " << sender_ << " to " << receiver_);
    MPI_Isend(vec2ptr(sent_entities->coords), sent_entities->coords.size(),
              MPI_DOUBLE, receiver_, COORDS_TAG, comm_, &coords_request_);

    // send offsets
    LOG_DEBUG(2, "sending offsets from " << sender_ << " to " << receiver_);
    MPI_Isend(vec2ptr(sent_entities->offsets), sent_entities->offsets.size(),
    ...
    MPI_Isend(vec2ptr(sent_entities->connections),
              sent_entities->connections.size(), MPI_INT, receiver_,
              CONNECTIONS_TAG, comm_, &connections_request_);

    // send materials
    LOG_DEBUG(2, "sending material numbers from " << sender_ << " to "
                                                  << receiver_);
    MPI_Isend(vec2ptr(sent_entities->material_numbers),
              sent_entities->material_numbers.size(), MPI_INT, receiver_,
              MATERIALS_TAG, comm_, &materials_request_);

    // send aligns
    LOG_DEBUG(2,
              "sending align numbers from " << sender_ << " to " << receiver_);
    MPI_Isend(vec2ptr(sent_entities->align_numbers),
              sent_entities->align_numbers.size(), MPI_INT, receiver_,
              ALIGNS_TAG, comm_, &aligns_request_);
    ...
    LOG_DEBUG(2, "Receiving local buf from " << sender_ << " on " << receiver_
                                             << " (blocking)");
    ...
    received_entities->align_numbers.resize(num_entities);

    // Receive four vectors
    LOG_DEBUG(2, "Receiving coords from " << sender_ << " on " << receiver_);
    MPI_Irecv(vec2ptr(received_entities->coords),
              received_entities->coords.size(), MPI_DOUBLE, sender_, COORDS_TAG,
              comm_, &coords_request_);

    LOG_DEBUG(2, "Receiving offsets from " << sender_ << " on " << receiver_);
    MPI_Irecv(vec2ptr(received_entities->offsets),
              received_entities->offsets.size(), MPI_INT, sender_, OFFSETS_TAG,
              comm_, &offsets_request_);

    LOG_DEBUG(2,
              "Receiving connections from " << sender_ << " on " << receiver_);
    MPI_Irecv(vec2ptr(received_entities->connections),
              received_entities->connections.size(), MPI_INT, sender_,
              CONNECTIONS_TAG, comm_, &connections_request_);

    LOG_DEBUG(2, "Receiving materials from " << sender_ << " on " << receiver_);
    MPI_Irecv(vec2ptr(received_entities->material_numbers), num_entities,
              MPI_INT, sender_, MATERIALS_TAG, comm_, &materials_request_);

    LOG_DEBUG(2, "Receiving aligns from " << sender_ << " on " << receiver_);
    MPI_Irecv(vec2ptr(received_entities->align_numbers), num_entities, MPI_INT,
              sender_, ALIGNS_TAG, comm_, &aligns_request_);
  }
}
...
void MpiNonBlockingPointToPoint::wait() const {
  ...
  if (is_sender()) {
    ...
    MPI_Wait(&aligns_request_, &status_);
  } else {
...
bool MpiNonBlockingPointToPoint::is_sender() const { return rank() == sender_; }
