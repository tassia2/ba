...
#include "mesh/mpi_communication.h"
...
void print_pack(EntityPackage &pack) {
  ...
  for (size_t l = 0; l < pack.connections.size(); ++l) {
    std::cout << pack.connections[l] << " ";
  }
  std::cout << std::endl << " coords " << std::endl;
  for (size_t l = 0; l < pack.coords.size(); ++l) {
...
void pack_ghost_data(GhostCommData22 *ret, QuadData &quad_data,
  ...
  for (int jc = 0; jc < ret->num_cell; ++jc) {
    for (int jv = 0; jv < num_vert_cell; ++jv) {
      ret->connections[ret->conn_offsets[jc] + jv] =
          cell_packs[jc].connections[jv];
      for (int jd = 0; jd < gdim; ++jd) {
        ret->coords[ret->coord_offsets[jc] + jv * gdim + jd] =
            cell_packs[jc].coords[jv * gdim + jd];
      }
    }
    ...
    ret->align_numbers[jc] = cell_packs[jc].align_numbers[0];
  }
  for (int jc = 0; jc < ret->num_facet; ++jc) {
    for (int jv = 0; jv < num_vert_facet; ++jv) {
      ret->connections[ret->conn_offsets[ret->num_cell + jc] + jv] =
          facet_packs[jc].connections[jv];
      for (int jd = 0; jd < gdim; ++jd) {
        ret->coords[ret->coord_offsets[ret->num_cell + jc] + jv * gdim + jd] =
            facet_packs[jc].coords[jv * gdim + jd];
      }
    }
    ...
    ret->conn_offsets[ret->num_cell + jc + 1] =
        ret->conn_offsets[ret->num_cell + jc] + num_vert_facet;
    ret->coord_offsets[ret->num_cell + jc + 1] =
        ret->coord_offsets[ret->num_cell + jc] + num_vert_facet * gdim;
    ret->material_numbers[ret->num_cell + jc] =
        facet_packs[jc].material_numbers[0];
    ret->align_numbers[ret->num_cell + jc] = facet_packs[jc].align_numbers[0];
  }
}
...
void unpack_ghost_data(GhostCommData22 &ghost_data, QuadData &quad_data,
                       std::vector< EntityPackage > &cell_packs,
                       std::vector< EntityPackage > &facet_packs) {
  ...
  for (int jc = 0; jc < num_cell; ++jc) {
    ...
    cell_packs[jc].align_numbers.resize(1, 0);
    ...
    cell_packs[jc].align_numbers[0] = ghost_data.align_numbers[jc];

    for (int jv = 0; jv < num_vert_cell; ++jv) {
      cell_packs[jc].connections[jv] =
          ghost_data.connections[ghost_data.conn_offsets[jc] + jv];
      for (int jd = 0; jd < gdim; ++jd) {
        cell_packs[jc].coords[jv * gdim + jd] =
            ghost_data.coords[ghost_data.coord_offsets[jc] + jv * gdim + jd];
      }
    }
  }
  for (int jc = 0; jc < num_facet; ++jc) {
    ...
    facet_packs[jc].align_numbers.resize(1, 0);
    ...
    facet_packs[jc].material_numbers[0] =
        ghost_data.material_numbers[num_cell + jc];
    facet_packs[jc].align_numbers[0] = ghost_data.align_numbers[num_cell + jc];
    for (int jv = 0; jv < num_vert_facet; ++jv) {
      facet_packs[jc].connections[jv] =
          ghost_data.connections[ghost_data.conn_offsets[num_cell + jc] + jv];
      for (int jd = 0; jd < gdim; ++jd) {
        facet_packs[jc].coords[jv * gdim + jd] =
            ghost_data.coords[ghost_data.coord_offsets[num_cell + jc] +
                              jv * gdim + jd];
      }
    }
  }
...
void unpack_ghost_data(GhostCommData33 &ghost_data, QuadData &quad_data,
                       std::vector< EntityPackage > &cell_packs,
                       std::vector< EntityPackage > &facet_packs) {
  const int num_cell = ghost_data.num_cell;
  const int num_facet = ghost_data.num_facet;
...
void pXest_copy_ghost(p4est_ghost_t *input, p4est_ghost_t *&ghost) {
  ...
  ghost->mirror_proc_mirrors =
      new p4est_locidx_t[ghost->mirror_proc_offsets[mpisize]];
  ...
  ghost->mirror_proc_fronts =
      new p4est_locidx_t[ghost->mirror_proc_front_offsets[mpisize]];
  ...
  for (size_t i = 0; i < ghost->ghosts.elem_count; ++i) {
    QuadData *q_ptr =
        pXest_get_quad_data_ptr(p4est_quadrant_array_index(&input->ghosts, i));
  ...
  for (size_t i = 0; i < ghost->mirrors.elem_count; ++i) {
  ...
  std::cout << quad->p.piggy1.which_tree << " " << quad->p.piggy1.owner_rank <<
  std::endl; std::cout << quad->p.piggy3.which_tree << " " <<
  quad->p.piggy3.local_num << std::endl; std::cout << "
  ------------------------- " << std::endl;
   */
}
...
void pXest_copy_ghost(p8est_ghost_t *input, p8est_ghost_t *&ghost) {
  ...
  for (size_t i = 0; i < ghost->ghosts.elem_count; ++i) {
    QuadData *q_ptr =
        pXest_get_quad_data_ptr(p8est_quadrant_array_index(&input->ghosts, i));
...
void pXest_build_conn(int num_vertices, int num_cells,
                      const std::vector< double > &vertices,
                      const std::vector< int > &tree_to_vertices,
                      p4est_connectivity_t *&conn) {
  LOG_DEBUG(1, "num_vertices: " << num_vertices << " num_cells: " << num_cells
                                << " vertices.size(): " << vertices.size()
                                << " tree_to_vertices.size(): "
                                << tree_to_vertices.size());
  ...
  if (DEBUG_LEVEL > 0) {
    int min_id = 1e8;
    int max_id = -1;
    for (size_t l = 0; l < tree_to_vertices.size(); ++l) {
    ...
    LOG_DEBUG(1, "Max(tree_to_vertices) = "
                     << max_id << " Min(tree_to_vertices) = " << min_id);
  }
  ...
  std::copy(tree_to_vertices.begin(), tree_to_vertices.end(),
            conn->tree_to_vertex);
  ...
  for (int tree = 0; tree < conn->num_trees; ++tree) {
    for (int face = 0; face < P4EST_FACES; ++face) {
      ...
      LOG_DEBUG(3, " tree " << tree << " , face " << face << " , index1 "
                            << P4EST_FACES * tree + face << " index2 "
                            << P4EST_FACES * tree + face);
    }
  }
...
void pXest_build_conn(int num_vertices, int num_cells,
  ...
  for (int tree = 0; tree < conn->num_trees; ++tree) {
    for (int face = 0; face < P8EST_FACES; ++face) {
      ...
      LOG_DEBUG(3, " tree " << tree << " , face " << face << " , index1 "
                            << P8EST_FACES * tree + face << " index2 "
                            << P8EST_FACES * tree + face);
    }
  }
...
static int reorder_comp(const void *a, const void *b) {
  const int *A = static_cast< const int * >(a);
  const int *B = static_cast< const int * >(b);
...
void pXest_reorder_conn(const std::vector< int > &part,
                        p4est_connectivity_t *conn,
   ...
   * for itself*/
  newid = sc_array_new_size(sizeof(size_t), n);
  sorter = sc_array_new_size(2 * sizeof(int), n);
  for (i = 0; i < n; i++) {
    ip = static_cast< int * >(sc_array_index(sorter, i));
    ip[0] = part[i];
    ip[1] = i;
  }
  ...
  for (i = 0; i < n; i++) {
    ip = static_cast< int * >(sc_array_index(sorter, i));
    zp = static_cast< size_t * >(sc_array_index(newid, ip[1]));
    *zp = i;
...
p4est_gloidx_t
pXest_partition_forest(p4est_t *forest, int partition_for_coarsening,
                       std::vector< p4est_locidx_t > &num_quads_per_proc) {
  ...
  P4EST_GLOBAL_PRODUCTIONF(
      "Into " P4EST_STRING "_partition with %lld total quadrants\n",
      static_cast< long long >(forest->global_num_quadrants));
  ...
  // FEATURE_TODO: remove comments, problem: p4est_partition_for_coarsening is
  // not contained in p4est.h, although it is defined in p4est.c. Either, one
  // has modify p4est.h accordingly, or, one has to include
  // p4est_partition_for_coarsening in hiflow
  /* correct partition */
  /*
              if (partition_for_coarsening)
              {
                  p4est_gloidx_t num_corrected = p4est_partition_for_coarsening
     (forest, &num_quads_per_proc[0]); P4EST_GLOBAL_INFOF ("Designated partition
     for coarsening %lld quadrants moved\n", (long long) num_corrected);
              }
   */
  ...
  P4EST_GLOBAL_PRODUCTIONF("Done " P4EST_STRING
                           "_partition shipped %lld quadrants %.3g%%\n",
                           static_cast< long long >(global_shipped),
...
p4est_gloidx_t
pXest_partition_forest(p8est_t *forest, int partition_for_coarsening,
                       std::vector< p4est_locidx_t > &num_quads_per_proc) {
  p4est_gloidx_t global_shipped = 0;
  const p4est_gloidx_t global_num_quadrants = forest->global_num_quadrants;
  ...
  // FEATURE_TODO: remove comments, problem: p8est_partition_for_coarsening is
  // not contained in p8est.h, although it is defined in p8est.c. Either, one
  // has modify p8est.h accordingly, or, one has to include
  // p8est_partition_for_coarsening in hiflow
  /* correct partition */
  /*
              if (partition_for_coarsening)
              {
                  p4est_gloidx_t num_corrected = p8est_partition_for_coarsening
     (forest, &num_quads_per_proc[0]); P4EST_GLOBAL_INFOF ("Designated partition
     for coarsening %lld quadrants moved\n", (long long) num_corrected);
              }
   */
...
std::vector< int > pXest_get_partition_from_initial_forest(p4est_t *forest) {
  ...
  MPI_Allgather(&first_local_tree, 1, MPI_INT, vec2ptr(first_remote_tree), 1,
                MPI_INT, forest->mpicomm);
  MPI_Allgather(&last_local_tree, 1, MPI_INT, vec2ptr(last_remote_tree), 1,
                MPI_INT, forest->mpicomm);
...
// PERF_TODO check efficiency, maybe use MPI_PACK, check whether deadlock can
// occure -> use nonblocking communication modes

void pXest_distribute_coarse_maps(MPI_Comm comm, int master_rank,
                                  std::vector< int > &partitioning,
                                  std::vector< int > &coarse_cell_ids,
                                  std::vector< int > &local_cell_ids,
                                  EntityToQuadMap *coarse_map,
                                  EntityToQuadMap &map) {
  int rank;
  int num_ranks;
  ...
  for (std::vector< int >::iterator it = partitioning.begin();
  ...
  if (rank == master_rank) {
    ...
    for (EntityNumber jc = 0; jc < num_cells; ++jc) {
      ...
      if (proc != master_rank) {
        LOG_DEBUG(3, " send coarse id " << coarse_id << " to proc " << proc
                                        << " with base tag " << jc * size_map);
        ...
        LOG_DEBUG(3, " keep id " << coarse_id << " as local id "
                                 << local_cell_ids[jc]);
    ...
    for (EntityNumber jc = 0; jc < num_cells; ++jc) {
      ...
      if (proc == rank) {
        ...
        LOG_DEBUG(3, " proc " << rank << " receives data from " << master_rank
                              << " with base tag " << tag_id * size_map);

        std::vector< MPI_Status > status(size_map);
        MPI_Recv(&coarse_id, 1, MPI_INT32_T, master_rank, tag_id * size_map + 0,
                 comm, &status[0]);
        MPI_Recv(&coord.tree, 1, MPI_INT32_T, master_rank,
                 tag_id * size_map + 1, comm, &status[1]);
        MPI_Recv(&coord.level, 1, MPI_INT, master_rank, tag_id * size_map + 2,
                 comm, &status[2]);
        MPI_Recv(&coord.x, 1, MPI_INT32_T, master_rank, tag_id * size_map + 3,
                 comm, &status[3]);
        MPI_Recv(&coord.y, 1, MPI_INT32_T, master_rank, tag_id * size_map + 4,
                 comm, &status[4]);
        MPI_Recv(&coord.z, 1, MPI_INT32_T, master_rank, tag_id * size_map + 5,
                 comm, &status[5]);
        MPI_Recv(&coord.localId, 1, MPI_INT, master_rank, tag_id * size_map + 6,
                 comm, &status[6]);
        MPI_Recv(&coord.tdim, 1, MPI_INT, master_rank, tag_id * size_map + 7,
                 comm, &status[7]);
        MPI_Recv(&coord.coarseId, 1, MPI_INT, master_rank,
                 tag_id * size_map + 8, comm, &status[8]);

        LOG_DEBUG(3, " proc " << rank << " received coarse id " << coarse_id
                              << " which is pushed back as local id "
                              << local_cell_ids[jc]);
...
void pXest_extract_local_maps(MPI_Comm comm, int master_rank,
  ...
  size_t num_cells = partitioning.size();
  map.clear();

  // Loop over all cell indices in coarse mesh
  for (size_t jc = 0; jc < num_cells; ++jc) {
    int proc = partitioning[jc];
    Id coarse_id = coarse_cell_ids[jc];
    QuadCoord coord = (*coarse_map)[coarse_id];
    ...
    LOG_DEBUG(3, " considered id " << coarse_id);
    if (proc == rank) {
      LOG_DEBUG(3, " extract id " << coarse_id << " as local id "
                                  << local_cell_ids[jc]);
      assert(local_cell_ids[jc] >= 0);
      map.insert(std::pair< Id, QuadCoord >(local_cell_ids[jc], coord));
    }
...
int64_t pXest_get_quad_pos_in_tree_array(p4est_tree_t *tree,
                                         p4est_quadrant_t *quad) {
  // binary search
  int64_t pos =
      sc_array_bsearch(&tree->quadrants, quad, pXest_compare_quad4_in_tree);
  return pos;
}

int64_t pXest_get_quad_pos_in_tree_array(p8est_tree_t *tree,
                                         p8est_quadrant_t *quad) {
  // binary search
  int64_t pos =
      sc_array_bsearch(&tree->quadrants, quad, pXest_compare_quad8_in_tree);
  return pos;
}

int64_t pXest_get_quad_pos_in_ghost_array(p4est_ghost_t *ghost,
                                          p4est_quadrant_t *quad,
                                          treeId tree_id) {
...
int64_t pXest_get_quad_pos_in_ghost_array(p8est_ghost_t *ghost,
                                          p8est_quadrant_t *quad,
                                          treeId tree_id) {
  // binary search for quadrant in ghost layer
...
// PERF_TODO make more efficient by using range of possible mortonIds (computed
// from input quad) and binary search PERF_TODO: BOTTLENECK

std::vector< int64_t >
pXest_get_leaf_desc_in_tree_array(p4est_tree_t *tree, p4est_quadrant_t *quad) {
  ...
  for (int jq = 0; jq < num_quads; ++jq) {
    ...
    if (p4est_quadrant_is_ancestor(quad, cur_quad) != 0) {
...
std::vector< int64_t >
pXest_get_leaf_desc_in_tree_array(p8est_tree_t *tree, p8est_quadrant_t *quad) {
  std::vector< int64_t > leaf_pos;
  const int num_quads = tree->quadrants.elem_count;
  for (int jq = 0; jq < num_quads; ++jq) {
    p8est_quadrant_t *cur_quad = pXest_get_local_quad_in_tree(tree, jq);
    if (p8est_quadrant_is_ancestor(quad, cur_quad) != 0) {
      leaf_pos.push_back(jq);
    }
  }
  return leaf_pos;
...
std::vector< int64_t >
pXest_get_leaf_desc_in_ghost_array(p4est_ghost_t *ghost, p4est_quadrant_t *quad,
                                   treeId tree_id) {
  std::vector< int64_t > leaf_pos;
  const size_t num_quads = ghost->ghosts.elem_count;
  for (size_t jq = 0; jq < num_quads; ++jq) {
    p4est_quadrant_t *cur_quad = p4est_quadrant_array_index(&ghost->ghosts, jq);
    LOG_DEBUG(3,
              " QuadData " << pXest_get_quad_data_ptr(cur_quad)->get_tree_id());

    if (pXest_get_quad_data_ptr(cur_quad)->get_tree_id() == tree_id &&
        (p4est_quadrant_is_ancestor(quad, cur_quad) != 0)) {
      leaf_pos.push_back(jq);
    }
  }
  return leaf_pos;
...
std::vector< int64_t >
pXest_get_leaf_desc_in_ghost_array(p8est_ghost_t *ghost, p8est_quadrant_t *quad,
                                   treeId tree_id) {
  std::vector< int64_t > leaf_pos;
  const size_t num_quads = ghost->ghosts.elem_count;
  for (size_t jq = 0; jq < num_quads; ++jq) {
    ...
    if (pXest_get_quad_data_ptr(cur_quad)->get_tree_id() == tree_id &&
        (p8est_quadrant_is_ancestor(quad, cur_quad) != 0)) {
      leaf_pos.push_back(jq);
    }
  }
  return leaf_pos;
...
QuadData *pXest_get_quad_data_ptr(p4est_ghost_t *ghost, p4est_quadrant_t *quad,
                                  treeId tree_id) {
...
QuadData *pXest_get_quad_data_ptr(p8est_ghost_t *ghost, p8est_quadrant_t *quad,
                                  treeId tree_id) {
  const int64_t pos = pXest_get_quad_pos_in_ghost_array(ghost, quad, tree_id);
  if (pos >= 0) {
...
void pXest_build_ref_tree(std::vector< p4est_quadrant_t > &quads,
                          RefinementTree *tree) {
  size_t num_quads = quads.size();
  p4est_quadrant_t ancestor = quads[0];

  LOG_DEBUG(2, " Build RefTree with " << num_quads << " quadrants ");
  LOG_DEBUG(2, " Root cell is on level " << static_cast< int >(ancestor.level));
  ...
  for (size_t jq = 0; jq < num_quads; ++jq) {
    // level index
    size_t diff_level = static_cast< size_t >(quads[jq].level - ancestor.level);
    ...
    int family = 0;
    for (size_t jl = 1; jl < diff_level; ++jl) {
      // maximum number of descendant families on level quad[jq].level a quad on
      // level ancestor.level+jl can have
      int max_num_desc_family = static_cast< int >(
          std::pow(static_cast< double >(4), diff_level - jl - 1));
      int ancestor_number =
          p4est_quadrant_ancestor_id(&quads[jq], ancestor.level + jl);
      LOG_DEBUG(2, "Intermediate level "
                       << jl << " max_num_desc_families " << max_num_desc_family
                       << " ancestor_number " << ancestor_number);

      family += ancestor_number * max_num_desc_family;
    }

    LOG_DEBUG(2, "Current quad " << jq << " is on level "
                                 << static_cast< int >(quads[jq].level)
                                 << " and has family index " << family
                                 << " and x " << quads[jq].x << " and y "
                                 << quads[jq].y);

    // put quadrant number into map
    std::map< int, std::vector< int > >::iterator it =
        quads_index[diff_level].find(family);
    if (it == quads_index[diff_level].end()) {
      ...
      if (jq > 0) {
        ...
        quads_index[diff_level].insert(
            std::pair< int, std::vector< int > >(family, tmp));
      } else {
      ...
      it->second.at(
          pXest_z_order_to_ccw_order(pXest_get_sibling_nr(&quads[jq]))) = jq;
    }
  }
  size_t num_levels = quads_index.size();

  if (DEBUG_LEVEL >= 2) {
    for (size_t jl = 0; jl < num_levels; ++jl) {
      std::cout << " Level: " << jl << std::endl;
      for (std::map< int, std::vector< int > >::iterator jf =
               quads_index[jl].begin();
           jf != quads_index[jl].end(); ++jf) {
  ...
  // Loop over levels
  for (size_t jl = 1; jl < num_levels; ++jl) {
    ...
    for (std::map< int, std::vector< int > >::iterator jf =
      ...
      LOG_DEBUG(2, "   Family: "
                       << jf->first << " consists of quadrant numbers "
                       << quadrant_numbers[0] << " " << quadrant_numbers[1]
                       << " " << quadrant_numbers[2] << " "
                       << quadrant_numbers[3]);

      assert(!quadrant_numbers.empty());

      int parent_family = jf->first / 4;
      int parent_sibling = jf->first - parent_family * 4;
      int parent_quadrant_number =
          quads_index[jl - 1][parent_family]
                     [pXest_z_order_to_ccw_order(parent_sibling)];
      ...
      LOG_DEBUG(2, "   Family: "
                       << jf->first << " has parent family: " << parent_family
                       << ", parent sibling: " << parent_sibling
                       << ", parent_quadrant_number: " << parent_quadrant_number
      ...
      for (size_t jc = 0; jc < quadrant_numbers.size(); ++jc) {
        int cell_index = num_cells + jc;
        quadrant_to_cell_index.insert(
            std::pair< int, int >(quadrant_numbers[jc], cell_index));
      }
    }
  }
}

void pXest_build_ref_tree(std::vector< p8est_quadrant_t > &quads,
                          RefinementTree *tree) {
  size_t num_quads = quads.size();
  ...
  for (size_t jq = 0; jq < num_quads; ++jq) {
    ...
    for (size_t jl = 1; jl < diff_level; ++jl) {
      ...
      // level ancestor.level+jl can have
      int max_num_desc_family = static_cast< int >(
          std::pow(static_cast< double >(8), diff_level - jl - 1));
      int ancestor_number =
          p8est_quadrant_ancestor_id(&quads[jq], ancestor.level + jl);
      family += ancestor_number * max_num_desc_family;
    }
    ...
    std::map< int, std::vector< int > >::iterator it =
        quads_index[diff_level].find(family);
    if (it == quads_index[diff_level].end()) { // First occurence of family
      if (jq > 0) {
    ...
    } else { // Repeated occurence of family
      it->second.at(
          pXest_z_order_to_ccw_order(pXest_get_sibling_nr(&quads[jq]))) = jq;
    }
  }
  ...
  for (size_t jl = 1; jl < num_levels; ++jl) {
    ...
    for (std::map< int, std::vector< int > >::iterator jf =
      ...
      LOG_DEBUG(2, "   Family: " << jf->first
                                 << " consists of quadrant numbers "
                                 << string_from_range(quadrant_numbers.begin(),
                                                      quadrant_numbers.end()));
...
int pXest_get_db_desc_nr(p4est_quadrant_t *desc, p4est_quadrant_t *ancestor) {
  ...
  for (int r = ancestor->level + 1; r <= desc->level; ++r) {
    int co_num_cells_on_level = static_cast< int >(
        std::pow(static_cast< double >(4), diff_level - (r - ancestor->level)));
    int sibling_nr_on_level =
        pXest_z_order_to_ccw_order(p4est_quadrant_ancestor_id(desc, r));
...
int pXest_get_db_desc_nr(p8est_quadrant_t *desc, p8est_quadrant_t *ancestor) {
  ...
  for (int r = ancestor->level + 1; r <= desc->level; ++r) {
    int co_num_cells_on_level = static_cast< int >(
        std::pow(static_cast< double >(8), diff_level - (r - ancestor->level)));
    int sibling_nr_on_level =
        pXest_z_order_to_ccw_order(p8est_quadrant_ancestor_id(desc, r));
...
void pXest_init_fn(p4est_t *forest, p4est_topidx_t tree,
                   p4est_quadrant_t *quad) {
  // get pointer to external user data
  ForestInitData *i_ptr = static_cast< ForestInitData * >(forest->user_pointer);
...
void pXest_init_fn(p8est_t *forest, p4est_topidx_t tree,
                   p8est_quadrant_t *quad) {
  // get pointer to external user data
  ForestInitData *i_ptr = static_cast< ForestInitData * >(forest->user_pointer);
