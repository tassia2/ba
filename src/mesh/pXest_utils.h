...
/// \brief function that builds a p4est connectivity out of read in data
/// structures
...
/// \brief reoder connectivity struct such that forest is distributed according
/// to given partition
...
/// \brief extract mesh partition from refined forest -> consider childless
/// quadrants (leafs) as current mesh
/// @param[in] forest pointer to forest object
/// @param[in] master_rank
/// @return map: coarse cell id -> MPI rank
...
/// @param[in] coarse_maps vector of coarse_entity_to_quad maps, must be
/// nonempty iff on master process
/// @param[out] maps distributed entity_to_quad maps with adjusted id (now local
/// id instead of global coarse id)
...
/// \brief creates a quadrant object out of coordinates describing its positin
/// in the forest
...
/// \brief compute the position in quadrant array of tree object for some given
/// quad
...
/// \brief compute the position in quadrant array of ghost object for some given
/// quad
...
/// \brief  compute positions in quadrant array of some tree for those quadrants
...
/// \brief compute positions in quadrant array of ghost for those quadrants that
...
/// \brief get QuadData in p4est quadrant. Note: not all quadrants contain user
/// data
/// @param[in] quad pointer to some quadrant
/// @param[in] data reference to user quadrant data struct
...
/// \brief get pointer to data of quadrant in given tree. Here, quadrant denotes
/// an empty quadrant, i.e. without user data
/// @param[in] tree pointer to tree object
...
/// \brief get pointer to data of quadrant in given ghost layer. Here, quadrant
/// denotes an empty quadrant, i.e. without user data
/// @param[in] ghost pointer to ghost object
/// @param[in] quad pointer to some (empty) quadrant
...
/// \brief build a RefinementTree object whose structure matches the
/// relationship of the given quadrants
...
/// @param[out] tree initialized RefinementTree. sub_cell_numbers in tree
/// correspond to array indices+1
...
/// \brief get sibling id of quad, i.e. the position in the z curve within a
/// family created by refining one single cell
/// @param[in] quad pointer to some quadrant
...
/// \brief convert local facet number from p4est numbering to mesh database
/// numbering
/// @param[in] p4est number of facet
/// @return MeshDBView number of facet
int pXest_facet_number_pXest_to_db(int facet);

/// \brief convert local facet number from mesh database numbering to p4est
/// numbering
/// @param[in] MeshDBView number of facet
/// @return p4est number of facet
int pXest_facet_number_db_to_pXest(int facet);

/// \brief function which is called for each quadrant when creating a new p4est
/// forest object
/// @param[in] forest pointer to underlying forest object
...
int pXest_weight_fn(p4est_t *forest, p4est_topidx_t tree,
                    p4est_quadrant_t *quad);
int pXest_weight_fn(p8est_t *forest, p4est_topidx_t tree,
                    p8est_quadrant_t *quad);
...
inline bool pXest_quad_is_ancestor(p4est_quadrant_t *test,
                                   p4est_quadrant_t *desc) {
  return p4est_quadrant_is_ancestor(test, desc);
}

inline bool pXest_quad_is_ancestor(p8est_quadrant_t *test,
...
inline p4est_quadrant_t *pXest_get_local_quad_in_tree(p4est_tree_t *tree,
                                                      int64_t pos) {
  return p4est_quadrant_array_index(&tree->quadrants, pos);
}

inline p8est_quadrant_t *pXest_get_local_quad_in_tree(p8est_tree_t *tree,
                                                      int64_t pos) {
...
inline p4est_quadrant_t *pXest_get_local_quad_in_ghost(p4est_ghost_t *ghost,
                                                       int64_t pos) {
  return p4est_quadrant_array_index(&ghost->ghosts, pos);
}

inline p8est_quadrant_t *pXest_get_local_quad_in_ghost(p8est_ghost_t *ghost,
                                                       int64_t pos) {
...
/// \brief compute morton index of some quadrant w.r.t. the current level of the
/// quadrant
/// @param[in] quad pointer to some quadrant
/// @return morton index
...
/// \brief get Morton Index of first and last local quadrant on tree for given
/// level of refinement
/// @param[in] tree pointer to tree object
/// @param[in] level level of refinement
/// @return morton index

inline mortonId pXest_get_first_mortonId_in_tree(p4est_tree_t *tree,
                                                 int level) {
  return p4est_quadrant_linear_id(&tree->first_desc, level);
}

inline mortonId pXest_get_first_mortonId_in_tree(p8est_tree_t *tree,
                                                 int level) {
