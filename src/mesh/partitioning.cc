        ...
        for (IncidentEntityIterator iiit = mesh.begin_incident(*iit, tdim),
          ...
          if (it->index() != iiit->index()) {
            ...
            if (f_it == adjacency_map[it->index()].end()) {
              adjacency_map[it->index()][iiit->index()] =
...
void compute_dual_graph_facet(const Mesh &mesh, Graph *graph) {
  ...
  for (EntityIterator it = mesh.begin(tdim); it != end; ++it) {
    ...
    for (IncidentEntityIterator facet_it = mesh.begin_incident(*it, tdim - 1);
      ...
      for (IncidentEntityIterator neighbor_cell_it =
               mesh.begin_incident(*facet_it, tdim);
           neighbor_cell_it != mesh.end_incident(*facet_it, tdim);
           ++neighbor_cell_it) {
...
/// \param out Graph representing the local part which has been distributed to
/// this process. The offsets are adjusted to local meaning, the adjacencies
/// keep global meaning. The output graph can be used as local argument for
/// ParMetisGraphPartitioner.

void distribute_graph_naive(const Graph &in, const int master_rank,
                            const MPI_Comm &comm, Graph &out) {
  int rk = -1, sz = -2;
  ...
  std::copy(off_vec.begin() + my_off, off_vec.begin() + next_off + 1,
  ...
  std::copy(adj_vec.begin() + off_vec[my_off],
            adj_vec.begin() + off_vec[next_off], out.adjacent_nodes.begin());

  // copy my part of the adjacency weights
  std::copy(adj_wgts.begin() + off_vec[my_off],
            adj_wgts.begin() + off_vec[next_off], out.adjacent_weights.begin());
}
...
void compute_facet_from_cell_distribution(
    const Mesh &mesh, const CellDistribution &cell_distribution,
  ...
  for (int p = 0; p < num_partitions; ++p) {
    ...
    assert(cell_distribution.cell_indices.size() >=
    ...
    for (int c = offset; c < offset + cell_distribution.num_cells[p]; ++c) {
      ...
      // add facet indices of the current cell to the temporal facet
      // distribution
      for (IncidentEntityIterator it = cell.begin_incident(tdim - 1);
           it != cell.end_incident(tdim - 1); ++it) {
    ...
    std::vector< int >::iterator last_unique =
        std::unique(temp_facet_list.begin(), temp_facet_list.end());
    temp_facet_list.resize(last_unique - temp_facet_list.begin());
    facet_distribution->num_cells[p] = temp_facet_list.size();
    facet_distribution->cell_indices.insert(
        facet_distribution->cell_indices.end(), temp_facet_list.begin(),
        temp_facet_list.end());
  }
  assert(facet_distribution->cell_indices.size() ==
         std::accumulate(facet_distribution->num_cells.begin(),
                         facet_distribution->num_cells.end(), 0));
}

//////// NaiveGraphPartitioner ////////////////

void NaiveGraphPartitioner::partition_graph(
    const Graph &graph, const int master_rank,
    const std::vector< int > *nodal_weights,
    const std::vector< int > *edge_weights, const MPI_Comm &comm,
...
void MetisGraphPartitioner::partition_graph(
    const Graph &graph, const int master_rank,
    const std::vector< int > *nodal_weights,
    const std::vector< int > *edge_weights, const MPI_Comm &comm,
    std::vector< int > *partitioning) const {
  ...
  options[METIS_OPTION_MINCONN] =
      1; // minimize maximum connectivity of partitions
  ...
  METIS_PartGraphKway(&num_nodes, &num_constraints, xadj, adjncy, vwgt, vsize,
                      adjwgt, &nparts, tpwgts, ubvec, options, &num_edge_cuts,
                      vec2ptr(*partitioning));
#else

  METIS_PartGraphKway(&num_nodes, xadj, adjncy, vwgt, adjwgt, &wgtflag,
                      &numflag, &nparts, options, &num_edge_cuts,
                      vec2ptr(*partitioning));
#endif

  LOG_INFO("MetisGraphPartition", "partitioning done");
}
#endif // WITH_METIS

#ifdef WITH_PARMETIS

void ParMetisGraphPartitioner::partition_graph(
    const Graph &graph, const int master_rank,
    const std::vector< int > *nodal_weights,
    const std::vector< int > *edge_weights, const MPI_Comm &comm,
    std::vector< int > *partitioning) const {
  ...
  float ubvec = 1.05f; // max load imbalance

  int *my_part = new int[vtxdist[rk + 1] - vtxdist[rk]];
  MPI_Comm comm_copy = comm;
  int options[3] = {0, 0, 0};

  int metis_return = ParMETIS_V3_PartKway(
      vec2ptr(vtxdist), vec2ptr(distributed_graph.node_offsets),
      vec2ptr(distributed_graph.adjacent_nodes), vwgt, adjwgt, &wgtflag,
      &numflag, &ncon, &nparts, tpwgts, &ubvec, options, &num_edge_cuts,
      my_part, &comm_copy);
  ...
  MPI_Gatherv(my_part, vtxdist[rk + 1] - vtxdist[rk], MPI_INT,
              vec2ptr(*partitioning), vec2ptr(recv_cts), vec2ptr(vtxdist),
...
void ParMetisMeshPartitioner::partition_mesh(
