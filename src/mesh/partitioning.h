...
///
/// \details Stores a sparse graph G = (V,E) of n nodes, where the nodes are
/// implicitly numbered V = {0,1,...,n-1}. The nodes connected to node i (0 <= i
/// < n) are stored in the subarray
/// adjacent_nodes[node_offsets[i]],...,adjacent_nodes[node_offsets[i+1]-1]

struct Graph {

  int num_nodes() const { return node_offsets.size() - 1; }
...
/// \brief Naively distributes a graph from master proc to all procs in the
...
public:
  ...
  virtual void partition_graph(const Graph &graph, const int master_rank,
...
public:
  ...
  virtual bool is_collective() const { return false; }
};
...
/// \brief Parallel partitioning using the ParMETIS library. This is a
/// collective routine w.r.t. the given MPI communicator.
...
public:
  ...
  virtual bool is_collective() const { return true; }
};
