...
RefinedMeshDbView::RefinedMeshDbView(
    TDim tdim, GDim gdim, MeshDatabasePtr db, const std::vector< Id > &cells,
  ...
  parent_history_index_attr_ =
      AttributePtr(new IntAttribute(sorted_history_indices));
  add_attribute(std::string("__parent_history_index__"), tdim,
                parent_history_index_attr_);
  parent_cell_index_attr_ =
      AttributePtr(new IntAttribute(sorted_parent_cell_indices));
  add_attribute(std::string("__parent_cell_index__"), tdim,
                parent_cell_index_attr_);
  sub_cell_number_attr_ =
      AttributePtr(new IntAttribute(sorted_sub_cell_numbers));
  add_attribute(std::string("__sub_cell_number__"), tdim,
                sub_cell_number_attr_);
                
  this->mesh_impl_ = IMPL_REFINED;
}

RefinedMeshDbView::RefinedMeshDbView(TDim tdim, GDim gdim,
                                     std::vector< MasterSlave > period,
                                     bool is_rectangular)
    : MeshDbView(tdim, gdim, period, is_rectangular),
      history_(std::vector< ConstMeshPtr >(0)) 
      {
        this->mesh_impl_ = IMPL_REFINED;
      }

/// \return Parent cell id of cell with index cell_index, or -1 if the cell has
/// no parent

Id RefinedMeshDbView::get_parent_cell_id(EntityNumber cell_index) const {
  const int parent_history_index =
      parent_history_index_attr_->get_int_value(cell_index);
  if (parent_history_index >= 0) {
    const EntityNumber parent_cell_index =
        parent_cell_index_attr_->get_int_value(cell_index);
...
std::pair< ConstMeshPtr, EntityNumber >
RefinedMeshDbView::get_cell_parent(EntityNumber cell_index) const {
  const int parent_history_index =
      parent_history_index_attr_->get_int_value(cell_index);
  if (parent_history_index >= 0) {
    const ConstMeshPtr parent_mesh = get_ancestor_mesh(parent_history_index);
...
MeshPtr RefinedMeshDbView::refine(std::vector< int > &refinements) const {
  ...
  UMAP_NAMESPACE::unordered_map< EntityNumber, std::pair< int, EntityNumber > >
      coarsening_data;
  for (int c = 0; c < num_cells; ++c) {
    if (refinements[c] < 0) {
      int parent_history_index;
      EntityNumber parent_cell_index;
      if (find_coarsenable_parent(c, forbidden_ancestors, &parent_history_index,
                                  &parent_cell_index)) {
        coarsening_data[c] =
            std::make_pair(parent_history_index, parent_cell_index);
      } else {
        // do not refine cell
        refinements[c] = 0;
      }
  ...
  // Loop over cells and refine, coarsen, or keep according to refinements
  // marker
  for (int c = 0; c < num_cells; ++c) {
    ...
    if (r > 0) { // refinement (subdivision)
      ...
      if (!visited_coarsened_parents.find_insert(parent_id)) {
        ...
        if (parent_data.first > 0) {
          ...
          // get pointer to ancestor mesh (it is a RefinedMeshDbView* since it
          // is not the root mesh).
          const RefinedMeshDbView *ancestor_mesh =
              static_cast< const RefinedMeshDbView * >(
                  get_ancestor_mesh(parent_data.first).get());

          // get parent_history_index from attribute
          const int parent_history_index =
              ancestor_mesh->parent_history_index_attr_->get_int_value(
                  parent_data.second);
          parent_history_indices.push_back(parent_history_index);

          // get parent_cell_index from attribute
          const int parent_cell_index =
              ancestor_mesh->parent_cell_index_attr_->get_int_value(
                  parent_data.second);
          parent_cell_indices.push_back(parent_cell_index);

          // get sub_cell_number from attribute
          const int sub_cell_number =
              ancestor_mesh->sub_cell_number_attr_->get_int_value(
                  parent_data.second);
  ...
  MeshPtr refined_mesh = MeshPtr(new RefinedMeshDbView(
      td, gd, get_db(), new_cells, refined_history, parent_history_indices,
      parent_cell_indices, sub_cell_numbers, period_));

  return refined_mesh;
}

void RefinedMeshDbView::mark_forbidden_ancestor_cells(
    EntityNumber cell_index, SortedArray< Id > &ancestors) const {
  const TDim td = tdim();
  ...
  while (true) {
    int parent_history_index =
        mesh_ptr->parent_history_index_attr_->get_int_value(cell_index);
    if (parent_history_index >= 0) {
      ConstMeshPtr parent_mesh = get_ancestor_mesh(parent_history_index);
      const int parent_cell_index =
          mesh_ptr->parent_cell_index_attr_->get_int_value(cell_index);
...
bool RefinedMeshDbView::find_coarsenable_parent(
    EntityNumber cell_index, const SortedArray< Id > &forbidden_ancestors,
    int *parent_history_index, EntityNumber *parent_cell_index) const {
...
bool RefinedMeshDbView::is_boundary_facet(EntityNumber facet_index) const {
  ...
  if (incident_cells.size() == 1) {
    ...
    std::pair< ConstMeshPtr, EntityNumber > parent_mesh_index =
        get_cell_parent(cell_entity.index());
    ...
    if (parent_cell_index < 0) {
      ...
      const Entity parent_cell_entity =
          parent_mesh->get_entity(tdim(), parent_cell_index);
      const CellType &parent_cell_type = parent_cell_entity.cell_type();

      // get local vertices of cell
      const std::vector< int > cell_vertices(cell_entity.begin_vertex_ids(),
                                             cell_entity.end_vertex_ids());
      LOG_DEBUG(3, "Cell vertices: " << string_from_range(cell_vertices.begin(),
                                                          cell_vertices.end()));

      const CellType &cell_type = cell_entity.cell_type();

      // get facet vertices
      const std::vector< int > facet_vertices(facet_entity.begin_vertex_ids(),
                                              facet_entity.end_vertex_ids());
      LOG_DEBUG(3, "Facet vertices: " << string_from_range(
      ...
      // Get local facet number in child cell type by linear search, comparing
      // the vertex sets.
      ...
      LOG_DEBUG(3,
                "Facet number in child cell type: " << facet_number_in_child);

      // get facet number in parent cell type
      const int facet_number_in_parent = parent_cell_type.sub_entities_of_cell(
          facet_tdim, sub_cell_number)[facet_number_in_child];
      LOG_DEBUG(3,
                "Facet number in parent cell type: " << facet_number_in_parent);
      assert(facet_number_in_parent >= 0);

      // If child facet is regular, then we have already found the facet number
      // to use.
      EntityNumber parent_facet_number = facet_number_in_parent;
      if (parent_facet_number >=
          parent_cell_type.num_regular_entities(facet_tdim)) {
        // Else, if facet is an irregular facet in the parent, we have to get
        // its regular parent
        parent_facet_number =
            parent_cell_type.regular_parent(facet_tdim, facet_number_in_parent);
        LOG_DEBUG(3, "Parent facet number: " << parent_facet_number);
        assert(parent_facet_number == -1 ||
               parent_facet_number <
                   parent_cell_type.num_regular_entities(facet_tdim));
      }
      ...
      if (parent_facet_number != -1) {
        IncidentEntityIterator parent_facet_it =
            parent_cell_entity.begin_incident(facet_tdim);
        ...
        // In this case, the facet is interior in the parent, and hence not on
        // the boundary.
        is_boundary = false;
      }
    }
