...
const CellType &RefinedCell::cell_type() const { return *cell_type_; }

bool RefinedCell::is_refined() const { return num_children() != 0; }

bool RefinedCell::is_root() const { return parent_ == -1; }

int RefinedCell::parent() const { return parent_; }
...
int RefinedCell::num_children() const { return children_.size(); }

int RefinedCell::sub_cell_number() const { return sub_cell_number_; }

int RefinedCell::quadrant_number() const { return quadrant_number_; }
...
RefinedCell::RefinedCell(const CellType &cell_type,
                         const std::vector< int > &vertex_numbers, int parent,
                         int sub_cell_number, int quadrant_number)
    : cell_type_(&cell_type), parent_(parent),
      sub_cell_number_(sub_cell_number), vertex_numbers_(vertex_numbers),
      quadrant_number_(quadrant_number) {}

void RefinedCell::replace_child(int old_child, int new_child) {
  std::vector< int >::iterator it =
      std::find(children_.begin(), children_.end(), old_child);
...
void RefinementTree::add_subtree(int parent,
...
void RefinementTree::add_subtree(int parent,
                                 const std::vector< int > &sub_cell_numbers,
                                 const std::vector< int > &quadrant_numbers) {
  ...
  for (int i = 0; i < num_sub_cells; ++i) {
    ...
    const CellType &cell_type =
        CellType::get_instance(tdim_, vertex_numbers.size());
    ...
    refined_cells_.push_back(RefinedCell(cell_type, vertex_numbers, parent,
                                         cell_number, quadrant_number));
  }
}

#if 0 // does not compile due to swap() conflict with const CellType& member of
      // RefinedCell.
...
int RefinementTree::num_cells() const { return refined_cells_.size(); }
...
void compute_refined_vertices(const CellType &cell_type, GDim gdim,
  ...
  for (int v = 0; v < num_refined_vertices; ++v) {
    ...
    for (int c = 0; c < gdim; ++c) {
      refined_coords[v * gdim + c] /=
          static_cast< Coordinate >(num_super_vertices);
    }
  }
}

void standard_refined_geometry_fun(const Entity &cell,
                                   std::vector< Coordinate > &refined_coords) {
  assert(!cell.mesh()->is_periodic());

  std::vector< Coordinate > coords;
  cell.get_coordinates(coords);
  compute_refined_vertices(cell.cell_type(), cell.gdim(), coords,
                           refined_coords);
}

void recursive_refined_geometry_fun(const Entity &root_cell,
  ...
  std::vector< Coordinate > coords;
  cell.get_coordinates(coords);
  compute_refined_vertices(cell.cell_type(), root_cell.gdim(), coords,
                           refined_coords);
}

void periodic_refined_geometry_fun(const Entity &cell,
                                   std::vector< Coordinate > &refined_coords) {
  assert(cell.mesh()->is_periodic());
  ...
  compute_refined_vertices(cell.cell_type(), cell.gdim(), coords,
                           refined_coords);
  refined_coords =
      periodify_slave_to_master(refined_coords, cell.gdim(), period);
}

void recursive_periodic_refined_geometry_fun(
    const Entity &root_cell, const RefinedCell &cell,
    std::vector< Coordinate > &refined_coords) {
  assert(root_cell.mesh()->is_periodic());
  ...
  compute_refined_vertices(cell.cell_type(), root_cell.gdim(), coords,
                           refined_coords);
  refined_coords =
      periodify_slave_to_master(refined_coords, root_cell.gdim(), period);
}

void compute_refined_cells(const Entity &cell, const RefinementTree &tree,
  ...
  for (int i = 0; i < parent_cell.num_children(); ++i) {
    ...
    for (int i = 0; i < static_cast< int >(vertex_numbers.size()); ++i) {
      for (int c = 0; c < gdim; ++c) {
        child_coords[i * gdim + c] =
            refined_coords[vertex_numbers[i] * gdim + c];
      }
    }
...
void compute_refined_cells_recursive(
    const Entity &root_cell, RefinementTree &tree,
    std::vector< Coordinate > &refined_cell_coordinates,
    std::vector< int > &refined_cell_sizes,
    std::vector< int > &sub_cell_numbers, std::vector< int > &tree_node_numbers,
    bool leaf_only, RecursiveRefinedGeometryFunction coord_fun) {
  assert(tree.is_refined(0)); // root of tree should be refined
  ...
  while (!parent_cell_index.empty()) {
    ...
    // compute_refined_vertices(parent_type, gdim, parent_coords,
    // refined_coords);

    LOG_DEBUG(2, "refined_coords = " << string_from_range(
                     refined_coords.begin(), refined_coords.end()));
