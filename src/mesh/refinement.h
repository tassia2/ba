...
private:
  ...
  RefinedCell(const CellType &cell_type,
              ...
              int sub_cell_number, int quadrant_number = -1);
  ...
  // additional index which is needed when building a refinement tree that
  // matches an existing p4est tree
...
public:
  ...
  /// \brief constructor for Refinement tree containing root cell and given
  /// number of uniform refinement levels with each cell having num_children
  /// childrens
  RefinementTree(TDim tdim, const CellType &root_cell_type, int levels,
                 int num_children);
...
private:
  ...
  friend void compute_refined_cells_recursive(
      ...
      std::vector< int > &tree_node_numbers, bool leaf_only,
...
void compute_refined_cells(
    const Entity &cell, const RefinementTree &tree,
    std::vector< Coordinate > &refined_cell_coordinates,
    std::vector< int > &refined_cell_sizes,
    std::vector< int > &sub_cell_numbers,
    RefinedGeometryFunction coord_fun = &standard_refined_geometry_fun);

/// \brief Computes the vertex coordinates and sub-cells to be created in a
/// refinement tree. Here, all new cells are returned, i.e. also non-leaf cells.
/// The vector tree_node_numbers stores the index of the respective cell
/// according to its entry in the array refined_cells of tree.
void compute_refined_cells_recursive(
    ...
    bool leaf_only,
    RecursiveRefinedGeometryFunction coord_fun =
        &recursive_refined_geometry_fun);
} // namespace mesh
} // namespace hiflow
