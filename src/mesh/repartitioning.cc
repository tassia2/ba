...
void MpiAllToAll::communicate(
  ...
  MPI_Alltoall(vec2ptr(num_coordinates_sent), 1, MPI_INT,
               vec2ptr(num_coordinates_recv), 1, MPI_INT, mpi_comm_);

  for (int i = 0; i < num_procs; ++i) {
    received_entities[i].coords.resize(num_coordinates_recv[i]);
    LOG_DEBUG(3, "On proc " << rank() << ", " << num_coordinates_recv[i]
                            << " coordinates will be received from process "
                            << i);
  }
  ...
  MPI_Alltoallv(vec2ptr(coordinates_sent), vec2ptr(num_coordinates_sent),
                vec2ptr(sdispl), MPI_DOUBLE, vec2ptr(coordinates_recv),
                vec2ptr(num_coordinates_recv), vec2ptr(rdispl), MPI_DOUBLE,
                mpi_comm_);
  ...
  MPI_Alltoall(vec2ptr(offset_sizes_sent), 1, MPI_INT,
               vec2ptr(offset_sizes_recv), 1, MPI_INT, mpi_comm_);

  for (int i = 0; i < num_procs; ++i) {
    received_entities[i].offsets.resize(offset_sizes_recv[i]);
    LOG_DEBUG(3, "On proc " << rank() << ", " << offset_sizes_recv[i]
  ...
  MPI_Alltoallv(vec2ptr(sent_offsets), vec2ptr(offset_sizes_sent),
                vec2ptr(sdispl), MPI_INT, vec2ptr(received_offsets),
                vec2ptr(offset_sizes_recv), vec2ptr(rdispl), MPI_INT,
                mpi_comm_);
  ...
  MPI_Alltoall(vec2ptr(mat_sizes_sent), 1, MPI_INT, vec2ptr(mat_sizes_recv), 1,
               MPI_INT, mpi_comm_);

  for (int i = 0; i < num_procs; ++i) {
    received_entities[i].material_numbers.resize(mat_sizes_recv[i]);
    LOG_DEBUG(3, "On proc "
                     << rank() << ", " << mat_sizes_recv[i]
                     << " material numbers will be received from process "
                     << i);
  }
  ...
  MPI_Alltoallv(vec2ptr(sent_mat), vec2ptr(mat_sizes_sent), vec2ptr(sdispl),
                MPI_INT, vec2ptr(received_mat), vec2ptr(mat_sizes_recv),
                vec2ptr(rdispl), MPI_INT, mpi_comm_);
  ...
  MPI_Alltoall(vec2ptr(conn_sizes_sent), 1, MPI_INT, vec2ptr(conn_sizes_recv),
               1, MPI_INT, mpi_comm_);

  for (int i = 0; i < num_procs; ++i) {
    received_entities[i].connections.resize(conn_sizes_recv[i]);
    LOG_DEBUG(3, "On proc " << rank() << ", " << conn_sizes_recv[i]
                            << " connections will be received from process "
                            << i);
  }
  ...
  MPI_Alltoallv(vec2ptr(sent_conn), vec2ptr(conn_sizes_sent), vec2ptr(sdispl),
                MPI_INT, vec2ptr(received_conn), vec2ptr(conn_sizes_recv),
                vec2ptr(rdispl), MPI_INT, mpi_comm_);
...
void compute_dual_graph_distributed(const MeshPtr mesh, MPI_Comm &comm,
                                    Graph *graph,
  ...
  for (EntityIterator it = mesh->begin(tdim); it != end; ++it) {
    int remote_index;
    mesh->get_attribute_value("_remote_index_", tdim, it->index(),
                              &remote_index);
    if (remote_index == -1) {
        ...
        for (IncidentEntityIterator iit = mesh->begin_incident(*it, i),
          ...
          for (IncidentEntityIterator iiit = mesh->begin_incident(*iit, tdim),
            ...
            if (it->index() != iiit->index()) {
              int remote_index_2;
              mesh->get_attribute_value("_remote_index_", tdim, iiit->index(),
                                        &remote_index_2);
              if (remote_index_2 == -1) {
                std::map< int, float >::iterator f_it =
                    adjacency_map[it->index()].find(iiit->index() +
                                                    vtxdist[my_rank]);
                if (f_it == adjacency_map[it->index()].end()) {
                  adjacency_map[it->index()][iiit->index() + vtxdist[my_rank]] =
                      std::pow(static_cast< float >(tdim + 1), i);
                }
              } else {
                int subdomain;
                mesh->get_attribute_value("_sub_domain_", tdim, iiit->index(),
                                          &subdomain);
                std::map< int, float >::iterator f_it =
                    adjacency_map[it->index()].find(remote_index_2 +
                                                    vtxdist[subdomain]);
                if (f_it == adjacency_map[it->index()].end()) {
                  adjacency_map[it->index()]
                               [remote_index_2 + vtxdist[subdomain]] =
                                   std::pow(static_cast< float >(tdim + 1), i);
                }
              }
            }
  ...
  for (std::map< int, std::map< int, float > >::iterator
           it = adjacency_map.begin(),
  ...
  LOG_INFO("Time for computing distributed dual graph",
           dual_graph_timer.get_duration());
}
...
/// \param gpartitioner The graph partitioner to be employed, i.e., a
/// ParMetisGraphPartitioner. \return  Shared pointer to local part of the
/// repartitioned mesh.

MeshPtr repartition_mesh(const MeshPtr mesh, MPI_Comm &comm,
  ...
  MPI_Allgather(&num_local_cells, 1, MPI_INT, vec2ptr(local_cell_nums), 1,
                MPI_INT, comm);
  ...
  LOG_INFO("Time to compute ghost cells",
           ghost_cell_computation.get_duration());
  ...
  float ubvec = 1.05f; // max load imbalance (not used)
  MPI_Comm comm_copy = comm;
  int options[3] = {0, 0, 0};

  std::vector< int > partitioning(vtxdist[my_rank + 1] - vtxdist[my_rank], 0);
  int metis_return =
      ParMETIS_V3_PartKway(vec2ptr(vtxdist), vec2ptr(mesh_graph.node_offsets),
                           vec2ptr(mesh_graph.adjacent_nodes), nullptr,
                           vec2ptr(mesh_graph.adjacent_weights), &wgtflag,
                           &numflag, &ncon, &nparts, tpwgts, &ubvec, options,
                           &num_edge_cuts, vec2ptr(partitioning), &comm_copy);
  assert(metis_return == METIS_OK);

  delete[] tpwgts;

  partition_computation.stop();
  LOG_INFO("Time for computing repartitioning",
           partition_computation.get_duration());
  ...
  compute_facet_from_cell_distribution(*mesh_with_ghost, distribution,
                                       &facet_distribution);

  // prepare sent entities
  pack_distributed_entities(*mesh_with_ghost, tdim, distribution,
                            sent_entities);
  pack_distributed_entities(*mesh_with_ghost, tdim - 1, facet_distribution,
                            sent_facet_entities);
  ...
  LOG_INFO("Time for cell communication (repartitioning)",
           partition_communication.get_duration());
  ...
  partition_communication.stop();
  LOG_INFO("Time for facet communication (repartitioning)",
           partition_communication.get_duration());
  ...
  LOG_INFO("Overall time to fill mesh builder object",
           fill_mesh_builder.get_duration());
