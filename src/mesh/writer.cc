...
void Writer::add_attribute(const std::string &attribute_name, TDim tdim) {
  ...
  if (find(attributes_to_write_.begin(), attributes_to_write_.end(),
           std::make_pair(attribute_name, tdim)) ==
      attributes_to_write_.end()) {
    attributes_to_write_.push_back(std::make_pair(attribute_name, tdim));
  }
}

void Writer::add_all_attributes(const Mesh &mesh,
                                bool internal_mesh_attributes) {
  for (TDim i = 0; i <= mesh.tdim(); ++i) { // dimensions
    std::vector< std::string > names = mesh.get_attribute_names(i);
    for (std::vector< std::string >::const_iterator it = names.begin();
         it != names.end(); ++it) { // attributes of dimension
      ...
      if (find(attributes_to_write_.begin(), attributes_to_write_.end(),
               std::make_pair(*it, i)) == attributes_to_write_.end()) {
...
void UcdWriter::write_file(const char *filename, const Mesh &mesh) const {
  ...
  UMAP_NAMESPACE::unordered_map< Id, EntityNumber > id_map;
  ...
  for (EntityIterator cell_it = mesh.begin(tdim), end_cell = mesh.end(tdim);
    ...
    cell_verts.clear();
    for (IncidentEntityIterator vert_it = cell_it->begin_incident(0),
                                end_vert = cell_it->end_incident(0);
    ...
    if (cell_type == "unknown") {
      LOG_ERROR("Unknown cell type: tdim = " << tdim << ", num_verts = "
                                             << cell_verts.size());
    }
  ...
  for (EntityIterator facet_it = bdy_mesh->begin(bdim),
                      end_facet = bdy_mesh->end(bdim);
       facet_it != end_facet; ++facet_it) {

    const MaterialNumber mat_num = facet_it->get_material_number();

    facet_verts.clear();
    facet_verts.insert(facet_verts.end(), facet_it->begin_vertex_ids(),
                       facet_it->end_vertex_ids());
    ...
    if (facet_type == "unknown") {
      LOG_ERROR("Unknown facet type: bdim = " << bdim << ", num_verts = "
                                              << facet_verts.size());
    }
    ...
    for (int c = 0; c != static_cast< int >(facet_verts.size()); ++c) {
      if (id_map.count(facet_verts[c]) == 0) {
        LOG_ERROR("No vertex with id " << facet_verts[c]
                                       << " found in id_map!");
...
void VtkWriter::write_file(const char *filename, const Mesh &mesh) const {
  ...
  for (DataVectorPair::const_iterator it = attributes_to_write_.begin();
    ...
    if (!mesh.has_attribute(it->first, it->second)) {
      LOG_ERROR("No attribute " << it->first << " for dimension " << it->second
                                << " found in mesh");
      continue;
    }

    if ((it->second != 0) && (it->second != tdim)) {
      LOG_ERROR("VTK does not support data for entities other than cells or "
                "vertices.");
      continue;
    }
    ...
    if ((int_attr = dynamic_cast< IntAttribute * >(attr)) != 0) {
      ...
      // TODO (Staffan): this code duplication was done under extreme stress.
      // Clean up!
      ...
      if ((int_attr = dynamic_cast< IntAttribute * >(data_attr)) != 0) {
        ...
        tinyxml2::XMLText *data = doc.NewText(os.str().c_str());
        data_array->LinkEndChild(data);
      } else if ((double_attr = dynamic_cast< DoubleAttribute * >(data_attr)) !=
                 0) {
        // double attribute
        data_array->SetAttribute("type", "Float64");
        for (EntityIterator entity_it = mesh.begin(it->second);
  ...
  cell_data->LinkEndChild(data_array);
  tinyxml2::XMLText *material_id = doc.NewText(os.str().c_str());
  ...
  for (EntityIterator it = mesh.begin(0); it != mesh.end(0); ++it) {
    ...
    if (deformation_ != nullptr) {
      for (int c = 0; c < 3; ++c) {
        vertex[c] += (*deformation_)[3 * it->index() + c];
      }
    }

    copy(vertex.begin(), vertex.end(),
    ...
    min = *min_element(vertex.begin(), vertex.end()) < min
              ? *min_element(vertex.begin(), vertex.end())
              : min;
    max = *max_element(vertex.begin(), vertex.end()) > max
              ? *max_element(vertex.begin(), vertex.end())
              : max;
  }

  tinyxml2::XMLText *coords = doc.NewText(os.str().c_str());
  ...
  data_array->SetAttribute("format", "ascii");
  for (EntityIterator it = mesh.begin(tdim); it != mesh.end(tdim); ++it) {
    for (VertexIdIterator iit = it->begin_vertex_ids();
         iit != it->end_vertex_ids(); ++iit) {
      os << vtk_id_map[*iit] << " ";
  ...
  cells->LinkEndChild(data_array);
  tinyxml2::XMLText *offs = doc.NewText(os.str().c_str());
  ...
  for (EntityIterator it = mesh.begin(tdim); it != mesh.end(tdim); ++it) {
    ...
    typ_min = vtk_cell_types[static_cast< int >(type.tag())] < typ_min
                  ? vtk_cell_types[static_cast< int >(type.tag())]
                  : typ_min;
    typ_max = vtk_cell_types[static_cast< int >(type.tag())] > typ_max
                  ? vtk_cell_types[static_cast< int >(type.tag())]
                  : typ_max;
  }
  ...
  cells->LinkEndChild(data_array);
  tinyxml2::XMLText *types = doc.NewText(os.str().c_str());
...
void PVtkWriter::write_file(const char *filename, const Mesh &mesh) const {
  ...
  std::string str_src_filename =
      (filename_without_suffix + "_" + s.str() + ".vtu");
  ...
  if (rank == 0) {
    ...
    std::vector< std::string > attr_names =
        mesh.get_attribute_names(mesh.tdim());
    LOG_INFO("writer", "Mesh cell attributes = " << string_from_range(
                           attr_names.begin(), attr_names.end()));
    ...
    for (DataVectorPair::const_iterator it = attributes_to_write_.begin();
      ...
      if (!mesh.has_attribute(it->first, it->second)) {
        LOG_ERROR("No attribute " << it->first << " for dimension "
                                  << it->second << " found in mesh");
        continue;
      }
      ...
      if (it->second == 0) { // -> is point_data
        ...
        // Normals, ...)
        p_point_data->SetAttribute("Scalars", it->first.c_str());
      }
      if (it->second == mesh.tdim()) { // -> is cell_data
    ...
    tinyxml2::XMLElement *p_cell_data_array = doc.NewElement("PDataArray");
