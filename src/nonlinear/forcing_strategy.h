...
public:
  ...
  DataType GetCurrentForcingTerm() const { return forcing_terms_.back(); }

  void SetForcingTerm(DataType forcing) { forcing_terms_.push_back(forcing); }

  void SetResidualNorm(DataType norm) { residuals_.push_back(norm); }

  virtual void ComputeForcingTerm(DataType new_residual,
                                  DataType lin_solve_accuracy) = 0;
...
template <class DataType> ForcingStrategy<DataType>::ForcingStrategy() {}
