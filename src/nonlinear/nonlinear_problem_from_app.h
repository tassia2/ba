...
public:
  ...
  NonlinearProblemFromApp(Application< DataType > &newapp) { app_ = &newapp; }
  ~NonlinearProblemFromApp(app_ = nullptr;);

  /// user supplied init function

  void Reinit(Application< DataType > &newapp) { app_ = &newapp; }

  /// evaluate function

  void EvalFunc(const VectorType &in, VectorType *out) {
    // Integrate<Laplace<LAD::DataType>, LAD, UnitIntegrator2d>(solvec,
    // quadrature, laplace, &Laplace<LAD::DataType>::rhs2d_1, &b);
  }
