...
template < class LAD, int DIM > class NonlinearSolverFactory {
  typedef std::map< std::string, NonlinearSolverCreator< LAD, DIM > * > products_t;
  products_t products;
...
public:
  ...
  NonlinearSolverFactory() {
    this->Register("Newton", new Newtoncreator< LAD, DIM >());
  }

  /// Register new product in factory.

  bool Register(const std::string id, NonlinearSolverCreator< LAD, DIM > *cr) {
    return products.insert(typename products_t::value_type(id, cr)).second;
  }

  /// Get new NonlinearSolverCreator object of given type.

  NonlinearSolverCreator< LAD, DIM > *Get(const std::string &id) const {
    typename products_t::const_iterator it = products.find(id);
    if (it != products.end()) {
      return it->second;
    }
    LOG_ERROR(
        "NonlinearSolverFactory::Get: No solver of this name registered.");
    return nullptr;
  }
};
