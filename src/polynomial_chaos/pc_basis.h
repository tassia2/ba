...
///
/// \author Michael Schick, Jonathan Schwegler
...
#ifdef WITH_GAUSSQ
extern "C" {
void gaussq_(int *kind, int *n, double *alpha, double *beta, int *kpts,
...
public:
  ...
  enum Distribution { NOT_SET = -1, UNIFORM, GAUSSIAN, CUSTOM };
  ...
  ~PCBasis() {
    for (size_t i = 0; i < custom_distr_.size(); ++i) {
      delete custom_distr_[i];
    }
  }
  ...
  void Init(int N, int No, std::vector< Distribution > const &distributions,
            const std::vector< PDF< double > * > &custom_distributions =
                std::vector< PDF< double > * >(0));

  /// Initialize with random space dimension N, polynomial degree No,
  /// vector containing probability distributions of input (size N), exponent
  /// for hyperbolic index selection and maximal number of nonzero entries in
  /// multi index
  void Init(int N, int No, std::vector< Distribution > const &distributions,
            double q, int t,
            const std::vector< PDF< double > * > &custom_distributions =
  ...
  double poly(int dist_index, int degree, double x) const;
  ...
  int SubDim(int p) const { return SubDim_[p]; }

  /// Calculate the Dimension of PC space for given polynomial degree and number
  /// of rvs
  ...
  std::vector< int > G2L(int glo_idx) const { return alpha_[glo_idx]; }

  /// Local 2 Global (for multi-index)

  int L2G(std::vector< int > &idx) const { return l2g_.at(idx); }

  /// Returns vector of used probability distributions

  Distribution const &Distributions(int n) const { return distributions_[n]; }

  PDF< double > *GetPDF(int dist_index) const {
    return custom_distr_[dist_index];
  }

  void GetQuadrature(int distribution, std::vector< double > &qpts,
                     std::vector< double > &weights) const {
    qpts = x_qpts_[distribution];
    weights = w_qpts_[distribution];
  }

  void GetQuadrature(std::vector< std::vector< double > > &qpts,
                     std::vector< double > &weights) const;
...
protected:
  ...
  /// Total dimension of PC expansion w.r.t. all degrees less or equal to max.
  /// dimension
  std::vector< int > SubDim_;
  /// Number of implemented distributions
  static const int ndist_ = 4;

  /// Compute Multi-Indices for multidimensional random input
  void ComputeMultiIndices();

  void ComputeRecursionCoeff();

  void ComputeQpointsAndWeights();
  ...
  // TODO: PDF smartpointers?
  std::vector< PDF< double > * > custom_distr_;

  std::vector< std::vector< double > > custom_alpha_;
  std::vector< std::vector< double > > custom_beta_;
  ...
  double CustomP(int degree, double x, int index) const;
};
