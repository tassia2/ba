...
PCGalerkinMatrix< DataType >::PCGalerkinMatrix(
    la::CoupledVector< DataType > const &dummy) {
  numthreads_ = 1;
...
template < class DataType > PCGalerkinMatrix< DataType >::~PCGalerkinMatrix() {
...
template < class DataType >
void PCGalerkinMatrix< DataType >::SetMatrix(
    std::vector< la::CoupledMatrix< DataType > * > modes) {
  nmodes_ = modes.size();
...
template < class DataType >
void PCGalerkinMatrix< DataType >::SetMatrixAllocate(
    std::vector< la::CoupledMatrix< DataType > * > modes) {
  nmodes_ = modes.size();
  modes_.resize(nmodes_);
  for (int i = 0; i < nmodes_; ++i) {
...
template < class DataType >
void PCGalerkinMatrix< DataType >::VectorMult(
    PCGalerkinVector< DataType > &in, PCGalerkinVector< DataType > *out) const {
  out->Zeros();
  ...
  if (numthreads_ == 1) {
    ...
    for (int loc_idx = 0; loc_idx < pctensor_->SizeLocal(); ++loc_idx) {
      ...
      for (int pos = breakpoints[bp - 1]; pos < breakpoints[bp]; ++pos) {
        modes_[pctensor_->Indices(pos)[0]]->VectorMult(
            *in.Mode(pctensor_->Indices(pos)[1]), tmp_vec_[0]);
        out->ModeAxpy(pctensor_->Indices(pos)[2], *tmp_vec_[0],
                      pctensor_->Val(pos));
      }
    }
  } else {
...
#pragma omp parallel for
    for (int loc_idx = 0; loc_idx < pctensor_->SizeLocal(); ++loc_idx) {
      int bp = pctensor_->L2G(loc_idx) + 1;
      for (int pos = breakpoints[bp - 1]; pos < breakpoints[bp]; ++pos) {
        modes_[pctensor_->Indices(pos)[0]]->diagonal().VectorMult(
            in.Mode(pctensor_->Indices(pos)[1])->interior(),
            &(tmp_vec_[omp_get_thread_num()]->interior()));
        out->Mode(pctensor_->Indices(pos)[2])
            ->interior()
            .Axpy(tmp_vec_[omp_get_thread_num()]->interior(),
                  pctensor_->Val(pos));
      }
    }
...
#pragma omp parallel for
    for (int loc_idx = 0; loc_idx < pctensor_->SizeLocal(); ++loc_idx) {
      ...
      for (int pos = breakpoints[bp - 1]; pos < breakpoints[bp]; ++pos) {
        ...
        modes_[pctensor_->Indices(pos)[0]]->offdiagonal().VectorMult(
            in.Mode(pctensor_->Indices(pos)[1])->ghost(),
            &(tmp_vec_[omp_get_thread_num()]->interior()));
        out->Mode(pctensor_->Indices(pos)[2])
            ->interior()
            .Axpy(tmp_vec_[omp_get_thread_num()]->interior(),
...
template < class DataType > void PCGalerkinMatrix< DataType >::Zeros() {
  for (int i = 0; i < nmodes_; ++i) {
    modes_[i]->Zeros();
  }
}
...
template < class DataType >
void PCGalerkinMatrix< DataType >::CopyFrom(
    const PCGalerkinMatrix< DataType > &mat) {
  this->SetMatrix(*mat.GetModes());
  this->SetTensor(mat.GetTensor());
}

template < class DataType >
std::vector< la::CoupledMatrix< DataType > * > const *
PCGalerkinMatrix< DataType >::GetModes() const {
  return &modes_;
}

template < class DataType >
typename PCGalerkinMatrix< DataType >::GalerkinTensor *
PCGalerkinMatrix< DataType >::GetTensor() const {
  return pctensor_;
}
