...
/// using PCGalerkinVector class. The modes of the matrix correspond to the
/// modes of a Polynomial Chaos expansion of the differential operator modelling
/// the PDE. \author Michael Schick
...
template < class DataType > class PCGalerkinMatrix {
public:
  ...
  la::CoupledMatrix< DataType > *Mode(int mode) const { return modes_[mode]; }
  ...
  inline int GetCurrentLevel() const { return level_; }
...
int PCGalerkinMatrix< DataType >::nrows_local() const {
  return modes_[0]->num_rows_local();
}
...
template < class DataType >
void PCGalerkinMatrix< DataType >::ExtractDiagonalCSR(int *ia, int *ja,
                                                      DataType *val) const {
