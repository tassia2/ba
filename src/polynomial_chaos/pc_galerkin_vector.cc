...
template < class DataType > PCGalerkinVector< DataType >::PCGalerkinVector() {
  nmodes_ = -1;
  noff_modes_ = -1;
  nmodes_total_ = -1;
  comm_ = MPI_COMM_NULL;
...
PCGalerkinVector< DataType >::PCGalerkinVector(
    PCGalerkinVector< DataType > const &vec) {
  this->Clear();
...
template < class DataType >
void PCGalerkinVector< DataType >::ModeAxpy(
    int mode, la::CoupledVector< DataType > const &vec, const DataType alpha) {
  assert(mode < nmodes_total_ && nmodes_total_ > -1);
  modes_[mode]->Axpy(vec, alpha);
}

template < class DataType >
void PCGalerkinVector< DataType >::Axpy(PCGalerkinVector< DataType > const &vec,
...
template < class DataType >
DataType PCGalerkinVector< DataType >::Dot(
    const PCGalerkinVector< DataType > &vec) const {
  int min = nmodes_;
  if (vec.NModes() < min) {
    min = vec.NModes();
  }
...
template < class DataType > void PCGalerkinVector< DataType >::Zeros() {
...
template < class DataType >
void PCGalerkinVector< DataType >::ScaleAdd(
    const PCGalerkinVector< DataType > &vec, const DataType alpha) {
  int min = nmodes_;
  if (vec.NModes() < min) {
    min = vec.NModes();
  }
...
void PCGalerkinVector< DataType >::SynchronizeModes(GalerkinTensor *pctensor) {
  ...
  for (int rank = 0; rank < numproc; ++rank) {
    if (rank != my_rank) {
      nsend += pctensor->SendHostileModes(rank).size();
    }
  }
  ...
  for (int rank = 0; rank < numproc; ++rank) {
    if (rank != my_rank) {
      for (int n = 0; n < pctensor->SendHostileModes(rank).size(); ++n) {
        ...
        int tag = 4;
        int info = MPI_Isend(this->Mode(loc_idx)->interior().GetBuffer(),
                             this->DSizeLocal(), MPI_DOUBLE, rank, tag,
                             pctensor->Comm(), &request[cntr]);
      ...
      for (int n = 0; n < pctensor->RecvHostileModes(rank).size(); ++n) {
        ...
        MPI_Status status;
        MPI_Recv(this->Mode(loc_idx)->interior().GetBuffer(),
                 this->DSizeLocal(), MPI_DOUBLE, rank, tag, pctensor->Comm(),
                 &status);
      }
    }
  }
...
template < class DataType >
void PCGalerkinVector< DataType >::SynchronizeModesGhost(
    GalerkinTensor *pctensor) {
  int my_rank = pctensor->MyRank();
  int numproc = pctensor->NumProc();
  int nsend = 0;
  for (int rank = 0; rank < numproc; ++rank) {
  ...
  for (int rank = 0; rank < numproc; ++rank) {
    if (rank != my_rank) {
      for (int n = 0; n < pctensor->SendHostileModes(rank).size(); ++n) {
        ...
        int tag = 5;
        int info =
            MPI_Isend(this->Mode(loc_idx)->ghost().GetBuffer(),
                      this->Mode(loc_idx)->size_local_ghost(), MPI_DOUBLE, rank,
                      tag, pctensor->Comm(), &request[cntr]);
        assert(info == MPI_SUCCESS);
        cntr++;
      }
      ...
      for (int n = 0; n < pctensor->RecvHostileModes(rank).size(); ++n) {
        ...
        MPI_Status status;
        MPI_Recv(this->Mode(loc_idx)->ghost().GetBuffer(),
                 this->Mode(loc_idx)->size_local_ghost(), MPI_DOUBLE, rank, tag,
                 pctensor->Comm(), &status);
      }
    }
  }
  int info_send = MPI_Waitall(nsend, &request.front(), &status.front());
...
template < class DataType >
void PCGalerkinVector< DataType >::CreateCoarseVector(
    GalerkinTensor *pctensor, PCGalerkinVector< DataType > &coarse_vec) const {
  // 1. Allocate data in coarse vec
  int n_coarse_modes = pctensor->SizeLocal();
  int n_off_coarse_modes = pctensor->SizeOffModes();
  std::vector< la::CoupledVector< DataType > * > coarse_modes(
      n_coarse_modes + n_off_coarse_modes);
  for (int i = 0; i < coarse_modes.size(); ++i) {
    coarse_modes[i] = new la::CoupledVector< DataType >;
    coarse_modes[i]->CloneFrom(*this->Mode(
        0)); // Use dummy mode instead -> otherwise bug for low number of rvs?
  }
  ...
  for (int lid_coarse = 0; lid_coarse < n_coarse_modes; ++lid_coarse) {
    ...
    if (pctensor->Ownership(gid_coarse, coarse_level + 1) == my_rank) {
      coarse_vec.Mode(lid_coarse)
          ->CopyFrom(*this->Mode(pctensor->G2L(gid_coarse, coarse_level + 1)));
    }
  }
}

template < class DataType >
void PCGalerkinVector< DataType >::AssignToRefinedVector(
    GalerkinTensor *pctensor, const PCGalerkinVector< DataType > &coarse_vec) {
  // 1. Allocate data in coarse vec
  ...
  for (int lid_fine = 0; lid_fine < nmodes_; ++lid_fine) {
    ...
    if (gid_fine < pctensor->Size(fine_level - 1)) {
      if (pctensor->Ownership(gid_fine, fine_level - 1) == my_rank) {
        this->Mode(lid_fine)->CopyFrom(
            *coarse_vec.Mode(pctensor->G2L(gid_fine, fine_level - 1)));
      }
    }
  }
}

template < class DataType > void PCGalerkinVector< DataType >::Initialize() {
  if (!modes_.empty()) {
...
template < class DataType > void PCGalerkinVector< DataType >::Clear() {
  if (nmodes_total_ > -1) {
...
template < class DataType >
void PCGalerkinVector< DataType >::CopyMode(
    int mode, la::CoupledVector< DataType > const &vec) {
  assert(mode < nmodes_total_ && nmodes_total_ > -1);
...
void PCGalerkinVector< DataType >::SetMPIComm(const MPI_Comm &comm) {
  ...
  info = MPI_Comm_size(comm, &this->nb_procs_);
  assert(info == MPI_SUCCESS);
  assert(nb_procs_ > 0);
...
template < class DataType >
void PCGalerkinVector< DataType >::SetModes(
    std::vector< la::CoupledVector< DataType > * > const &modes, int nmodes,
    int noff_modes) {
  if (nmodes_total_ > -1) {
    this->Clear();
  }
...
template < class DataType >
void PCGalerkinVector< DataType >::CloneFrom(
    const PCGalerkinVector< DataType > &vec) {
  this->Clear();
...
template < class DataType >
void PCGalerkinVector< DataType >::CopyFrom(
    const PCGalerkinVector< DataType > &vec) {
...
template < class DataType >
void PCGalerkinVector< DataType >::CloneFromWithoutContent(
    const PCGalerkinVector< DataType > &vec) {
  this->Clear();
  nmodes_ = vec.NModes();
  noff_modes_ = vec.NOffModes();
...
template < class DataType >
void PCGalerkinVector< DataType >::WriteHDF5(const std::string &filename,
                                             const std::string &groupname,
                                             const std::string &datasetname) {
#ifdef WITH_HDF5

  for (int modes = 0; modes < nmodes_total_; ++modes) {
    std::stringstream vectorname;
    vectorname << datasetname << "_" << modes;
    modes_[modes]->WriteHDF5(filename, groupname, vectorname.str());
  }
  const int num_int_members = 4;
  std::vector< int > int_members(num_int_members);
  int_members[0] = nmodes_;
  int_members[1] = noff_modes_;
  int_members[2] = nmodes_total_;
  int_members[3] = size_local_deterministic_;
  H5FilePtr file_ptr(new H5File(filename, "w", this->comm_));
  H5GroupPtr group_ptr(new H5Group(file_ptr, groupname, "w"));
  write_array_parallel< int >(
      group_ptr, "nmodes.noff_modes.nmodes_total.size_local_deterministic",
      int_members, comm_);
#else
...
template < class DataType >
void PCGalerkinVector< DataType >::ReadHDF5(const std::string &filename,
                                            const std::string &groupname,
                                            const std::string &datasetname) {
#ifdef WITH_HDF5
  const int num_int_members = 4;
  ...
  H5FilePtr file_ptr(new H5File(filename, "w", this->comm_));
  H5GroupPtr group_ptr(new H5Group(file_ptr, groupname, "w"));
  read_array_parallel< int >(
      group_ptr, "nmodes.noff_modes.nmodes_total.size_local_deterministic",
      int_members, comm_);
  group_ptr.reset();
  file_ptr.reset();
  nmodes_ = int_members[0];
  noff_modes_ = int_members[1];
  nmodes_total_ = int_members[2];
  size_local_deterministic_ = int_members[3];
  for (int modes = 0; modes < nmodes_total_; ++modes) {
    std::stringstream vectorname;
    vectorname << datasetname << "_" << modes;
    modes_[modes]->ReadHDF5(filename, groupname, vectorname.str());
  }
#else
  throw "HiFlow was not compiled with HDF5 support!\n";
#endif
