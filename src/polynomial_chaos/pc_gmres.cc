...
template < class LAD > PCGMRES< LAD >::PCGMRES() : GMRES< LAD >() {}

/// destructor

template < class LAD > PCGMRES< LAD >::~PCGMRES() {
  this->op_ = NULL;
  this->precond_ = NULL;
}

template < class LAD >
void PCGMRES< LAD >::SetupResidualAssembler(
...
template < class LAD >
void PCGMRES< LAD >::GeneratePlaneRotation(const DataType &dx,
...
LinearSolverState PCGMRES< LAD >::SolveNoPrecond(const VectorType &b,
  ...
  std::vector< DataType > g(this->size_basis() +
  ...
  LOG_INFO("PCGMRES",
           " without preconditioning ended after " << iter << " iterations ");
...
template < class LAD >
LinearSolverState PCGMRES< LAD >::SolveRight(const VectorType &b,
                                             VectorType *x) {
  ...
  while (conv == IterateControl::kIterate) {
    ...
    for (int j = 0; j < this->size_basis(); ++j) {
      ...
      this->ApplyPreconditioner(*V[j], &z); // z ~= M^-1 v_j
      // w = A z
      if (solve_type_ == "Free") {
  ...
  LOG_INFO("PCGMRES", " with right preconditioning ended after "
                          << iter << " iterations ");
  LOG_INFO("PCGMRES", " with residual norm " << this->res_);
