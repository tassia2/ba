...
PCMultilevelSolver< LAD >::PCMultilevelSolver(const PropertyTree &config)
    : la::LinearSolver< LAD >(), config_(&config) {
  ...
  this->pctensor_ = NULL;
  this->use_as_precond_ = false;
}

template < class LAD > PCMultilevelSolver< LAD >::~PCMultilevelSolver() {
  this->Clear();
}

template < class LAD > void PCMultilevelSolver< LAD >::Clear() {
...
void PCMultilevelSolver< LAD >::SetupOperator(OperatorType &op) {
  ...
  if (mltype_ == "Free") {
    std::cout << "Warning, using Multilevel with matrix in matrix free mode\n";
  }
...
template < class LAD >
void PCMultilevelSolver< LAD >::SetupSmoother(
    la::CoupledMatrix< DataType > &smoother) {
  smoother_ = &smoother;
  this->SetModifiedSmoother(true);
}

template < class LAD >
void PCMultilevelSolver< LAD >::SetupResidualAssembler(
    ResidualAssembler< LAD > &assembler) {
  res_assembler_ = &assembler;
}

template < class LAD >
void PCMultilevelSolver< LAD >::BuildImpl(VectorType const *b, VectorType *x) {
...
#if 0
    mean_solver_umf_.Init_LU_umfpack();
    mean_solver_umf_.SetupOperator(*smoother_);
    mean_solver_umf_.Build(b, x);
...
//    interminable_assert(0);
    ...
    LOG_ERROR("Unkown Smoother type for PC Multilevel");
    exit(-1);
  }
  this->SetModifiedOperator(false);
  this->SetModifiedSmoother(false);
...
template < class LAD >
void PCMultilevelSolver< LAD >::UpdateControl(int maxit, double atol,
                                              double rtol, double dtol) {
  mean_solver_->InitControl(maxit, atol, rtol, dtol);
}

template < class LAD >
la::LinearSolverState PCMultilevelSolver< LAD >::SolveImpl(const VectorType &b,
                                                           VectorType *x) {
  if (this->use_as_precond_) {
    return this->SolvePrecond(b, x);
  }
  return this->SolveStandAlone(b, x);
}

template < class LAD >
la::LinearSolverState
PCMultilevelSolver< LAD >::SolvePrecond(const VectorType &b, VectorType *x) {
  if (mu_ == 1)
  {
    if (this->print_level_ > 2) 
    {
      LOG_INFO("Preconditioner", "Solving one V-cycle of Multilevel");
    }
  }
  if (mu_ == 2)
  {
    if (this->print_level_ > 2) 
    {
      LOG_INFO("Preconditioner", "Solving one W-cycle of Multilevel");
    }
  }
  ML(b, x, pctensor_->GetLevel());

  LOG_INFO("Preconditioner", "Multilevel preconditioner completed");
...
template < class LAD >
la::LinearSolverState
PCMultilevelSolver< LAD >::SolveStandAlone(const VectorType &b, VectorType *x) {

  if (mu_ == 1)
  {
    if (this->print_level_ > 2) 
    {
      LOG_INFO("LinearSolver", "Solving one V-cycle of Multilevel");
    }
  }
  if (mu_ == 2)
  {
    if (this->print_level_ > 2) 
    {
      LOG_INFO("LinearSolver", "Solving one W-cycle of Multilevel");
    }
  }
  int iter = 0;
  ...
  while (conv == IterateControl::kIterate) {
    ...
    if (global_mpi_rank_ == 0) {
      ...
      {
        std::cout << "Multilevel current residual: " << this->res_
                  << " in iteration: " << iter << "\n";
      }
    }
  }
  if (global_mpi_rank_ == 0) {
    if (this->print_level_ > 0) 
    {
      std::cout << "Multilevel final residual: " << this->res_ << " finished in "
                << iter << " iterations\n";
    }
  }
...
template < class LAD >
void PCMultilevelSolver< LAD >::Solve_Mean_Problem(VectorType const &b,
                                                   VectorType *x) {
  Solve_Mode_Problem(0, b, x);
}

template < class LAD >
void PCMultilevelSolver< LAD >::Solve_Mode_Problem(int mode,
                                                   VectorType const &b,
                                                   VectorType *x) {
...
template < class LAD >
void PCMultilevelSolver< LAD >::Smoothing(VectorType const &b, VectorType *x,
                                          int nu) {
