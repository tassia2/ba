...
public:
  ...
  virtual void SetPCTensor(GalerkinTensor *pctensor) { pctensor_ = pctensor; }
  ...
  virtual bool GetModifiedSmoother() { return this->modified_smoother_; }

  virtual void SetUseAsPrecond(bool flag) { this->use_as_precond_ = flag; }
...
private:
  ...
  /// where the matrix A is provided either by SetupOperator or by
  /// SetupResidualAssembler
  virtual la::LinearSolverState SolveImpl(const VectorType &b, VectorType *x);

  /// Applies the multilevel solver as a preconditioner.
  virtual la::LinearSolverState SolvePrecond(const VectorType &b,
                                             VectorType *x);

  /// Applies the multilevel solver as a preconditioner.
  virtual la::LinearSolverState SolveStandAlone(const VectorType &b,
                                                VectorType *x);

  bool use_as_precond_;
...
#ifdef WITH_UMFPACK
  /// Smoother Umfpack
  la::PreconditionerBlockJacobiExt < la::LADescriptorCoupledD > mean_solver_umf_;
#endif
#ifdef WITH_ILUPP
