  ...
  for (int p = 0; p < pcbasis.No() + 1; ++p) {
    ...
    for (int bp = 1; bp <= pc_size_[p]; ++bp) {
      for (int pos = breakpoints[bp - 1]; pos < breakpoints[bp]; ++pos) {
        ...
        if (ownership_[p][idx[2]] == my_rank_ &&
            ownership_[p][idx[1]] != my_rank_) {
    ...
    for (int rank = 0; rank < numproc_; ++rank) {
      ...
      std::sort(hostile_modes_needed_[p][rank].begin(),
                hostile_modes_needed_[p][rank].end());
      std::vector< int >::iterator it;
      it = std::unique(hostile_modes_needed_[p][rank].begin(),
                       hostile_modes_needed_[p][rank].end());
      hostile_modes_needed_[p][rank].resize(
          std::distance(hostile_modes_needed_[p][rank].begin(), it));
    }
    ...
    for (int rank = 0; rank < numproc_; ++rank) {
      if (rank != my_rank_ && sizes[my_rank_][rank] > 0) {
        int tag = 99;
        MPI_Isend(&hostile_modes_needed_[p][rank][0], sizes[my_rank_][rank],
                  MPI_INT, rank, tag, comm_, &request[send_cntr]);
        send_cntr++;
      }
    }
    ...
    for (int rank = 0; rank < numproc_; ++rank) {
      if (rank != my_rank_ && sizes[rank][my_rank_] > 0) {
        int tag = 99;
        MPI_Status status;
        MPI_Recv(&hostile_modes_send_[p][rank][0], sizes[rank][my_rank_],
                 MPI_INT, rank, tag, comm_, &status);
      }
    }
    ...
    for (int rank = 0; rank < numproc_; ++rank) {
      if (rank != my_rank_) {
        for (int i = 0;
             i < static_cast< int >(hostile_modes_needed_[p][rank].size());
             ++i) {
...
bool pair_compare(std::pair< int, int > const &lhs,
