...
public:
  ...
  void SetTensorEps(double tensor_eps) { tensor_eps_ = tensor_eps; }
  ...
  int Size() const { return pc_size_[level_]; }
  ...
  int SizeLocal() const { return nmodes_local_[level_]; }
  /// Returns number of ghost layer modes on the current level

  int SizeOffModes() const { return noff_modes_[level_]; }

  /// Return value of tensor at specific position
  /// To be used either with IndicesNL(int pos) or Indices(int pos)

  inline double Val(int pos) const { return val_[level_][pos]; }
  /// Return value of tensor at specific position (special case linear random
  /// input) To be used together with the function IndicesL(int pos)
  ...
  inline const std::vector< int > &IndicesGlobal(int pos) const {
    return glo2loc_[level_][pos];
  }

  /// Direct access to L^2 - squared norm of a Polynomial Chaos basis function

  double SquareNorm(int mode) const { return square_norm_[mode]; }

  /// Returns end index of PCTensor on current level

  inline int End() const { return nnz_[level_]; }
  /// Returns k-breakpoints for seperation of PC test functions

  inline const std::vector< int > &k_breakpoints() const {
    return k_breakpoints_[level_];
  }
  /// Returns k-breakpoints for seperation of PC test functions (special case
  /// for linear random inputs)

  inline const std::vector< int > &k_breakpointsL() const {
  ...
  inline int L2G(int loc_idx) const { return l2g_[level_][loc_idx]; }
  /// Mapping local to global index on specified level

  inline int L2G(int loc_idx, int level) const { return l2g_[level][loc_idx]; }
...
protected:
  ...
  int q_;
  /// Threshold value for determining if c_{ijk}/c_{0kk} = 0 or not. Default
  /// value is 1.e-15
  double tensor_eps_;
  ...
  /// k-breakpoints, sorting the iteration over the Tensor entries by the
  /// testfunctions global index (the index k of c_{ijk})
  ...
  /// Number of modes owned by this process, offsets for this process and ghost
  /// layer size
