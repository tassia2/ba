...
// TODO: does it make sense to explicitely use the datatype double as template parameter in PolyCoeff, but allowing 
// floats and doubles as argument x in LagrangePolynomials??
// Or should LagrangeCoeff get a template parameter <class DataType> ?
...
inline const PolyCoeff< double > &LagrangeCoeff::poly(int k) const {
  assert((k >= 0) && (k < static_cast< int >(_poly.size())));
  return _poly[k];
}

inline PolyCoeff< double > &LagrangeCoeff::poly(int k) {
  assert((k >= 0) && (k < static_cast< int >(_poly.size())));
  return _poly[k];
}
...
inline const PolyCoeff< double > &LagrangeCoeff::poly_x(int k) const {
  assert((k >= 0) && (k < static_cast< int >(_poly_x.size())));
  return _poly_x[k];
}

inline PolyCoeff< double > &LagrangeCoeff::poly_x(int k) {
  assert((k >= 0) && (k < static_cast< int >(_poly_x.size())));
  return _poly_x[k];
}
...
inline const PolyCoeff< double > &LagrangeCoeff::poly_xx(int k) const {
  assert((k >= 0) && (k < static_cast< int >(_poly_xx.size())));
  return _poly_xx[k];
}

inline PolyCoeff< double > &LagrangeCoeff::poly_xx(int k) {
  assert((k >= 0) && (k < static_cast< int >(_poly_xx.size())));
  return _poly_xx[k];
}
