...
template < class DataType > class QuadratureCreator {
public:
...
public:
  Quadrature< DataType > *params(const PropertyTree &c) {
    ...
    if (c.contains("Order")) {
      newQuad->set_quadrature_by_order(
          c["Name"].template get< std::string >().c_str(),
          c["Order"].template get< int >());
    }
    else if (c.contains("Size")) {
      newQuad->set_quadrature_by_size(c["Name"].template get< std::string >().c_str(),
                                      c["Size"].template get< int >());
    } 

    std::string type = c["CellType"].template get< std::string >().c_str();
    const char *cellType = type.c_str();

    if (strcmp(cellType, "Hexahedron") == 0) {
      newQuad->set_cell_tag(mesh::CellType::HEXAHEDRON);    //replaced "set_cell_type" by "set_cell_tag"

    } else if (strcmp(cellType, "Quadrilateral") == 0) {
      newQuad->set_cell_tag(mesh::CellType::QUADRILATERAL);

    } else if (strcmp(cellType, "Triangle") == 0) {
      newQuad->set_cell_tag(mesh::CellType::TRIANGLE);

    } else if (strcmp(cellType, "Pyramid") == 0) {
      newQuad->set_cell_tag(mesh::CellType::PYRAMID);
    }
...
template < class DataType > class QuadratureFactory {
