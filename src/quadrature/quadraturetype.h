...
template < class DataType > class QuadratureType {
public:
  /// Default constructor
  QuadratureType();
  ...
  {
    int order = -1;
    for (int cur_order = 0; cur_order < this->order_size_map_.size(); ++cur_order)
    {
      const int cur_size = this->order_size_map_[cur_order];
      if (size >= cur_size)
      {
        order = cur_order;
      }
