...
/// @brief Handling of periodic boundary conditions in case of boundaries
/// orthogonal to cartesian axes.
...
template < class DataType, int DIM >
void PeriodicBoundaryConditionsCartesian< DataType, DIM >::compute_conditions(
  ...
  for (mesh::EntityIterator it_boundary = boundary_mesh->begin(tdim - 1);
    ...
    if (subdomain == rank) {
      ...
      typename PeriodicBoundaryConditionsCartesian< DataType, DIM >::MaterialNumber
          mat_num = face.get_material_number();

      // get dofs and coords on face for a given variable
      std::vector< doffem::DofID > dofs_on_face;
      std::vector< Vec<DIM, DataType> > coords_on_face;

      for (int var = 0; var < space.aaa_get_nb_var(); ++var) {
        // Get the dofs indices and coords on the face
        aaa_find_dofs_and_coords_on_face< DataType, DIM >(it_boundary, space, var,
        ...
        for (int i = 0; i < dofs_on_face.size(); ++i) {
          if (this->is_periodic_boundary(mat_num)) {
            bool new_entry; // true if entry hasn't been considered
            new_entry = this->insert_dof_id_entry(mat_num, var, dofs_on_face[i]);
  ...
  typename std::map<
      ...
      typename PeriodicBoundaryConditionsCartesian<
          DataType, DIM >::MaterialNumber >::const_iterator mat_tuple_it =
  ...
  while (mat_tuple_it != this->boundary_descriptor_.end()) {
    ...
    if (mat_a < mat_b) {
      ...
      for (int var = 0; var < space.aaa_get_nb_var(); ++var) {
        ...
        std::map< doffem::DofID, DataType > *ordinate_1 = 0;
        std::map< doffem::DofID, DataType > *ordinate_2 = 0;
