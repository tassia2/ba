...
#define BOOST_TEST_MODULE boundary_extraction
...
BOOST_AUTO_TEST_CASE(boundary_extraction) {
  const TDim tdim = 3;
  const GDim gdim = 3;
  ...
  BOOST_CHECK_EQUAL(refined_mesh->num_entities(tdim), 8);
  BOOST_CHECK_EQUAL(refined_mesh->num_entities(tdim - 1), 36);

  LOG_DEBUG(1, "Extracting boundary mesh of refined unit cube");
  MeshPtr refined_bdy_mesh = refined_mesh->extract_boundary_mesh();

  BOOST_CHECK_EQUAL(refined_bdy_mesh->num_entities(tdim - 1), 24);
  ...
  std::string filename_elaborate =
    std::string(datadir) + std::string("L_domain2.inp");
  MeshBuilder *mb_elaborate(
    new MeshDbViewBuilder(tdim_elaborate, gdim_elaborate));
  ...
  for (EntityIterator cell_it = mesh_elaborate->begin(tdim_elaborate);
       cell_it != mesh_elaborate->end(tdim_elaborate); ++cell_it) {
  ...
  for (EntityIterator cell_it = mesh_elaborate->begin(tdim_elaborate);
    ...
    for (IncidentEntityIterator vertex_it = cell_it->begin_incident(0);
      ...
      // std::cout << "Coordinates of vertex with index: " << vertex_it->id() <<
      // std::endl;
  ...
  MeshPtr boundary_mesh_elaborate =
    refined_mesh_elaborate->extract_boundary_mesh();

  assert(boundary_mesh_elaborate != 0);

  ScopedPtr< Writer >::Type writer_elaborate(new VtkWriter());
  writer_elaborate->write("Ldomain2_refined_by_attribute_step3.vtu",
                          *refined_mesh_elaborate);
  writer_elaborate->write("boundary_mesh_refined_by_attribute.vtu",
                          *boundary_mesh_elaborate);

  BOOST_CHECK_EQUAL(refined_mesh_elaborate->num_entities(tdim_elaborate), 42);
  // Hanging nodes and so on...
  BOOST_CHECK_EQUAL(refined_mesh_elaborate->num_entities(tdim_elaborate - 1), 81);

  BOOST_CHECK_EQUAL(boundary_mesh_elaborate->num_entities(tdim_elaborate - 1),
                    18);
