...
#define BOOST_TEST_MODULE cell_type
...
BOOST_AUTO_TEST_CASE(cell_type) {
...
void test_triangle() {
  ...
  for (int i = 0; i < num_triangle_sub_cells; ++i) {
    const std::vector< int > edges_of_sub_cell =
        cell_type.sub_entities_of_cell(facet_tdim, i);
    for (int e = 0; e < 3; ++e) {
      const int edge = edges_of_sub_cell[e];
      BOOST_CHECK_EQUAL(cell_type.regular_parent(facet_tdim, edge),
                 triangle_sub_cell_parent_edges[i][e]);
    }
  }
}
...
void test_hex() {
  ...
  for (int i = 0; i < num_hex_sub_cells; ++i) {
    const std::vector< int > facets_of_sub_cell =
        cell_type.sub_entities_of_cell(facet_tdim, i);
    for (int f = 0; f < 6; ++f) {
      const int facet = facets_of_sub_cell[f];
      BOOST_CHECK_EQUAL(cell_type.regular_parent(facet_tdim, facet),
                 hexahedron_sub_cell_parent_facets[i][f]);
    }
  }
}
