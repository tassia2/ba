...
int main(int argc, char *argv[]) {
  ...
  for (int test_number = 0; test_number < static_cast< int >(filenames.size());
    ...
    std::vector<FEType> fe_ansatz_c (1, FEType::LAGRANGE);
    std::vector<FEType> fe_ansatz_dc (1, FEType::LAGRANGE);
    ...
    dof_c->number(DOF_ORDERING::HIFLOW_CLASSIC);      //still throwing error: Attribute error: attribute _sub_domain_ missing
    ...
    dof_dc->number(DOF_ORDERING::HIFLOW_CLASSIC);      //still throwing error: Attribute error: attribute _sub_domain_ missing
