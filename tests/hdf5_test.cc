...
void read_dataset() {
  ...
  std::cout << "\n";
  BOOST_CHECK_EQUAL(r_map["Ceterum"], "censeo");
  BOOST_CHECK_EQUAL(r_map["Carthaginem"], "esse");
  BOOST_CHECK_EQUAL(r_map["delendam"], "!");
  ...
  BOOST_CHECK_EQUAL(r_sv_s, "Discite moniti!");
  ...
  for (int i = 0; i < static_cast< int >(r_v_s.size()); i++)
    std::cout << r_v_s[i] << "  ";
  std::cout << "\n";
  BOOST_CHECK_EQUAL(r_v_s[0], "Alea");
  BOOST_CHECK_EQUAL(r_v_s[1], "iacta");
  BOOST_CHECK_EQUAL(r_v_s[2], "est!");
  ...
  for (int i = 0; i < size_r_ar_s; i++)
    std::cout << r_ar_s[i] << "  ";
  std::cout << "\n";
  BOOST_CHECK_EQUAL(r_ar_s[0], "Cui");
  BOOST_CHECK_EQUAL(r_ar_s[1], "bono?");
  ...
  for (int i = 0; i < size_r_ar_i; i++)
    std::cout << r_ar_i[i] << "  ";
  std::cout << "\n";
  for (int i = 0; i < size_r_ar_i; i++)
    BOOST_CHECK_EQUAL(r_ar_i[i], i);
  free(r_ar_i);
  ...
  for (int i = 0; i < static_cast< int >(r_v_i.size()); i++)
    std::cout << r_v_i[i] << "  ";
  std::cout << "\n";
  for (int i = 0; i < static_cast< int >(r_v_i.size()); i++)
    BOOST_CHECK_EQUAL(r_v_i[i], i);
}
