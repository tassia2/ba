...
#define BOOST_TEST_MODULE hp_assembly
...
public:
  ...
  struct ExactSol
  {
    ...
    double operator() ( const Vec<DIM, double>& pt ) const
    {
      ...
      else if ( LaplaceApp::DIM == 3 )
      {
        ...
        const double y = pt[1];
        const double z = pt[2];
        return std::cos ( M * pi * x ) * std::cos ( N * pi * y ) * std::cos (
                 P * pi * z );
      }
      else
      {
  ...
  class LocalLaplaceAssembler
    : private AssemblyAssistant< LaplaceApp::DIM, double > {
  public:
    void operator()(const Element< double, DIM > &element,
                    const Quadrature< double > &quadrature,
                    GlobalAssembler< double, DIM >::LocalMatrix &lm) {
    ...
    void operator()(const Element< double, DIM > &element,
                    const Quadrature< double > &quadrature,
                    GlobalAssembler< double, DIM >::LocalVector &lv) {
      ...
      for (int q = 0; q < num_q; ++q) {
        ...
        for (int i = 0; i < num_local_dofs; ++i) {
          lv[dof_index(i, 0)] += wq * (my_f(x(q)) * phi(i, q)) * dJ;
        }
      }
    }
#if 1
  ...
  class L2ErrorIntegrator
    : private AssemblyAssistant< LaplaceApp::DIM, double > {
  public:
    L2ErrorIntegrator(const LAD::VectorType &dof) : dof_(dof) {}
  ...
  template < class ExactSol >
  class H1ErrorIntegrator
    : private AssemblyAssistant< LaplaceApp::DIM, double > {
  public:
    H1ErrorIntegrator(const LAD::VectorType &dof) : dof_(dof) {}
    ...
    void operator()(const Element< double, DIM > &element,
      ...
      for (int q = 0; q < num_q; ++q) {
        ...
        value += w(q) *
                 (dot(grad_u, grad_u) - 2. * dot(grad_u, approx_grad_u_[q]) +
...
private:
  ...
  doffem::DofPartition< double, DIM > dof_partition_;
  ...
  HpFemAssembler< double, DIM > global_asm_;
};
