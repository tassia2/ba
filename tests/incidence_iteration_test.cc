...
#define BOOST_TEST_MODULE incidence_iteration
...
void read_connectivities(const char *filename,
                         vector< pair< TDim, TDim > > &dims,
...
void check_incidence_iteration(
  TDim d1, TDim d2, const Mesh &mesh,
  const vector< vector< Id > > &correct_incidences) {
  LOG_DEBUG(1, "Testing " << d1 << " -> " << d2);
  BOOST_CHECK_EQUAL(mesh.num_entities(d1),
                    static_cast< int >(correct_incidences.size()));
  int i = 0;
  for (EntityIterator it = mesh.begin(d1); it != mesh.end(d1); ++it) {
    ...
    for (IncidentEntityIterator it2 = mesh.begin_incident(*it, d2);
      ...
      BOOST_TEST(j < correct_incidence.size());
      BOOST_CHECK(find(correct_incidence.begin(), correct_incidence.end(),
                      it2->id()) != correct_incidence.end());
      ++j;
    }
    BOOST_CHECK_EQUAL(j, correct_incidence.size());
    ++i;
  }
}

BOOST_AUTO_TEST_CASE(incidence_iteration) {
  ...
  for (unsigned int i = 0; i < incidence_lists.size(); ++i) {
    check_incidence_iteration(dims[i].first, dims[i].second, *mesh,
                              incidence_lists[i]);
  }
