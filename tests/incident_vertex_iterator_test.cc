...
#define BOOST_TEST_MODULE incident_vertex_iterator
...
BOOST_AUTO_TEST_CASE(incident_vertex_iterator) {
  ...
  std::string debug_filename =
    string("dbg_output.") + rank_str.str() + string(".log");
  ...
  if (rank == master_rank) {
    std::string filename =
      std::string(datadir) + std::string("dfg_bench3d_rect.inp");
    ...
    num_entities_on_proc.push_back(master_mesh->num_entities(tdim) -
                                   total_entities);
  ...
  LOG_DEBUG(1, "Received entity_package on process "
            << rank << ", num_coordinates = "
  ...
  for (EntityIterator it = mesh->begin(tdim); it != mesh->end(tdim); ++it) {
    ...
    if (vertex_ids.size() != incident_vertex_ids.size()) {
      LOG_DEBUG(
        1, "Not the same number of incident vertices: vertex_ids.size() == "
        << vertex_ids.size() << "incident_vertex_ids.size() == "
        << incident_vertex_ids.size());
    }
    BOOST_CHECK_EQUAL(vertex_ids.size(), incident_vertex_ids.size());
    for (int i = 0; i < static_cast< int >(vertex_ids.size()); ++i) {
      if (vertex_ids[i] != incident_vertex_ids[i]) {
        LOG_DEBUG(1,
                  "Different vertex ids: vertex_ids[i] = "
                  << vertex_ids[i] << ", "
