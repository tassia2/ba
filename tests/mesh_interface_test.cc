...
#define BOOST_TEST_MODULE mesh_interface
...
BOOST_AUTO_TEST_CASE(mesh_interface) {
  ...
  // test of level 1 regularly refined unit square: only boundary and regular
  // facets.
  test_regular_refined_unit_square();

  // test of irregularly refined unit square: boundary, regular and irregular
  // facets.
...
MeshPtr build_unit_square() {
  ...
  for (int k = 0; k < 4; ++k) {
    vertices[k] = builder.add_vertex(
        std::vector< double >(&vertex_coords[k][0], &vertex_coords[k][2]));
  }
...
void test_unit_square() {
  ...
  BOOST_CHECK_EQUAL(interface_list.size(), 4);
  ...
  for (InterfaceList::const_iterator it = interface_list.begin();
    ...
    BOOST_CHECK_EQUAL(it->num_slaves(), 0);

    InterfacePattern pattern = compute_interface_pattern(mesh, *it);
    LOG_DEBUG(2, pattern);
    BOOST_CHECK_EQUAL(pattern.master_facet_number(), it->master_facet_number());
    BOOST_CHECK_EQUAL(pattern.num_slaves(), 0);
    BOOST_CHECK_EQUAL(pattern.orientation(), -1);
  }
...
void test_regular_refined_unit_square() {
  ...
  BOOST_CHECK_EQUAL(interface_list.size(), 12);
...
/*
  const int correct_orientations[4][4] = {{-1, 0, 1, -1},      // cell 0
                                          {-1, -1, 1, -10},    // cell 1
                                          {-10, -1, -1, 1},    // cell 2
                                          {-10, -10, -1, -1}}; // cell 3
*/
...
/*
  const int correct_slave_facets[4][4] = {{-1, 3, 0, -1},      // cell 0
                                          {-1, -1, 0, -10},    // cell 1
                                          {-10, -1, -1, 1},    // cell 2
                                          {-10, -10, -1, -1}}; // cell 3
*/
  ...
  for (InterfaceList::const_iterator it = interface_list.begin();
    ...
    BOOST_CHECK_NE(correct_slave_cells[master_cell][master_facet_number], -10);

    if (correct_slave_cells[master_cell][master_facet_number] == -1) {
      BOOST_CHECK_EQUAL(it->num_slaves(), 0);
    } else {
      BOOST_CHECK_EQUAL(it->num_slaves(), 1);
      BOOST_CHECK_EQUAL(it->slave_index(0),
                 correct_slave_cells[master_cell][master_facet_number]);
    }

    // check pattern
    InterfacePattern pattern = compute_interface_pattern(mesh, *it);
    LOG_DEBUG(2, pattern);
    BOOST_CHECK_EQUAL(pattern.master_facet_number(),
               master_facet_number); // always true

    if (correct_slave_cells[master_cell][master_facet_number] == -1) {
      BOOST_CHECK_EQUAL(pattern.num_slaves(), 0);
    } else {
      BOOST_CHECK_EQUAL(pattern.num_slaves(), 1);
      BOOST_CHECK_EQUAL(pattern.slave_facet_number(0),
                 correct_slave_facets[master_cell][master_facet_number]);
    }
    BOOST_CHECK_EQUAL(pattern.orientation(),
               correct_orientations[master_cell][master_facet_number]);
  }
...
/// The variable arguments should list the cell indices of the slaves, one after
/// the other. num_slaves extra arguments should hence be provided (num_slaves
/// can be 0).

void add_interface(InterfaceMap &interfaces, int master_cell, int master_facet,
                   int num_slaves, ...) {
  using std::make_pair;
  ...
  for (int i = 0; i < num_slaves; ++i) {
    ...
    interfaces.find(make_pair(master_cell, master_facet))
        ->second.add_slave(slave_cell, i); // TODO is i correct here??
  }
...
/// The variable arguments should list the facet numbers of the slave facets,
/// one after the other. num_slaves extra arguments should hence be provided
/// (num_slaves can be 0).

void add_pattern(PatternMap &patterns, int master_cell, int master_facet,
                 int orientation, int num_slaves, ...) {
  using std::make_pair;
  patterns.insert(
      make_pair(make_pair(master_cell, master_facet), InterfacePattern()));
  patterns[make_pair(master_cell, master_facet)].set_master_facet_number(
      master_facet);
  ...
  for (int i = 0; i < num_slaves; ++i) {
    ...
    if (num_slaves == 1) {
      // regular interface
      patterns[make_pair(master_cell, master_facet)].add_regular_slave_facet(
          slave_facet);
    } else {
      // irregular inteface
      // TODO: find correct arguments for call below.
      patterns[make_pair(master_cell, master_facet)].add_irregular_slave_facet(
          -1, -2);
    }
  }
...
void build_correct_interfaces_and_patterns_irregular_square(
    InterfaceMap &interfaces, PatternMap &patterns) {
...
/// \brief Checks an Interface object against the one in the provided map,
/// without taking into account the order of the slave cells.

std::vector< int > check_interface(const InterfaceMap &correct_interfaces,
                                   const Interface &test_interface) {
  using std::make_pair;
...
/// provided map, using the given order to permute the slave facets of the
/// object in the map.

void check_pattern(int master_cell, const PatternMap &correct_patterns,
                   const InterfacePattern &test_pattern,
                   const std::vector< int > &order) {
  using std::make_pair;
  const int master_facet = test_pattern.master_facet_number();

  PatternMap::const_iterator it =
      correct_patterns.find(make_pair(master_cell, master_facet));
  assert(it != correct_patterns.end());
  BOOST_CHECK_EQUAL(test_pattern.master_facet_number(),
             it->second.master_facet_number());
  BOOST_CHECK_EQUAL(test_pattern.num_slaves(), it->second.num_slaves());

  // check that slave facets correspond
  // NB: no check is performed that relative ordering corresponds between
  // interface and pattern
  for (int k = 0; k < test_pattern.num_slaves(); ++k) {
    BOOST_CHECK_EQUAL(test_pattern.slave_facet_number(k),
               it->second.slave_facet_number(order.at(k)));
  }

  BOOST_CHECK_EQUAL(test_pattern.orientation(), it->second.orientation());
}
...
void test_irregular_refined_unit_square() {
  ...
  InterfaceList interface_list = InterfaceList::create(mesh);
  BOOST_CHECK_EQUAL(interface_list.size(), 18);

  InterfaceMap correct_interfaces;
  PatternMap correct_patterns;
  build_correct_interfaces_and_patterns_irregular_square(correct_interfaces,
                                                         correct_patterns);
