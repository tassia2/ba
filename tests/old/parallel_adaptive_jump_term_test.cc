...
void JumpTest::assemble_jump_terms() {
  ...
  for (mesh::EntityIterator it = mesh_ptr->begin(DIMENSION),
    ...
    space_.aaa_GetDofIndices(0, *it, &global_dof_ids);
...
void JumpTest::visualize() {
  ...
  ParallelCellVisualization< double, DIMENSION > visu(space_, num_intervals, comm_,
                                           MASTER_RANK);
...
<<<<<<< HEAD
  visu.visualize(EvalFeFunction< LAD, DIMENSION >(space_, *(sol_)), "u");
=======
