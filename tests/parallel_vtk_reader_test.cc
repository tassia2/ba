...
#define BOOST_TEST_MODULE parallel_vtk_reader
...
BOOST_AUTO_TEST_CASE(parallel_vtk_reader) {
  ...
  std::string filename =
      std::string(datadir) + std::string("unitsquare_8_cells.pvtu");
  ...
  for (EntityIterator it = mesh->begin(tdim - 1); it != mesh->end(tdim - 1);
       ++it) {
    std::cout << *it << std::endl;
  }
  std::cout << "\n\n";
  ...
  for (EntityIterator it = mesh->begin(tdim - 2); it != mesh->end(tdim - 2);
       ++it) {
    std::cout << *it << std::endl;
  }
  std::cout << "\n\n";
