...
#define BOOST_TEST_MODULE refinement
...
BOOST_AUTO_TEST_CASE(refinement) {
  ...
  const int expected_num_new_cells =
    static_cast< int >(std::pow(static_cast< double >(8), NUM_REFINEMENTS));
  ...
  const int expected_num_new_cells2 =
    (new_num_cells - num_ref_cells) + num_ref_cells / 8;
  BOOST_CHECK_EQUAL(refined_mesh->num_entities(refined_mesh->tdim()),
