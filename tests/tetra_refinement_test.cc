...
#define BOOST_TEST_MODULE tetra_refinement
...
BOOST_AUTO_TEST_CASE(tetra_refinement) {
  ...
  const int expected_num_new_cells =
    8 * NUM_REFINED_CELLS_TYPE0 + 4 * NUM_REFINED_CELLS_TYPE1;
  const int expected_num_refined_cells =
    (num_cells - NUM_REFINED_CELLS) + expected_num_new_cells;
  BOOST_CHECK_EQUAL(refined_mesh->num_entities(refined_mesh->tdim()),
