...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    MASTER_WRITE("Usage : mpirun -np P " << argv[0]
                                         << " [dimension] filename\n");
    return 1;
  }
  ...
  MeshPtr local_mesh = partition_and_distribute(
      in_mesh, MASTER_RANK, MPI_COMM_WORLD, &partitioner, uniform_ref_steps);
  assert(local_mesh != 0);
  ...
  std::string out_filename =
      in_filename.substr(0, point_pos) + std::string(".pvtu");
