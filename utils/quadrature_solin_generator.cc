...
/// transformed.
///
/// Transformation used: eta_coord_hiflow = (eta_coord_solin - 1) / 2, eta = x,
/// y, z
...
void print_source_code(std::ostream &os,
                       const std::vector< Quadrature > &quadratures);
std::vector< int >
compute_order_size_map(const std::vector< Quadrature > &quadratures);
...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    std::cerr << "Usage: quadrature_solin_generator name dimension < "
                 "quadrature_file\n";
    return 1;
  }
  ...
  std::transform(class_name_lower.begin(), class_name_lower.end(),
...
void print_header_code(std::ostream &os) {
  ...
  std::transform(class_name_upper.begin(), class_name_upper.end(),
...
void print_source_code(std::ostream &os,
  ...
  for (int i = 0; i < num_quadratures; ++i) {
    ...
    for (int d = 0; d < g_dimension; ++d) {
      os << "  " << coord_char[d] << "_[cntr].reserve(size); " << coord_char[d]
         << "_[cntr].insert(" << coord_char[d] << "_[cntr].begin(), "
         << coord_char[d] << i << ", " << coord_char[d] << i << " + size);\n";
    }
    ...
    os << "  "
       ...
       << "w_[cntr].insert(w_[cntr].begin(), w" << i << ", w" << i
       << " + size);\n";
...
void print_copyright(std::ostream &os) {
  os << "// Copyright (C) 2011-2015 Vincent Heuveline\n"
     << "//\n"
     << "// HiFlow3 is free software: you can redistribute it and/or modify it "
        "under the\n"
     ...
     << "// later version.\n"
     << "// \n"
     << "// HiFlow3 is distributed in the hope that it will be useful, but "
        "WITHOUT ANY\n"
     << "// WARRANTY; without even the implied warranty of MERCHANTABILITY or "
        "FITNESS FOR\n"
     << "// A PARTICULAR PURPOSE.  See the GNU Lesser General Public License "
        "for more\n"
     << "// details.\n"
     << "//\n"
     ...
     << "\n"
     << "// This code was generated with quadrature_solin_generator by Staffan "
        "Ronnas.\n"
     << "// The quadrature data comes from the book \n"
     << "// P. Solin, et. al., \"Higher-Order Finite Element Methods\"\n"
     << "// Michael Schick developed the original class on which the generated "
        "code is based.\n"
     << "\n"
     << "\n";
}
...
void print_debug(const std::vector< Quadrature > &quadratures) {
  ...
  for (int i = 0; i < static_cast< int >(quadratures.size()); ++i) {
    ...
    std::cout << "Quadrature of order " << quadratures[i].order << " with "
              << num_p << " points\n";

    for (int d = 0; d < g_dimension; ++d) {
      std::copy(quadratures[i].coords[d].begin(),
                quadratures[i].coords[d].end(),
                std::ostream_iterator< long double >(std::cout, " "));
      std::cout << "\n";
    }
    std::copy(quadratures[i].weights.begin(), quadratures[i].weights.end(),
              std::ostream_iterator< long double >(std::cout, " "));
    std::cout << "\n\n";
  }
}
